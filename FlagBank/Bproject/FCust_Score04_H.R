...
FCust_score4minMaxLst <- list( "pBanN.r"=c(min=0,max=1) ,"pBanN.n"=c(min=0,max=1) ,"pLCol"=c(min=0,max=1) 
...
FCust_score4minMaxLst.hard <- list( "pBanN.r"=c(min=0,max=1) ,"pBanN.n"=c(min=0,max=0) ,"pLCol"=c(min=0,max=1) 
                                    ,"pE3"=c(min=0,max=0) ,"pE4"=c(min=0,max=0) ,"pMH"=c(min=0,max=0) ,"pfNum"=c(min=0,max=0) 
                            ,"iBanN"=c(min=0,max=1) ,"iLCol"=c(min=0,max=1) 
                                    ,"iE3"=c(min=0,max=0) ,"iE4"=c(min=0,max=0) ,"iMH"=c(min=0,max=0) ,"ifNum"=c(min=0,max=0) 
...
bFCust.score4HardFlag <- function( hName ,pName ){
	hardPh	<- c("sfcLate")
	hardH	<- c("basic")
...
bFCust.A_score4_A_A <- function(  ){
	...
	rObj$createCutter <- function( ctrlCfg ,tgtId=c(hName="", mName="", pName="", fcName="") ,auxInfo=c(auxInfo="") ){
		...
		cutterObj$hardFlag <- bFCust.score4HardFlag(cutterObj$idObj["hName"],cutterObj$idObj["pName"])$flag
        cutterObj$minMax <- FCust_score4minMaxLst[[tgtId["fcName"]]]
		if( cutterObj$hardFlag["p"] ){
			cutterObj$minMax <- FCust_score4minMaxLst.hard[[tgtId["fcName"]]]
		}
...
bFCust.A_score4_A_Row01 <- function(  ){
	...
	rObj$cutFLst[[1+length(rObj$cutFLst)]] <- function( smRow ,hardFlag ){
		...
		evt <- bUtil.getEvtVal( smRow ,FCust_score4EvtLst )
		cnt <- sum( !is.na(evt) )
	...
	rObj$createCutter <- function( tgtId=c(hName="", mName="", pName="") ,auxInfo=c(auxInfo="") ){
		...
		cutterObj$hardObj <- bFCust.score4HardFlag(cutterObj$idObj["hName"],cutterObj$idObj["pName"])
...
#	[score4:Col Cutter(Rebound/Sequencial)] -----------------------------------------------------
bFCust.A_score4_A_rReb01 <- function(  ){	# evt rebind
	#	row rebind라고는 했지만 사실 seq 이다. 
	#	즉 이전 evt 컬럼이 다음에도 동일 evt가 일어나는지 체크 (다음에서 추가적으로 발생하는 evt는 상관없음)
	rObj <- list( )
	rObj$defId <- c( typ="cust_RReb"	,hName="*"	,mName="score4"	,pName="*"	,rFId="rReb01" )	# row filt ID
	rObj$description <- sprintf("(cust)  ")
	...
	rObj$createCutter <- function( hMtxLst ,tgtId=c(hName="", mName="", pName="") ,auxInfo=c(auxInfo="") ){
		...
		if( !is.null(scoreMtx) ){	# build checkLst
			...
			evtChkInfo <- list( cutId="grp.p" ,fireThld=NA ,evtLst=rObj$evtLst[cName] )
			...
			evtChkInfo <- list( cutId="grp.i" ,fireThld=NA ,evtLst=rObj$evtLst[cName] )
			evtChkInfo$evtLast <- bUtil.getEvtVal( scoreMtx.last ,evtChkInfo$evtLst )
			cutterObj$evtChkLst[[1+length(cutterObj$evtChkLst)]] <- evtChkInfo
			...
			#	byHIdx에서 다수 발생 체크
			evtChkInfo <- list( cutId="evtAll" ,fireThld=ifelse( cutterObj$hardFlag["p"],1,2) ,evtLst=rObj$evtLst )
			evtChkInfo$evtLast <- bUtil.getEvtVal( scoreMtx.last ,evtChkInfo$evtLst )
			cutterObj$evtChkLst[[1+length(cutterObj$evtChkLst)]] <- evtChkInfo
	...
	return( rObj )
} # bFCust.A_score4_A_rReb01()

bFCust.A_score4_A_rRebAA <- function(  ){	#	이전 마지막 score(cutterObj$lastRow) 값과의 일치여부(raw Value) 

	rObj <- list( )
	rObj$defId <- c( typ="cust_RReb"	,hName="*"	,mName="score4"	,pName="*"	,rFId="rRebAA" )	# row filt ID
	rObj$description <- sprintf("(cust)  ")
	...
	rObj$createCutter <- function( lastMtx ,tgtId=c(hName="", mName="", pName="") ,auxInfo=c(auxInfo="") ){
		...
		cutterObj$cut <- function( scoreMtx ,alreadyDead=NULL ){
			...
			for( idx in seq_len(val.len) ){
				...
				diffCnt <- sum( cutterObj$lastRow!=scoreMtx[idx,] )
				# if( !bUtil.in(diffCnt,c(min=1,max=6)) ){
				if( 0==diffCnt ) {
					infoStr <- sprintf("cut Id : rRebAA (raw all Hpn %d diffCnt:%d)",sum(cutterObj$lastRow>0),diffCnt )
					cutLst[[1+length(cutLst)]] <- list( idx=idx ,idObjDesc=cutterObj$idObjDesc ,info=infoStr )
				}
			}

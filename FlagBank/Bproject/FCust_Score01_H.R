...
FCust_score1minMaxLst <- list( "rem0.num"=c(min=0,max=1) ,"rem0.len.tot"=c(min=0,max=3) ,"rem0.len.val"=c(min=0,max=3)
...
FCust_score1minMaxLst.hard <- list( "rem0.num"=c(min=0,max=1) ,"rem0.len.tot"=c(min=0,max=2) ,"rem0.len.val"=c(min=0,max=2)
								,"rem1.num"=c(min=0,max=2) ,"rem1.len.tot"=c(min=0,max=4) ,"rem1.len.val"=c(min=0,max=2)
								,"c0.num"=c(min=0,max=1) ,"c0.len.tot"=c(min=0,max=2) ,"c0.len.val"=c(min=0,max=2)
								,"c1.num"=c(min=0,max=2) ,"c1.len.tot"=c(min=0,max=4) ,"c1.len.val"=c(min=0,max=2)
								,"f0.num"=c(min=0,max=1) ,"f0.len.tot"=c(min=0,max=3) ,"f0.len.val"=c(min=0,max=2)
...
bFCust.score1HardFlag <- function( hName ,pName ){
...
bFCust.A_score1_A_A <- function(  ){
	...
	rObj$createCutter <- function( ctrlCfg ,tgtId=c(hName="", mName="", pName="", fcName="") ,auxInfo=c(auxInfo="") ){
		...
		if( cutterObj$hardFlag["p"] ){
			cutterObj$minMax <- FCust_score1minMaxLst.hard[[tgtId["fcName"]]]
		}
...
bFCust.A_score1_A_Row01 <- function(  ){
	...
	rObj$cutFLst[[1+length(rObj$cutFLst)]] <- function( smRow ,hardFlag ){
		...
		if( !bUtil.in(remN.len.val,surWin) ){
			crObj$cutFlag <- TRUE
	...
	rObj$createCutter <- function( tgtId=c(hName="", mName="", pName="") ,auxInfo=c(auxInfo="") ){
		...
		cutterObj$cut <- function( scoreMtx ,alreadyDead=NULL ){
			...
			for( idx in seq_len(val.len) ){
				...
				lst <- lapply( rObj$cutFLst ,function( pFunc ){ pFunc( scoreMtx[idx,] ,hardFlag=cutterObj$hardFlag ) } )
...
bFCust.A_score1_A_rReb01 <- function(  ){	# evt rebind
	...
	rObj$createCutter <- function( hMtxLst ,tgtId=c(hName="", mName="", pName="") ,auxInfo=c(auxInfo="") ){
		...
		if( !is.null(scoreMtx) ){	# build checkLst
			...
			evtChkInfo <- list( cutId="grp0.len" ,fireThld=2 ,evtLst=rObj$evtLst[grp0.len] )
			evtChkInfo$evtLast <- bUtil.getEvtVal( scoreMtx.last ,evtChkInfo$evtLst )
			cutterObj$evtChkLst[[1+length(cutterObj$evtChkLst)]] <- evtChkInfo
			...
			evtChkInfo <- list( cutId="grp0.val" ,fireThld=2 ,evtLst=rObj$evtLst[grp0.val] )
			evtChkInfo$evtLast <- bUtil.getEvtVal( scoreMtx.last ,evtChkInfo$evtLst )
			cutterObj$evtChkLst[[1+length(cutterObj$evtChkLst)]] <- evtChkInfo

			evtChkInfo <- list( cutId="grp1.val" ,fireThld=2 ,evtLst=rObj$evtLst[grp1.val] )
			evtChkInfo$evtLast <- bUtil.getEvtVal( scoreMtx.last ,evtChkInfo$evtLst )
			cutterObj$evtChkLst[[1+length(cutterObj$evtChkLst)]] <- evtChkInfo
...
bFCust.byFCol_A_score1_A_rReb01 <- function( ){
	...
	rObj$fireThld.min <- c(  "rem0.num"=2 ,"rem0.len.tot"=1 ,"rem0.len.val"=1
							...
							,"f1.num"=2 ,"f1.len.tot"=2 ,"f1.len.val"=1
							,"zwNum"=3	,"zwC1Num"=1 
					)
...
bFCust.byHIdx_A_score1 <- function( ){
	...
	rObj$createCutter <- function( mtxLst=NULL ,tgtId=c(hName="", mName="") ,auxInfo=c(auxInfo="") ){
		...
		cutterObj$idObj <- rObj$defId
		cutterObj$idObj[names(tgtId)] <- tgtId
		if( cutterObj$defId["mName"]!=tgtId["mName"] )	return(cutterObj)
		...
		cutterObj$cutLst[["F_stdEvtChk"]] <- function( scoreMtx ,chkEvt=NULL ){
			...
			evtCnt <- sum(colSum >= 3)
			maxThld <- 5		;evtCnt.tot <- evtCnt.tot + evtCnt
		...
		cutterObj$cutLst[["F_RareCol"]] <- function( scoreMtx ,chkEvt ){	# working
			...
			chkLst[["remN.len.val"]] <- list( max=3 ,evt=2 ,cName=c("rem0.len.val","rem1.len.val") )
			...
			grpLst <- list()
			grpLst[["grp.num"]] <- list( max=3 ,evt=1 ,cName1=c("rem0.num","c0.num","f0.num") ,cName2=c("rem1.num","c1.num","f1.num") )
			grpLst[["grp.len"]] <- list( max=3 ,evt=1 ,cName1=c("rem0.len.tot","c0.len.tot","f0.len.tot") ,cName2=c("rem1.len.tot","c1.len.tot","f1.len.tot") )
			...
			evtCnt <- sum(gCnt>=2)
			if( evtCnt >= 3 )	rCutId <- c( rCutId, sprintf("gCnt %d",evtCnt) )

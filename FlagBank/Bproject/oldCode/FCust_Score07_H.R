FCust_score7EvtLst <- list( "pBanN.r"=1 ,"pBanN.n"=1 ,"pLCol"=1 ,"pE3"=1 ,"pE4"=1 ,"pMH"=1 ,"pfNum"=1
                            ,"iBanN"=1 ,"iLCol"=1 ,"iE3"=1 ,"iE4"=1 ,"iMH"=1 ,"ifNum"=1
                            ,"FVa.m"=2:4 ,"FVa.c"=2:4 ,"aFV.m"=2:4 ,"aFV.c"=2:4
                            ,"m4"=1
							)

FCust_score7minMaxLst <- list( "pBanN.r"=c(min=0,max=1) ,"pBanN.n"=c(min=0,max=1) ,"pLCol"=c(min=0,max=1) 
                                    ,"pE3"=c(min=0,max=0) ,"pE4"=c(min=0,max=0) ,"pMH"=c(min=0,max=0) ,"pfNum"=c(min=0,max=0) 
                            ,"iBanN"=c(min=0,max=1) ,"iLCol"=c(min=0,max=1) 
                                    ,"iE3"=c(min=0,max=0) ,"iE4"=c(min=0,max=0) ,"iMH"=c(min=0,max=0) ,"ifNum"=c(min=0,max=2) 
                            ,"FVa.m"=c(min=0,max=2) ,"FVa.c"=c(min=0,max=2) ,"aFV.m"=c(min=0,max=2) ,"aFV.c"=c(min=0,max=2) 
                            ,"m4"=c(min=0,max=0)
							)
FCust_score7minMaxLst.hard <- list( "pBanN.r"=c(min=0,max=1) ,"pBanN.n"=c(min=0,max=1) ,"pLCol"=c(min=0,max=0) 
                                    ,"pE3"=c(min=0,max=0) ,"pE4"=c(min=0,max=0) ,"pMH"=c(min=0,max=0) ,"pfNum"=c(min=0,max=0) 
                            ,"iBanN"=c(min=0,max=1) ,"iLCol"=c(min=0,max=0) 
                                    ,"iE3"=c(min=0,max=0) ,"iE4"=c(min=0,max=0) ,"iMH"=c(min=0,max=0) ,"ifNum"=c(min=0,max=0) 
                            ,"FVa.m"=c(min=0,max=2) ,"FVa.c"=c(min=0,max=2) ,"aFV.m"=c(min=0,max=2) ,"aFV.c"=c(min=0,max=2) 
                            ,"m4"=c(min=0,max=0)
							)

bFCust.score7HardFlag <- function( hName ,pName ){
	hardPh	<- c("sfcLate")
	hardH	<- c("basic")
...
#	[score7:Col Cutter(1 col)] ------------------------------------------------------------------
#	c( typ="cust"	,hName="*"	,mName="score7"	,pName="*"	,fcName="*" )
bFCust.A_score7_A_A <- function(  ){

	rObj <- list( defId=c( typ="cust"	,hName="*"	,mName="score7"	,pName="*"	,fcName="*" ) )
	...
	rObj$createCutter <- function( ctrlCfg ,tgtId=c(hName="", mName="", pName="", fcName="") ,auxInfo=c(auxInfo="") ){
		...
		cutterObj$hardFlag <- bFCust.score7HardFlag(cutterObj$idObj["hName"],cutterObj$idObj["pName"])$flag
        cutterObj$minMax <- FCust_score7minMaxLst[[tgtId["fcName"]]]
		if( cutterObj$hardFlag["p"] ){
			cutterObj$minMax <- FCust_score7minMaxLst.hard[[tgtId["fcName"]]]
		}
...
#	[score7:Col Cutter(N col)] ------------------------------------------------------------------
bFCust.A_score7_A_Row01 <- function(  ){
	rObj <- list( )
	rObj$defId <- c( typ="c_NCol.5"	,hName="*"	,mName="score7"	,pName="*"	,rFId="Row01" )	# row filt ID
	rObj$description <- sprintf("(cust)  ")

	rObj$evtLst <- FCust_score7EvtLst
	...
	rObj$cutFLst[[1+length(rObj$cutFLst)]] <- function( smRow ,hardFlag ){
		...
		surWin <- if( hardFlag["p"] ) c(min=0,max=1) else c(min=0,max=3)
		if( !bUtil.in(iCnt,surWin) ){
			crObj$cutFlag <- TRUE
	...
	rObj$createCutter <- function( tgtId=c(hName="", mName="", pName="") ,auxInfo=c(auxInfo="") ){
		...
		cutterObj$idObj <- rObj$defId
		cutterObj$idObj[names(tgtId)] <- tgtId
		cutterObj$hardObj <- bFCust.score7HardFlag(cutterObj$idObj["hName"],cutterObj$idObj["pName"])
	...
	return( rObj )
} # bFCust.A_score7_A_Row01( )

#	[score7:Col Cutter(Rebound/Sequencial)] -----------------------------------------------------
bFCust.A_score7_A_rReb01 <- function(  ){	# evt rebind
	#	row rebind라고는 했지만 사실 seq 이다. 
	#	즉 이전 evt 컬럼이 다음에도 동일 evt가 일어나는지 체크 (다음에서 추가적으로 발생하는 evt는 상관없음)
	rObj <- list( )
	rObj$defId <- c( typ="cust_RReb"	,hName="*"	,mName="score7"	,pName="*"	,rFId="rReb01" )	# row filt ID
	rObj$description <- sprintf("(cust)  ")
	...
	return( rObj )
} # bFCust.A_score7_A_rReb01()

bFCust.A_score7_A_rRebAA <- function(  ){	#	이전 마지막 score(cutterObj$lastRow) 값과의 일치여부(raw Value) 

	rObj <- list( )
	rObj$defId <- c( typ="cust_RReb"	,hName="*"	,mName="score7"	,pName="*"	,rFId="rRebAA" )	# row filt ID
	rObj$description <- sprintf("(cust)  ")
	...
	return( rObj )
} # bFCust.A_score7_A_rRebAA()


#	[score7:byFCol(Rebound/Sequencial)] ---------------------------------------------------------
...
bFCust.byFCol_A_score7_A <- function( ){

	rObj <- list( )
	rObj$defId <- c( typ="c_byFCol"	,hName="*"	,mName="score7"	,fcName="*"  )
	rObj$description <- sprintf("(cust)  ")
	...
	if( TRUE ){	# build cutFLst
		...
		rObj$cutFLst[["pBanN.n"]] <- function( smRow ,fcName ){
		...
		rObj$cutFLst[["pLCol"]] <- function( smRow ,fcName ){
		...
		rObj$cutFLst[["pE3"]] <- function( smRow ,fcName ){
		...
		rObj$cutFLst[["pE4"]] <- function( smRow ,fcName ){
		...
		rObj$cutFLst[["pMH"]] <- function( smRow ,fcName ){
		...
		rObj$cutFLst[["pfNum"]] <- function( smRow ,fcName ){
		...
		rObj$cutFLst[["iBanN"]] <- function( smRow ,fcName ){
		...
		rObj$cutFLst[["iLCol"]] <- function( smRow ,fcName ){
		...
		rObj$cutFLst[["iE3"]] <- function( smRow ,fcName ){
		...
		rObj$cutFLst[["iE4"]] <- function( smRow ,fcName ){
		...
		rObj$cutFLst[["iMH"]] <- function( smRow ,fcName ){
		...
		rObj$cutFLst[["ifNum"]] <- function( smRow ,fcName ){
		...
		rObj$cutFLst[["FVa.m"]] <- function( smRow ,fcName ){
		...
		rObj$cutFLst[["FVa.c"]] <- function( smRow ,fcName ){
		...
		rObj$cutFLst[["aFV.m"]] <- function( smRow ,fcName ){
		...
		rObj$cutFLst[["aFV.c"]] <- function( smRow ,fcName ){
		...
		rObj$cutFLst[["m4"]] <- function( smRow ,fcName ){
	...
	return( rObj )
} # bFCust.byFCol_A_score7_A( )

#	typ="c_byFCol"	,hName="*"	,mName="score7"	,pName="*"	,fcName="*" )
bFCust.byFCol_A_score7_A_rReb01 <- function( ){
	#	row rebind라고는 했지만 사실 seq 이다. 
	#	즉 이전 evt 발생 phase에서 다음에도 동일 evt가 일어나는지 체크(다음에서 추가적으로 발생하는 evt는 상관없음.)
	rObj <- list( )
	rObj$defId <- c( typ="c_byFCol"	,hName="*"	,mName="score7"	,pName="*"	,fcName="*" )	# row filt ID
	rObj$description <- sprintf("(cust)  ")

	rObj$evtLst <- FCust_score7EvtLst
	rObj$fireThld.min <- c( "pBanN.r"=1 ,"pBanN.n"=1 ,"pLCol"=1 ,"pE3"=1 ,"pE4"=1 ,"pMH"=1 ,"pfNum"=1
							,"iBanN"=1 ,"iLCol"=1 ,"iE3"=1 ,"iE4"=1 ,"iMH"=1 ,"ifNum"=1
							,"FVa.m"=1 ,"FVa.c"=1 ,"aFV.m"=1 ,"aFV.c"=1 ,"m4"=0
					)
	...
	return( rObj )
} # bFCust.byFCol_A_score7_A_rReb01( )

#	c( typ="c_byFCol"	,hName="*"	,mName="score7"	,fcName="*"  )
bFCust.byFCol_A_score7_A_rRebAA <- function( ){
	rObj <- list( )
	rObj$defId <- c( typ="c_byFCol"	,hName="*"	,mName="score7"	,fcName="*"  )
	rObj$description <- sprintf("(cust)  ")
	...
	return( rObj )
} # bFCust.byFCol_A_score7_A_rRebAA( )
...
#		c( typ="cust_byHIdx"	,hName="*"	,mName="score7" )
bFCust.byHIdx_A_score7 <- function( ){

	rObj <- list( )
	rObj$defId <- c( typ="cust_byHIdx"	,hName="*"	,mName="score7" )
	rObj$description <- sprintf("(cust)  ")
	...
	rObj$createCutter <- function( mtxLst=NULL ,tgtId=c(hName="", mName="") ,auxInfo=c(auxInfo="") ){
		...
		if( tgtId["mName"]==cutterObj$defId["mName"] ){
			cutterObj$chkEvt.last <- bFCust.get_byHIdx_score7ChkEvt( mtxLst[[length(mtxLst)]] )
		}
		...
		cutterObj$cut <- function( scoreMtx ,aIdx ){
			...
			chkEvt <- bFCust.get_byHIdx_score7ChkEvt( scoreMtx )
		...
		cutterObj$cutLst[["F_stdEvtChk"]] <- function( scoreMtx ,chkEvt=NULL ){
			...
			maxThld <- 1		;evtCnt.tot <- evtCnt.tot + evtCnt		# 이 부분은 재검토 좀 하자. H879
			...
			if( !is.null(stdEvt$rebSummReb) ){
				...
				maxThld <- 7		;evtCnt.tot <- evtCnt.tot + rsrSum
		...
		cutterObj$cutLst[["F_matEvt"]] <- function( scoreMtx ,chkEvt=NULL ){
            ...
            if( 1 < sum(scoreMtx>0) ){  #  0 이 너무 많다면 적용.
                banPastH <- 6	# 바로 이전 H를 제외한 크기(rawDiffCnt에서 체크되므로)
                surWindow <- c(min=1,max=35)
		...
		cutterObj$cutLst[["F_colMat"]] <- function( scoreMtx ,chkEvt=NULL ){
			...
			# raw match -------------------------------------------
			surWindow <- c(min=0,max=9)	# survive window
			for( cIdx in 2:ncol(scoreMtx) ){
	...
	return( rObj )
} # bFCust.byHIdx_A_score7



bFCust.get_byHIdx_score7ChkEvt <- function( scoreMtx ){
...
} # bFCust.get_byHIdx_score7ChkEvt( )

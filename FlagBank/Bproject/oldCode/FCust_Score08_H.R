...
FCust_score8minMaxLst.hard <- list( "c31"=c(min=0,max=1),"c32"=c(min=0,max=1),"c33"=c(min=0,max=1),"c34"=c(min=0,max=1)
								,"c21"=c(min=0,max=1),"c22"=c(min=0,max=1),"c23"=c(min=0,max=1),"c24"=c(min=0,max=1),"c25"=c(min=0,max=1)
								,"max3"=c(min=0,max=0),"min3"=c(min=0,max=1),"max2"=c(min=0,max=1),"min2"=c(min=0,max=1)
								,"cTbl"=c(min=0,max=0)
							)

bFCust.score8HardFlag <- function( hName ,pName ){
	hardPh	<- c("sfcLate")		;hardH	<- c("basic")

	hFlag <- hName %in% hardPh	;pFlag <- pName %in% hardH
...
bFCust.A_score8_A_A <- function(  ){
	...
	rObj$createCutter <- function( ctrlCfg ,tgtId=c(hName="", mName="", pName="", fcName="") ,auxInfo=c(auxInfo="") ){
		...
		cutterObj$hardFlag <- bFCust.score8HardFlag(cutterObj$idObj["hName"],cutterObj$idObj["pName"])$flag
        cutterObj$minMax <- FCust_score8minMaxLst[[tgtId["fcName"]]]
		if( cutterObj$hardFlag["p"] ){
			cutterObj$minMax <- FCust_score8minMaxLst.hard[[tgtId["fcName"]]]
		}
...
bFCust.A_score8_A_Row01 <- function(  ){
	...
	rObj$cutFLst[[1+length(rObj$cutFLst)]] <- function( smRow ,hardFlag ){
		...
		surWin <- if( hardFlag["p"] ) c(min=0,max=2) else c(min=0,max=5)
		if( !bUtil.in(cntEvt,surWin) ){
			crObj$cutFlag <- TRUE
		...
		surWin <- if( hardFlag["p"] ) c(min=0,max=2) else c(min=0,max=3)
		if( !bUtil.in(cntC2,surWin) ){
			crObj$cutFlag <- TRUE
		...
		cnt <- sum( smRow==0 )
		if( !bUtil.in(cnt,c(min=9,max=14)) ){
			crObj$cutFlag <- TRUE
	...
	rObj$createCutter <- function( tgtId=c(hName="", mName="", pName="") ,auxInfo=c(auxInfo="") ){
		...
		cutterObj$idObj <- rObj$defId
		cutterObj$idObj[names(tgtId)] <- tgtId
		cutterObj$hardObj <- bFCust.score8HardFlag(cutterObj$idObj["hName"],cutterObj$idObj["pName"])
...
bFCust.A_score8_A_rReb01 <- function(  ){	# evt rebind
	...
	rObj$createCutter <- function( hMtxLst ,tgtId=c(hName="", mName="", pName="") ,auxInfo=c(auxInfo="") ){
		...
		if( !is.null(scoreMtx) ){	# build checkLst
			...
			evtChkInfo <- list( cutId="c3" ,fireThld=2 ,evtLst=rObj$evtLst[cName] )
			evtChkInfo$evtLast <- bUtil.getEvtVal( scoreMtx.last ,evtChkInfo$evtLst )
			cutterObj$evtChkLst[[1+length(cutterObj$evtChkLst)]] <- evtChkInfo
...
bFCust.A_score8_A_rRebAA <- function(  ){	#	이전 마지막 score(cutterObj$lastRow) 값과의 일치여부(raw Value) 
	...
	rObj$createCutter <- function( hMtxLst ,tgtId=c(hName="", mName="", pName="") ,auxInfo=c(auxInfo="") ){
		...
		if( is.null(scoreMtxObj) ){	cutterObj$lastRow <- NULL
			...
			hpnCnt <- sum(cutterObj$lastRow[cName]>0)
			if( hpnCnt<=ifelse(cutterObj$hardFlag["p"],0,1) ){
		...
		cutterObj$cut <- function( scoreMtx ,alreadyDead=NULL ){
			...
			for( idx in seq_len(val.len) ){
				...
				if( all(matFlag) ){
					infoStr <- sprintf("cut Id : %s(%d)",cutterObj$idObjDesc["rFId"],sum(cutterObj$lastRow[cName]>0) )
					cutLst[[1+length(cutLst)]] <- list( idx=idx ,idObjDesc=cutterObj$idObjDesc ,info=infoStr )
				}
			}
...
bFCust.byHIdx_A_score8 <- function( ){
	...
	rObj$createCutter <- function( mtxLst=NULL ,tgtId=c(hName="", mName="") ,auxInfo=c(auxInfo="") ){
		...
		cutterObj$cutLst[["F_stdEvtChk"]] <- function( scoreMtx ,chkEvt=NULL ){
			...
			maxThld <- 4		;evtCnt.tot <- evtCnt.tot + rebCnt
			...
			evtCnt <- sum(colSum >= 3)
			maxThld <- 3		;evtCnt.tot <- evtCnt.tot + evtCnt
		...
		cutterObj$cutLst[["F_matEvt"]] <- function( scoreMtx ,chkEvt=NULL ){
			...
			# match happen count ----------------------------------
			if( 4<=sum( matMtx ) ) rCutId <- c( rCutId, sprintf("matHpnCnt.%d",sum( matMtx )) )
		...
		cutterObj$cutLst[["F_hpnInfo"]] <- function( scoreMtx ,chkEvt ){	# working
			...
			surWindow <- c(min=0,max=14)	# 0도 발생하긴 함.

...
#	[score6:Col Cutter(Rebound/Sequencial)] -----------------------------------------------------
bFCust.A_score6_A_rReb01 <- function(  ){	# evt rebind
	#	row rebind라고는 했지만 사실 seq 이다. 
	#	즉 이전 evt 컬럼이 다음에도 동일 evt가 일어나는지 체크 (다음에서 추가적으로 발생하는 evt는 상관없음)
	rObj <- list( )
	rObj$defId <- c( typ="cust_RReb"	,hName="*"	,mName="score6"	,pName="*"	,rFId="rReb01" )	# row filt ID
	rObj$description <- sprintf("(cust)  ")
	...
	return( rObj )
} # bFCust.A_score6_A_rReb01()

bFCust.A_score6_A_rRebAA <- function(  ){	#	이전 마지막 score(cutterObj$lastRow) 값과의 일치여부(raw Value) 

	rObj <- list( )
	rObj$defId <- c( typ="cust_RReb"	,hName="*"	,mName="score6"	,pName="*"	,rFId="rRebAA" )	# row filt ID
	rObj$description <- sprintf("(cust)  ")
...
bFCust.byFCol_A_score6_A <- function( ){

	rObj <- list( )
	rObj$defId <- c( typ="c_byFCol"	,hName="*"	,mName="score6"	,fcName="*"  )
	rObj$description <- sprintf("(cust)  ")
	...
	return( rObj )
} # bFCust.byFCol_A_score6_A( )

#	typ="c_byFCol"	,hName="*"	,mName="score6"	,pName="*"	,fcName="*" )
bFCust.byFCol_A_score6_A_rReb01 <- function( ){
	#	row rebind라고는 했지만 사실 seq 이다. 
	#	즉 이전 evt 발생 phase에서 다음에도 동일 evt가 일어나는지 체크(다음에서 추가적으로 발생하는 evt는 상관없음.)
	rObj <- list( )
	rObj$defId <- c( typ="c_byFCol"	,hName="*"	,mName="score6"	,pName="*"	,fcName="*" )	# row filt ID
	rObj$description <- sprintf("(cust)  ")
	...
	return( rObj )
} # bFCust.byFCol_A_score6_A_rReb01( )

#	c( typ="c_byFCol"	,hName="*"	,mName="score6"	,fcName="*"  )
bFCust.byFCol_A_score6_A_rRebAA <- function( ){
	rObj <- list( )
	rObj$defId <- c( typ="c_byFCol"	,hName="*"	,mName="score6"	,fcName="*"  )
	rObj$description <- sprintf("(cust)  ")
	...
	return( rObj )
} # bFCust.byFCol_A_score6_A_rRebAA( )
...
#		c( typ="cust_byHIdx"	,hName="*"	,mName="score6" )
bFCust.byHIdx_A_score6 <- function( ){

	rObj <- list( )
	rObj$defId <- c( typ="cust_byHIdx"	,hName="*"	,mName="score6" )
	rObj$description <- sprintf("(cust)  ")
	...
	rObj$createCutter <- function( mtxLst=NULL ,tgtId=c(hName="", mName="") ,auxInfo=c(auxInfo="") ){
		...
		if( tgtId["mName"]==cutterObj$defId["mName"] ){
			cutterObj$chkEvt.last <- bFCust.get_byHIdx_score6ChkEvt( mtxLst[[length(mtxLst)]] )
		}
		...
		cutterObj$cut <- function( scoreMtx ,aIdx ){
			...
			chkEvt <- bFCust.get_byHIdx_score6ChkEvt( scoreMtx )
	...
	return( rObj )
} # bFCust.byHIdx_A_score6



bFCust.get_byHIdx_score6ChkEvt <- function( scoreMtx ){
...
} # bFCust.get_byHIdx_score6ChkEvt( )

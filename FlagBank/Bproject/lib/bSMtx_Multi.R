...
crMName <- "bSMScr01R"  # Cut-Result, Score N, Raw val only
if( TRUE ){
    bSMtxMLst[[crMName]] <- function( hCRScr=NULL ){
...
crMName <- "bSMScr01E"  # Cut-Result, Score N, Evt val only
if( TRUE ){
    bSMtxMLst[[crMName]] <- function( hCRScr=NULL ){
        # hCRScr : cutRst1Score 히스토리. Rebound 체크 기능은 나중에 구현한다.
...
crMName <- "bSMScr01PhEvt"  # phase 별로 evt 갯수 제한.(한 개 phase에서 다수 mName을 대상으로..)
if( TRUE ){

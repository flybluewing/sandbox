...
if( TRUE ){
    ...
    bNScrExtMtxCfg[[mName]] <- list()
    bNScrExtMtxCfg[[mName]]$filter01 <- list(
    ...
    bNScrExtMtxCfg[[mName]]$filter02 <- list(
        mName = mName   ,style=c( freqZero=TRUE )
        ,fCol = list(
...
if( TRUE ){    # sample
    bNScrExtMtxCfg[[mName]] <- list()
    bNScrExtMtxCfg[[mName]]$freqValReb <- list(
...
for( mName in names( bNScrExtMtxCfg ) ){              # mName <- names( bNScrExtMtxCfg )[1]
    for( fName in names(bNScrExtMtxCfg[[mName]]) ){   # fName <- names(bNScrExtMtxCfg[[mName]])[1]
        # bNScrExtMtxCfg[[mName]][[fName]]
        for( fcName in names(bNScrExtMtxCfg[[mName]][[fName]]$fCol) ){    # fcName <- names(bNScrExtMtxCfg[[mName]][[fName]]$fCol)[1]
            colnames(bNScrExtMtxCfg[[mName]][[fName]]$fCol[[fcName]]$rng) <- c("lev1","lev2")
            rownames(bNScrExtMtxCfg[[mName]][[fName]]$fCol[[fcName]]$rng) <- c("min","max")

            colnames(bNScrExtMtxCfg[[mName]][[fName]]$fCol[[fcName]]$evt) <- c("lev","val")

            if( is.null(bNScrExtMtxCfg[[mName]][[fName]]$fCol[[fcName]]$evtMax.fCol ) ){
                # fCol 별 전체 phase 대상으로 evt 발생 제한.
                bNScrExtMtxCfg[[mName]][[fName]]$fCol[[fcName]]$evtMax.fCol <- c( minLev=2 ,maxHpn=1 )
            }
            if( is.null(bNScrExtMtxCfg[[mName]][[fName]]$fCol[[fcName]]$forbidEvtReb) ){
                bNScrExtMtxCfg[[mName]][[fName]]$fCol[[fcName]]$forbidEvtReb <- c(2,3,4)
            }
        }

        if( is.null(bNScrExtMtxCfg[[mName]][[fName]]$evtMax) ){
            ...
            bNScrExtMtxCfg[[mName]][[fName]]$evtMax     <- evtMax
        }
        if( is.null(bNScrExtMtxCfg[[mName]][[fName]]$rowReb) ){
            bNScrExtMtxCfg[[mName]][[fName]]$rowReb <- c( rawMin=1 ,lowE=2 ,rareE=1 ,dupESum=2 )
        }
        if( is.null(bNScrExtMtxCfg[[mName]][[fName]]$rowRebDup) ){  # 조건 : >=
            bNScrExtMtxCfg[[mName]][[fName]]$rowRebDup <- c( lowE=1 ,rareE=1 )
        }
        if( is.null(bNScrExtMtxCfg[[mName]][[fName]]$summMtx) ){
        ...
        if( is.null(bNScrExtMtxCfg[[mName]][[fName]]$summMtx.reb ) ){
            ...
            bNScrExtMtxCfg[[mName]][[fName]]$summMtx.reb <- matrix( thldVal ,byrow=T
                        ,ncol=length(cName) ,nrow=length(rName) ,dimnames=list(rName,cName)
                    )
        }
        if( is.null(bNScrExtMtxCfg[[mName]][[fName]]$scMtx.sz ) ){
            ...
            bNScrExtMtxCfg[[mName]][[fName]]$scMtx.sz <- matrix( thldVal ,byrow=T
                        ,ncol=length(scMtx.szName$cName) ,nrow=length(scMtx.szName$rName) 
                        ,dimnames=list(scMtx.szName$rName,scMtx.szName$cName)
                    )
        }
        if( is.null(bNScrExtMtxCfg[[mName]][[fName]]$summMtx.sum) ){
            # 개개 cut은 피했지만 전체 발생 총합은 한계를 넘는 경우.
            bNScrExtMtxCfg[[mName]][[fName]]$summMtx.sum <- c(raw=2 ,evt=2)
        }
        if( is.null(bNScrExtMtxCfg[[mName]][[fName]]$scMtx.sz.sum ) ){
            #   rebCnt.r = r.ph+r.fCol    rebCnt.e = e.ph+e.fCol
            bNScrExtMtxCfg[[mName]][[fName]]$scMtx.sz.sum <- c(rebCnt.r=2 ,rebCnt.e=2)
        }

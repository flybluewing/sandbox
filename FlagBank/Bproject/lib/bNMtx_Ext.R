...
if( TRUE ){
    ...
    bNMtxExtFltLst[[mName]]$filter01 <- fltCreater(mName)    #   bNMtxExtFltLst[[mName]]$flter01$getScoreMtx( scoreMtx.grp$basic$basic$score1$scoreMtx )
    ...
    fltCreater <- function( mName ){    # $flter01
        ...
        fltObj$getScore <- function( score ){
            ...
            scrEvt <- bFCust.getEvt( score ,bNScoreMtxCfg[[fltObj$mInfo$mName]]$fCol )["lev",]
    ...
    bNMtxExtFltLst[[mName]]$filter02 <- fltCreater(mName)
}
...
if( TRUE ){
    ...
    bNMtxExtFltLst[[mName]]$filter01 <- fltCreater(mName)
...
if( TRUE ){
    ...
    bNMtxExtFltLst[[mName]]$freqValReb <- fltCreater(mName)
...
if( TRUE ){
    ...
    fltCreater <- function( mName ){
        fltObj <- list( mInfo=list() )
        fltObj$mInfo$mName = mName
        fltObj$mInfo$cName <- c( "rCnt" ,"eD2" ,"eLr" ,"eRl" ,"cD2" ,"cLr" ,"cRl" ,"fCnt" )
        fltObj$getScore <- function( score ){
            #   score <- scoreMtx.grp$basic$basic[[mName]]$scoreMtx[1,]
            rVal <- rep(0,length(fltObj$mInfo$cName))  ;names(rVal) <- fltObj$mInfo$cName
            scrEvt <- bFCust.getEvt( score ,bNScoreMtxCfg[[fltObj$mInfo$mName]]$fCol )["lev",]

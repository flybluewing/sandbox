FCust_bScr01EvtLst <- list(		"rCnt"=2:4 ,"rD2"=2:4 ,"rDn"=1:4 ,"rLr"=2:4 ,"rRl"=2:4
								,"eCnt"=2:5 ,"eD2"=2:4 ,"eDn"=1:4 ,"eLr"=2:4 ,"eRl"=2:4
								,"cCnt"=2:4 ,"cD2"=2:4 ,"cDn"=1:4 ,"cLr"=2:4 ,"cRl"=2:4
								,"fCnt"=2:4 ,"fD2"=2:4 ,"fDn"=1:4 ,"fLr"=2:4 ,"fRl"=2:4
							)

FCust_bScr01minMaxLst <- list( "rCnt"=c(min=0,max=2) ,"rD2"=c(min=0,max=1) ,"rDn"=c(min=0,max=0) ,"rLr"=c(min=0,max=1) ,"rRl"=c(min=0,max=1)
...
#	c( typ="cust"	,hName="*"	,mName="bScr01"	,pName="*"	,fcName="*" )
bFCust.A_bScr01_A_A <- function(  ){

	rObj <- list( defId=c( typ="cust"	,hName="*"	,mName="bScr01"	,pName="*"	,fcName="*" ) )
...
bFCust.A_bScr01_A_Row01 <- function(  ){
	rObj <- list( )
	rObj$defId <- c( typ="c_NCol.5"	,hName="*"	,mName="bScr01"	,pName="*"	,rFId="Row01" )	# row filt ID
...
bFCust.A_bScr01_A_rRebAA <- function(  ){	#	이전 마지막 score(cutterObj$lastRow) 값과의 일치여부(raw Value) 
	...
	rObj <- list( )
	rObj$defId <- c( typ="cust_RReb"	,hName="*"	,mName="bScr01"	,pName="*"	,rFId="rRebAA" )	# row filt ID
	rObj$description <- sprintf("(cust)  ")
...
#	[bScr01:byFCol(Rebound/Sequencial)] ---------------------------------------------------------
...
bFCust.byFCol_A_bScr01_A <- function( ){

	rObj <- list( )
	rObj$defId <- c( typ="c_byFCol"	,hName="*"	,mName="bScr01"	,fcName="*"  )
	rObj$description <- sprintf("(cust)  ")
	...
	return( rObj )
} # bFCust.byFCol_A_bScr01_A( )

#	typ="c_byFCol"	,hName="*"	,mName="bScr01"	,pName="*"	,fcName="*" )
bFCust.byFCol_A_bScr01_A_rReb01 <- function( ){
	#	row rebind라고는 했지만 사실 seq 이다. 
	#	즉 이전 evt 발생 phase에서 다음에도 동일 evt가 일어나는지 체크(다음에서 추가적으로 발생하는 evt는 상관없음.)
	rObj <- list( )
	rObj$defId <- c( typ="c_byFCol"	,hName="*"	,mName="bScr01"	,pName="*"	,fcName="*" )	# row filt ID
	rObj$description <- sprintf("(cust)  ")
	...
	return( rObj )
} # bFCust.byFCol_A_bScr01_A_rReb01( )

#	c( typ="c_byFCol"	,hName="*"	,mName="bScr01"	,fcName="*"  )
bFCust.byFCol_A_bScr01_A_rRebAA <- function( ){
	rObj <- list( )
	rObj$defId <- c( typ="c_byFCol"	,hName="*"	,mName="bScr01"	,fcName="*"  )
	rObj$description <- sprintf("(cust)  ")
	...
	return( rObj )
} # bFCust.byFCol_A_bScr01_A_rRebAA( )
...
#		c( typ="cust_byHIdx"	,hName="*"	,mName="bScr01" )
bFCust.byHIdx_A_bScr01 <- function( ){

	rObj <- list( )
	rObj$defId <- c( typ="cust_byHIdx"	,hName="*"	,mName="bScr01" )
	rObj$description <- sprintf("(cust)  ")
	...
	rObj$createCutter <- function( mtxLst=NULL ,tgtId=c(hName="", mName="") ,auxInfo=c(auxInfo="") ){
		...
		if( tgtId["mName"]==cutterObj$defId["mName"] ){
			cutterObj$chkEvt.last <- bFCust.get_byHIdx_bScr01ChkEvt( mtxLst[[length(mtxLst)]] )
		}
		...
		cutterObj$cut <- function( scoreMtx ,aIdx ){
			...
			chkEvt <- bFCust.get_byHIdx_bScr01ChkEvt( scoreMtx )
...
bFCust.get_byHIdx_bScr01ChkEvt <- function( scoreMtx ){
...
} # bFCust.get_byHIdx_bScr01ChkEvt( )

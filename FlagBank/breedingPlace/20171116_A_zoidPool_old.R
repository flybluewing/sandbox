...
surviveFlag.G <- rep(T,nrow(allZoidMtx))
    ...
    save( surviveObj ,file="Obj_surviveObj.byBaseStep2.save" )


# ========================================================
#   cliper - last stand 적용.
myObj <- load("Obj_surviveObj.byBaseStep2.save")
indices.bStep2 <- which(surviveObj$surviveFlag)
flagLst <- list()
# k <- apply( allZoidMtx ,1 ,function(p){ all(lastZoid==p) }) # 4800520
allZoidMtx <- allZoidMtx[surviveObj$surviveFlag,]

    k.FLogStr("start last stand")
    #-----------------------------------------------------
    # cliper.backStep()
    ...
    clpObj <- cliper.quotient( zh )
    tStmp <- Sys.time()
    flag <- clpObj$byLate( allZoidMtx ,zh ,pDebugInfo=T )
    tDiff <- Sys.time() - tStmp
    k.FLogStr(sprintf("cliper.quotient() %.f%s",tDiff,units(tDiff)))
    flagLst[[1+length(flagLst)]] <- flag

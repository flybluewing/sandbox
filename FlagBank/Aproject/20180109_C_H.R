...
# 단축버전.
getPtnScanner <- function( ){

    rObj <- list()
    rObj$seqReb <- function( pVal ){
        val.len <- length(pVal)
        banVal <- integer(0)
        ...
        if( pVal[val.len-1]==pVal[val.len-2] ){
            banVal <- pVal[val.len]
        }
        return( banVal )
    } # rObj$seqReb()

    rObj$symm <- function( pVal ){
        val.len <- length(pVal)
        banVal <- integer(0)

        if( 3 <= val.len){ # 1,2,2,?
            if( pVal[val.len]==pVal[val.len-1] ){
                banVal <- c( banVal ,pVal[val.len-2] )
            }
        }
        if( 4 <= val.len){ # 1,2,3,2,?
            if( pVal[val.len]==pVal[val.len-2] ){
                banVal <- c( banVal ,pVal[val.len-1] ,pVal[val.len-3] )
            }
        }
        return( banVal )
    } # rObj$seqReb()

    rObj$stepReb <- function( pVal ,pOnlyFirst=TRUE ){
        val.len <- length(pVal)
        banVal <- integer(0)

        for( dIdx in 1:5 ){
            scanSpan <- val.len - ( (1:2)*dIdx - 1 )
            if( 1>scanSpan[2] ){
                break
            }
            if( pVal[scanSpan[1]]==pVal[scanSpan[2]] ){
                banVal <- c( banVal ,pVal[scanSpan[1]] )
                if( pOnlyFirst ){
                    break
                }
            }
    ...
    rObj$grad <- function( pVal ){
        ...
        codeDiff <- pVal[val.len] - pVal[val.len-1]
        if( 1==abs(codeDiff) ){
            banVal <- pVal[val.len] + codeDiff
        }
...
fnd2SeqReb <- function( pMtx ,pZoid ,pDepth=50 ,pSrcNum=2 ){
	...
	for( idx in 1:pDepth ){
		...
		for( aIdx in scanSpan ){
			fnd <- scanFnc( pMtx[hIdx,aIdx:(aIdx+1)] ,pZoid )
			if( !is.null(fnd) ){
				fndLst[[1+length(fndLst)]] <- list( idx=idx ,aIdx=aIdx ,ptn=pMtx[hIdx,aIdx:(aIdx+1)] ,fnd=fnd ,hIdx=hIdx )
...
fnd3SeqReb <- function( pMtx ,pZoid ,pDepth=50 ,pSrcNum=2 ){
	...
	for( idx in 1:pDepth ){
		...
		for( aIdx in scanSpan ){
			...
			if( !is.null(fnd) ){
				fndLst[[1+length(fndLst)]] <- list( idx=idx ,aIdx=aIdx ,ptn=pMtx[hIdx,aIdx+0:2] ,fnd=fnd ,hIdx=hIdx )
				if( pSrcNum == length(fndLst) ){
					return(fndLst) # reach to max level
				}
				break
...
cutEadge.colValCut <- function( gEnv ,allIdx ,colValLst ){
    ...
    allZoidMtx <- gEnv$allZoidMtx[allIdx,,drop=F]

    colIdx <- 1
    valMtx <- colValLst[[colIdx]]
    availVal <- valMtx["val", valMtx["freq",]>5 ]
    surviveMtx[,colIdx] <- apply( allZoidMtx ,1 ,function(p){p[colIdx]%in%availVal})
    ...
    for( colIdx in 1:6 ){
        ...
        for( aIdx in 1:length(allIdx) ){
            ...
            for( stdIdx in 1:nrow(anaObj$stdCodeMtx) ){
                if( all(anaObj$stdCodeMtx[stdIdx,]==gEnv$allZoidMtx[aIdx,colSpan] ) ){
                    colFlag[aIdx] <- FALSE
                    break
                }
            }
...
cutEadge.remLstHard <- function( gEnv ,allIdx ){
    ...
    tEnv <- list( allZoidMtx = gEnv$allZoidMtx[allIdx,,drop=F]
                    ,zhF = gEnv$zhF
                    ,logFile = logFile
                    ,doLog = TRUE
                )
...
cutEadge.getBanSym <- function( gEnv ,allIdx ,pDebug=F ){
    ...
    for( azColIdx in 1:6 ){
        for( vIdx in azColValLst[[azColIdx]] ){
            valMtx <- gEnv$zhF[gEnv$zhF[,azColIdx]==vIdx ,,drop=F]
...
cutEadge.getBanGrad <- function( gEnv ,allIdx ,pDebug=F ){
...
cutEadge.getBanRebBin <- function( gEnv ,allIdx ){
    ...
    allCodeMtx <- gEnv$allZoidMtx[allIdx,,drop=F] %% 2
...
cutEadge.banDupSeqBin <- function( gEnv ,allIdx ){
    ...
    flag.base <- if( is.null(banCode) ){ rep(TRUE,length(allIdx))
                        ...
                        apply( gEnv$allZoidMtx[allIdx,,drop=F]%%2 ,1 ,function(aCode){!all(aCode==banCode)} )
                    }
    ...
    for( azColIdx in 1:6 ){
        for( vIdx in azColValLst[[azColIdx]] ){
            ...
            for( idx in seq_len(length(allIdx)) ){
                ...
                if( all( (gEnv$allZoidMtx[aIdx,,drop=F]%%2)==banCode ) ){
                    flagLst.cv[[idx]][[ 1+length(flagLst.cv[[idx]]) ]] <- c( azColIdx ,vIdx )
                }
            }
        } # vIdx
...
cutEadge.getBanSymBin <- function( gEnv ,allIdx ){
    ...
    flag.base <- apply( gEnv$allZoidMtx[allIdx,,drop=F]%%2 ,1 ,function(aZoid){
    ...
    for( azColIdx in 1:6 ){
        for( vIdx in azColValLst[[azColIdx]]){
            tMtx <- gEnv$zhF[gEnv$zhF[,azColIdx]==vIdx ,,drop=F] %% 2
            ...
            for( idx in seq_len(length(allIdx)) ){
                ...
                aCode <- gEnv$allZoidMtx[allIdx[idx],,drop=F] %% 2
...
cutEadge.banDupSeqDiff <- function( gEnv ,allIdx ){

    scanPtn <- function( pVal ){
        valLen <- length(pVal)
        if( 3>valLen ){
            return( integer(0) )
        }

        if( pVal[valLen-1]==pVal[valLen-2] ){
            return( pVal[valLen] )
        }
        return( integer(0) )
    ...
    # flagLst.base
    flagLst.base <- vector( "list" ,length(allIdx) )
    lastZoid <- gEnv$zhF[nrow(gEnv$zhF),]
    stdCodeMtx <- gEnv$zhF[1:(nrow(gEnv$zhF)-1),] - gEnv$zhF[2:nrow(gEnv$zhF),]
    for( azColIdx in 1:6 ){
        banPtn <- scanPtn( stdCodeMtx[,azColIdx] )
        for( idx in seq_len(length(allIdx)) ){
            zoidDiff <- lastZoid - gEnv$allZoidMtx[allIdx[idx],]
            if( zoidDiff[azColIdx]%in%banPtn ){
    ...
    for( azColIdx in 1:6 ){
        for( vIdx in azColValLst[[azColIdx]] ){
            ...
            lastRaw <- tRawMtx[rawLen,]
            tStdMtx <- tRawMtx[1:(rawLen-1),,drop=F] - tRawMtx[2:rawLen,,drop=F]
            for( scanColIdx in (1:6)[-azColIdx] ){
                banPtn <- scanPtn( tStdMtx[,scanColIdx] )
                for( idx in seq_len(length(allIdx)) ){
                    if( vIdx!=gEnv$allZoidMtx[allIdx[idx],azColIdx] ){
                        next
                    }
                    zoidDiff <- lastRaw - gEnv$allZoidMtx[allIdx[idx],]
                    if( zoidDiff[scanColIdx]%in%banPtn ){
                        flagLst.cv[[idx]][[1+length(flagLst.cv[[idx]])]] <- c(azColIdx,vIdx,scanColIdx)
                    }
                }
            } # scanColIdx
        } # vIdx
    ...
    rObj <- list( idStr="cutEadge.banDupSeqDiff" )
    rObj$flag <- sapply( seq_len(length(allIdx)) ,function(idx){
                        (length(flagLst.base[[idx]])==0) && (length(flagLst.cv[[idx]])==0)
                    })
    return( rObj )

} # cutEadge.banDupSeqDiff()

cutEadge.getBanSymDiff <- function( gEnv ,allIdx ){
    ...
    scanPtn <- function( pVal ){
        ...
        if( 3<=val.len ){ # A,B,B,?
            if( pVal[val.len]==pVal[val.len-1] ){
                banPtn <- c( banPtn, pVal[val.len-2] ) 
            }
        }

        if( 4<=val.len ){ # A,B,C,B,?
            if( pVal[val.len]==pVal[val.len-2] ){
                banPtn <- c( banPtn, pVal[val.len-1] ,pVal[val.len-3] ) 
            }
        }
    ...
    stdZoidMtx <- gEnv$zhF[1:(nrow(gEnv$zhF)-1),,drop=F] - gEnv$zhF[2:nrow(gEnv$zhF),,drop=F]
    lastZoid <- gEnv$zhF[nrow(gEnv$zhF),]
    for( azColIdx in 1:6 ){
        banPtn <- scanPtn( stdZoidMtx[,azColIdx] )
        for( idx in seq_len(allIdx.len) ){
            if( (lastZoid-gEnv$allZoidMtx[allIdx[idx],])[azColIdx] %in% banPtn ){
                flagLst.base[[idx]][[1+length(flagLst.base[[idx]])]] <- c(azColIdx)
            }
        }
    }
    ...
    for( azColIdx in 1:6 ){
        for( vIdx in azColValLst[[azColIdx]] ){
            ...
            lastRaw <- tRawMtx[nrow(tRawMtx),]
            tStdMtx <- tRawMtx[1:(nrow(tRawMtx)-1),] - tRawMtx[2:nrow(tRawMtx),]
            for( scanColIdx in (1:6)[-azColIdx] ){
                banPtn <- scanPtn( tStdMtx[,scanColIdx] )
                for( idx in seq_len(allIdx.len) ){
                    if( vIdx!=gEnv$allZoidMtx[allIdx[idx],azColIdx] ){
                    ...
                    aCode <- lastRaw - gEnv$allZoidMtx[allIdx[idx],]
                    if( aCode[scanColIdx]%in%banPtn ){
                       flagLst.cv[[idx]][[1+length(flagLst.cv[[idx]])]] <- c(azColIdx,vIdx,scanColIdx)
                    }
                } # idx
            } # scanColIdx
    ...
    rObj <- list( idStr="cutEadge.getBanSymDiff" )
    rObj$flag <- sapply( seq_len(allIdx.len) ,function(idx){
                        (length(flagLst.base[[idx]])==0) && (length(flagLst.cv[[idx]])==0)
                    })
    return( rObj )

} # cutEadge.getBanSymDiff()

#   걸리는 게 자주 있는 듯. 2개 이상 중복만 피해야 할 듯 하다.
cutEadge.banSeqRebCStep <- function( gEnv ,allIdx ){
    ...
    rObj <- list( idStr="cutEadge.banSeqRebCStep" )
    rObj$flag <- sapply( seq_len(allIdx.len) ,function(idx){
                        (length(flagLst.base[[idx]])==0) && (length(flagLst.cv[[idx]])==0)
                    })
    return( rObj )

} # cutEadge.banSeqRebCStep()


cutEadge.getBanSymCStep <- function( gEnv ,allIdx ,pDebug=F ){
    ...
    rObj <- list( idStr="cutEadge.getBanSymCStep" )
    rObj$flag <- flag.base & flag.cv
	if( pDebug ){
...
cutEadge.getBanGradCStep <- function( gEnv ,allIdx ,pDebug=F ){
    ...
    rObj <- list( idStr="cutEadge.getBanGradCStep" )
    rObj$flag <- flag.base & flag.cv
	if( pDebug ){
		rObj$flagLst.base	<- flagLst.base
		rObj$flagLst.cv		<- flagLst.cv
...
} # cutEadge.getBanGradCStep()


cutEadge.getBanGradWidth <- function( gEnv ,allIdx ){
    ...
    rObj <- list( idStr="cutEadge.getBanGradWidth" )
    rObj$flag <- sapply( seq_len(allIdx.len) ,function(idx){
                        (length(flagLst.base[[idx]])==0) && (length(flagLst.cv[[idx]])==0)
                    })
...
cutEadge.banSeq3Twice <- function( gEnv ,allIdx ){
    ...
    flagLst.base <- vector("list",allIdx.len)
    for( idx in seq_len(allIdx.len) ){
		fndLst <- fnd3SeqReb( gEnv$zhF ,gEnv$allZoidMtx[allIdx[idx],] ,pDepth=50 ,pSrcNum=2  )
        if( 2==length(fndLst) ){
            flagLst.base[[idx]] <- fndLst
        }
    }
    ...
    rObj <- list( idStr="cutEadge.banSeq3Twice" )
    rObj$flag <- sapply( seq_len(allIdx.len) ,function(idx){
                        (length(flagLst.base[[idx]])<2) && (length(flagLst.cv[[idx]])<2)
                    })

...
fCutCnt.basic <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(2,0,2)) ) return(FALSE)	# next rebind of 0,2,2
					if( all(quoSize[1:3+2]==c(1,3,0)) ) return(FALSE)	# next rebind of 2,2,0
					if( all(quoSize[1:3+1]==c(2,0,2)) ) return(FALSE)	# next rebind of 1,1,3
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			# <36>
			if( fCutU.hasPtn(c(         26,NA,36),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(17,14,28,NA,NA,36),aZoid,thld=3,fixIdx=6) ) cnt<-cnt+1
			# <38>
			if( fCutU.hasPtn(c( 9,NA,NA,NA,NA,38),aZoid) ) cnt<-cnt+1 
			if( fCutU.hasPtn(c(            32,38),aZoid) ) cnt<-cnt+1 
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			...
			if( fCutU.remFilt(aZoid[2],c( 7,2,0 ),c( 17    )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 5,4   ),c( 24    )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 3     ),c(       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 3,4,0 ),c( 33,34 )) ) cnt<-cnt+1 # 5
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w2 <- 0
			if( all(aCStep[c(1,2)]==aCStep[c(5,6)]) )		cnt.w2<-cnt.w2+1

			cntMtx[idx,"cStep.w1"] <- cnt.w1	;cntMtx[idx,"cStep.w2"] <- cnt.w2
			cntMtx[idx,"cStep"] <- cnt + cnt.w1 + cnt.w2

			#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
			#      1 12 13 24 29 44    |11  1 11  5 15 |                        |1 2 2 0 1 |1 2 2 1
			#      9 18 20 24 27 36(1) | 9  2  4  3  9 |  8   6   7   0  -2  -8 |1 1 3 1 0 |1 1 3 1
			#     12 18 24 26 39 40(2) | 6  6  2 13  1 |  3   0   4   2  12   4 |0 2 2 1 1 |2 2 1 1
			#      7  9 24 29 34 38(1) | 2 15  5  5  4 | -5  -9   0   3  -5  -2 |2 0 2 2 0 |2 2 2
			#      8 15 21 31 33 38(1) | 7  6 10  2  5 |  1   6  -3   2  -1   0 |1 1 1 3 0 |1 1 1 3
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[4]%in%c( 2, 0 ) ) cnt<-cnt+1
			if( aFStep[5]%in%c( 1    ) ) cnt<-cnt+1
			if( aFStep[6]%in%c( 4, 0 ) ) cnt<-cnt+1
...
fCutCnt.nextZW <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( all(quoSize[1:3+1]==c(1,2,2)) ) return(FALSE)	# next rebind of 2,1,0 reverse
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag			
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			...
			if( aZoid[1]%in%c(  9    ) ) cnt<-cnt+1
			if( aZoid[2]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[3]%in%c( 11,16 ) ) cnt<-cnt+1
			if( aZoid[4]%in%c(       ) ) cnt<-cnt+1
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			cnt <- 0
			# < 2>
			if( fCutU.hasPtn(c(  2,NA,NA,NA,36 ),aZoid) ) cnt<-cnt+1
			# <10>
			# <11>
			if( fCutU.hasPtn(c( 11,24,NA,37 ),aZoid) ) cnt<-cnt+1
			# <14>
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			...
			if( fCutU.remFilt(aZoid[3],c( 1, 6 ),c( 11,16 )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 3    ),c(       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c(      ),c(       )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 6, 5 ),c(       )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c(         ) ) cnt<-cnt+1
			if( aCStep[2]%in%c( 2, 6, 9,20 ) ) cnt<-cnt+1	# 5,1,20
			...
			if( all(aCStep[1:2+2]==c( 7,20 )) )	cnt.w1<-cnt.w1+1

			cnt.w2 <- 0
			if( all(aCStep[c(3,4)]== aCStep[1]*c(1,2)) )	cnt.w2<-cnt.w2+1
			if( all(aCStep[c(1,3)]== aCStep[5]*c(2,2)) )	cnt.w2<-cnt.w2+1

			cntMtx[idx,"cStep.w1"] <- cnt.w1	;cntMtx[idx,"cStep.w2"] <- cnt.w2
			cntMtx[idx,"cStep"] <- cnt + cnt.w1 + cnt.w2

			#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
			#      1  2  4 23 31 34    | 1  2 19  8  3 |                        |3 0 1 2 0 |3 1 2
			#     11 13 25 26 29 33    | 2 12  1  3  4 | 10  11  21   3  -2  -1 |0 2 3 1 0 |2 3 1
...
fCutCnt.nextQuo10 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( all(quoSize[1:3+1]==c(0,2,0)) ) return(FALSE)	# next rebind of 1,1,1
					if( all(quoSize[1:3+2]==c(1,2,1)) ) return(FALSE)	# next rebind of 1,3,0
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			...
			if( aZoid[2]%in%c(10,11,15 ) ) cnt<-cnt+1
			if( aZoid[3]%in%c(14       ) ) cnt<-cnt+1
			if( aZoid[4]%in%c(         ) ) cnt<-cnt+1
			if( aZoid[5]%in%c(32       ) ) cnt<-cnt+1
			if( aZoid[6]%in%c(38,39    ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			...
			if( fCutU.remFilt(aZoid[2],c( 0,5 ),c( 10,11,15 )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 4,2 ),c( 14       )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 2   ),c(          )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 2   ),c( 32       )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 8,9 ),c( 38,39    )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt

		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c( 7, 8, 6, 9   ) ) cnt<-cnt+1
			if( aCStep[2]%in%c( 3     ) ) cnt<-cnt+1
			if( aCStep[3]%in%c( 7,15, 5    ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(      ) ) cnt<-cnt+1
			if( aCStep[5]%in%c( 4, 1, 6, 5  ) ) cnt<-cnt+1
			...
			if( all(aCStep[1:2+3]==c( 2, 5 )) )	cnt.w1<-cnt.w1+1	# 9,1 16,4

			cnt.w2 <- 0
			if( aCStep[1]==sum(aCStep[c(4,5)]) )			cnt.w2<-cnt.w2+1
			if( sum(aCStep[c(1,5)])==sum(aCStep[c(3,4)]) )	cnt.w2<-cnt.w2+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[5]%in%c( -1, -4  ) ) cnt<-cnt+1
			if( aFStep[6]%in%c(  1, -5  ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			if( fCutU.hasPtn(c( -1, 1 ),aFStep) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c(  1, 1 ),aFStep) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c(  1,-1 ),aFStep) )	cnt.w1<-cnt.w1+1
			if( 1<sum(aFStep[1:3+3]==c(  7, -1, -1 )) )	cnt.w1<-cnt.w1+1

			cnt.w2 <- 0
			if( all(aFStep[1:2]== aFStep[3]*c(1,1) ) )		cnt.w2<-cnt.w2+1
...
fCutCnt.nextBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+1]==c(0,2,3)) ) return(FALSE)	# next rebind of 1,0,2
					if( all(quoSize[1:3+2]==c(2,1,2)) ) return(FALSE)	# next rebind of 0,2,2
					...
					if( all(quoSize[1:3+2]==c(1,2,0)) ) return(FALSE)	# next rebind of 2,0,1 reverse
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			...
			if( aZoid[5]%in%c(          ) ) cnt<-cnt+1
			if( aZoid[6]%in%c( 45,41,42 ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			# <18>
			if( fCutU.hasPtn(c(  7,18,31,22,45,38 ),aZoid,thld=3,fixIdx=2) ) cnt<-cnt+1
			# <22>
			if( fCutU.hasPtn(c( 22,28 ),aZoid) ) cnt<-cnt+1
			# <26>
			if( fCutU.hasPtn(c(       26,27 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(  3,11,26    ),aZoid) ) cnt<-cnt+1
			# <27>
			if( fCutU.hasPtn(c(       26,27 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(  3,11,NA,27 ),aZoid) ) cnt<-cnt+1
			# <28>
			if( fCutU.hasPtn(c( 22,28 ),aZoid) ) cnt<-cnt+1
			# <32>
			if( fCutU.hasPtn(c(          32,36 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c( 24,14,14,32    ),aZoid,thld=3,fixIdx=4) ) cnt<-cnt+1
			# <34>
			# <40>
			if( fCutU.hasPtn(c(  8,20,20,40 ),aZoid,thld=3,fixIdx=4) ) cnt<-cnt+1
			# <41>
			if( fCutU.hasPtn(c(  6,NA,36,41 ),aZoid) ) cnt<-cnt+1
			# <45>
			if( fCutU.hasPtn(c( 11,26,NA,NA,NA,45 ),aZoid) ) cnt<-cnt+1

			cntMtx[idx,"rawFV"] <- cnt
		}
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 6,5,6     ),c( 15       )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 6,8       ),c(          )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 7,6,8,1   ),c(          )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 2,8       ),c( 22       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 9,5       ),c(          )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 5,1,6,2,0 ),c( 45,41,42 )) ) cnt<-cnt+1 # 6
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[4]%in%c(      ) ) cnt<-cnt+1
			if( aCStep[5]%in%c(  8   ) ) cnt<-cnt+1	# 5&6,3,8?

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]
			if( fCutU.hasPtn(c( 5, 6 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( all(aCStep[1:2+3]==c( 7, 8 )) )	cnt.w1<-cnt.w1+1
			#  x,11,x  : 11 좌우에 같은 값 발생
			...
			#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
			#      6 22 28 32 34 40    |16  6  4  2  6 |                        |1 0 2 2 1 |1 2 2 1
			#     14 19 25 26 27 34(1) | 5  6  1  1  7 |  8  -3  -3  -6  -7  -6 |0 2 3 1 0 |2 3 1
			#     15 18 21 32 35 44    | 3  3 11  3  9 |  1  -1  -4   6   8  10 |0 2 1 2 1 |2 1 2 1
			#     11 18 26 27 40 41(1) | 7  8  1 13  1 | -4   0   5  -5   5  -3 |0 2 2 0 2 |2 2 2
			#      1  6 17 22 28 45    | 5 11  5  6 17 |-10 -12  -9  -5 -12   4 |2 1 2 0 1 |2 1 2 1
			#      6 16 37 38 41 45(2) |10 21  1  3  4 |  5  10  20  16  13   0 |1 1 0 2 2 |1 1 2 2
			# -<standard zoid>---------------------------------------------------------------------
			#      5 11 12 29 33 44    | 6  1 17  4 11 | -1  -5 -25  -9  -8  -1 |1 2 1 1 1 |1 2 1 1 1
		}
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
...
fCutCnt.nextRebNum <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+2]==c(0,3,0)) ) return(FALSE)	# next rebind of 0,1,1 reverse
					if( all(quoSize[1:3+1]==c(1,0,2)) ) return(FALSE)	# next rebind of 1,0,3
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			# < 6>
			if( fCutU.hasPtn(c(  6, 8,36,41,39 ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
			# <17>
			# <24>
			if( fCutU.hasPtn(c( 12,24,31,43,42,45 ),aZoid,thld=3,fixIdx=2) ) cnt<-cnt+1
			# <27>
			if( fCutU.hasPtn(c(       26,27       ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(          27,33    ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(  2,NA,NA,27,NA,33 ),aZoid) ) cnt<-cnt+1
			# <34>
			...
			# <38>
			if( fCutU.hasPtn(c(  6, 4,NA,38 ),aZoid) ) cnt<-cnt+1

			cntMtx[idx,"rawFV"] <- cnt
		}
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 5,2,0     ),c(  2 )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 4     ),c( 29 )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c(      ),c(    )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 0,9,6,8     ),c( 39 )) ) cnt<-cnt+1 # 4
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[3]%in%c(       ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(  5    ) ) cnt<-cnt+1
			if( aCStep[5]%in%c(  2,22 ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]
			if( fCutU.hasPtn(c( 4,3 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( all(aCStep[1:2+3]==c( 20, 4 )) )	cnt.w1<-cnt.w1+1

			cnt.w2 <- 0
			if( all(aCStep[1:2]==aCStep[3]*c(1,1)) )	cnt.w2<-cnt.w2+1

			cntMtx[idx,"cStep.w1"] <- cnt.w1	;cntMtx[idx,"cStep.w2"] <- cnt.w2
			cntMtx[idx,"cStep"] <- cnt + cnt.w1 + cnt.w2

			#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
			#      6 12 24 27 35 37    | 6 12  3  8  2 |                        |1 1 2 2 0 |1 1 2 2
			#      4 24 25 27 34 35(3) |20  1  2  7  1 | -2  12   1   0  -1  -2 |1 0 3 2 0 |1 3 2
...
fCutCnt.nextCStepBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(2,2,1)) ) return(FALSE)	# next rebind of 0,1,2 reverse
					...
					if( all(quoSize[1:3+0]==c(2,2,0)) ) return(FALSE)	# next rebind of 2,2,1 reverse
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			cnt <- 0
			if( aZoid[1]%in%c( 10,41   ) ) cnt<-cnt+1
			if( aZoid[2]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[3]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[4]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[5]%in%c(       ) ) cnt<-cnt+1
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			if( fCutU.hasPtn(c(  6,15 ),aZoid) ) cnt<-cnt+1
			# <10>
			if( fCutU.hasPtn(c( 10,NA,16,15 ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
			# <12>
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 0,2   ),c( 10,41 )) ) cnt<-cnt+1 # 1
			...
			if( fCutU.remFilt(aZoid[6],c( 5   ),c( 45,35 )) ) cnt<-cnt+1 # 6
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( all(aCStep[1:2+1]==c( 21, 1 )) )	cnt.w1<-cnt.w1+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[4]%in%c(     ) ) cnt<-cnt+1
			if( aFStep[5]%in%c(     ) ) cnt<-cnt+1
			if( aFStep[6]%in%c( 23  ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			cnt.w2 <- 0
			if( aFStep[4]==sum(aFStep[c(1,3,6)]) )			cnt.w2<-cnt.w2+1
			if( sum(aFStep[c(2,5)])==sum(aFStep[c(1,4,6)]) )	cnt.w2<-cnt.w2+1	# 24
...
fCutCnt.nextFStepBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(1,0,2)) ) return(FALSE)	# next rebind of 1,1,3
					if( all(quoSize[1:3+1]==c(0,2,2)) ) return(FALSE)	# next rebind of 1,3,1
					if( all(quoSize[1:3+2]==c(3,1,0)) ) return(FALSE)	# next rebind of 3,1,0
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			...
			if( aZoid[3]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[4]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[5]%in%c( 23,24 ) ) cnt<-cnt+1
			if( aZoid[6]%in%c( 26,27 ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			...
			if( fCutU.remFilt(aZoid[2],c( 6,8,9 ),c( 26    )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c(       ),c(       )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c(       ),c(       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 3,4   ),c( 23,24 )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 6,7   ),c( 26,27 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c( 7,3,1   ) ) cnt<-cnt+1
			if( aCStep[2]%in%c( 4       ) ) cnt<-cnt+1
			if( aCStep[3]%in%c( 5,2,9,3 ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(         ) ) cnt<-cnt+1
			if( aCStep[5]%in%c( 9,3,6   ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]
			if( fCutU.hasPtn(c( 1, 1 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( all(aCStep[1:2+3]==c( 4,13 )) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c( 3, 9 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c( 3, 6 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( all(aCStep[1:2+3]==c(10, 3)) )	cnt.w1<-cnt.w1+1
			...
			#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
			#     15 24 31 32 33 40    | 9  7  1  1  7 |                        |0 1 1 3 1 |1 1 3 1
			...
			#      9 18 20 24 27 36    | 9  2  4  3  9 | -5   3  -5  -4  -2   6 |1 1 3 1 0 |1 1 3 1
			# -<standard zoid>---------------------------------------------------------------------
			#      5 11 12 29 33 44    | 6  1 17  4 11 | -4  -7  -8   5   6   8 |1 2 1 1 1 |1 2 1 1 1
		}
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c(     ) ) cnt<-cnt+1
			...
			if( aFStep[1]==aFStep[2] )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_1 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+1]==c(1,2,0)) ) return(FALSE)	# next rebind of 0,3,1 reverse
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			...
			if( aZoid[1]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[2]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[3]%in%c( 23,18 ) ) cnt<-cnt+1
			if( aZoid[4]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[5]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[6]%in%c(       ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			if( fCutU.hasPtn(c(       22,41,NA,39 ),aZoid) ) cnt<-cnt+1
			# <25>
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 4,0     ),c(       )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c(         ),c(       )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 3,8,5   ),c( 23,18 )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 8,3,2,5 ),c(       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 3,8,1   ),c(       )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 6,7     ),c(       )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			...
			if( aCStep[4]%in%c( 3       ) ) cnt<-cnt+1
			if( aCStep[5]%in%c( 1,14, 8 ) ) cnt<-cnt+1
			...
			if( aCStep[4]==sum(aCStep[c(1,2,3,5)]) )	cnt.w2<-cnt.w2+1

			cntMtx[idx,"cStep.w1"] <- cnt.w1	;cntMtx[idx,"cStep.w2"] <- cnt.w2
			cntMtx[idx,"cStep"] <- cnt + cnt.w1 + cnt.w2

			#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
			#     11 17 21 24 26 36    | 6  4  3  2 10 |                        |0 2 3 1 0 |2 3 1
			#      2 19 25 26 27 43(1) |17  6  1  1 16 | -9   2   4   2   1   7 |1 1 3 0 1 |1 1 3 1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( all(aFStep[1:2+4]==c( -12,-20 )) )	cnt.w1<-cnt.w1+1
			cnt.w2 <- 0
			if( all(aFStep[c(1,6)]==aFStep[c(2,5)]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_2 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			...
			if( aZoid[3]%in%c(             ) ) cnt<-cnt+1
			if( aZoid[4]%in%c(             ) ) cnt<-cnt+1
			if( aZoid[5]%in%c( 39,36,38,31 ) ) cnt<-cnt+1
			if( aZoid[6]%in%c( 40,45       ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			cnt <- 0
			# < 2>
			if( fCutU.hasPtn(c(  2,28,22,29,32 ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
			# < 6>
			if( fCutU.hasPtn(c(  6,20,43,26 ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
			# <13>
			...
			if( fCutU.hasPtn(c( 24,25,21,37    ),aZoid,thld=3,fixIdx=4) ) cnt<-cnt+1
			# <38>
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 1,7     ),c( 17          )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 9,5     ),c( 19          )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c(         ),c(             )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 4,7,8   ),c(             )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 9,6,8.1 ),c( 39,36,38,31 )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 0,5     ),c( 40,45       )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			...
			#      7 18 22 24 31 34    |11  4  2  7  3 |  6  16  16   8  12  -8 |1 1 2 2 0 |1 1 2 2
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[5]%in%c(  2,  6 ) ) cnt<-cnt+1
			if( aFStep[6]%in%c(     ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			cnt.w2 <- 0
			if( all(aFStep[c(1,2)]==aFStep[c(6,5)]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_3 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( all(quoSize[1:3+1]==c(2,1,1)) ) return(FALSE)	# next rebind of 1,1,3
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			cnt <- 0
			if( aZoid[1]%in%c(  9,24 ) ) cnt<-cnt+1
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			# <33>
			if( fCutU.hasPtn(c(       31,33 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c( 10,12,NA,33 ),aZoid) ) cnt<-cnt+1
			# <38>
			if( fCutU.hasPtn(c(  9,NA,NA,NA,NA,38 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(             32,38 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(    21,18,33,NA,38 ),aZoid,thld=3,fixIdx=5) ) cnt<-cnt+1
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 9,4,6 ),c(  9,24 )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c(       ),c( 21    )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 3     ),c( 33    )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 9,2   ),c( 12    )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 4,5,2 ),c( 32    )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 8,3   ),c( 38,43 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c( 3, 7, 2 ) ) cnt<-cnt+1
			if( aCStep[2]%in%c(         ) ) cnt<-cnt+1
			if( aCStep[3]%in%c( 6       ) ) cnt<-cnt+1
			if( aCStep[4]%in%c( 5,13    ) ) cnt<-cnt+1
			if( aCStep[5]%in%c( 6, 4, 2 ) ) cnt<-cnt+1
			...
			if( all(aCStep[1:2+3]==c( 6,10 )) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c( 10, 3 ),aCStep) )	cnt.w1<-cnt.w1+1

			cnt.w2 <- 0
			if( all(aCStep[2:3]== aCStep[4]*c(3,5) ) )		cnt.w2<-cnt.w2+1
			if( aCStep[1]==sum(aCStep[c(4,5)]) )			cnt.w2<-cnt.w2+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c(       ) ) cnt<-cnt+1
			if( aFStep[2]%in%c(  6, 1 ) ) cnt<-cnt+1
			if( aFStep[3]%in%c(  6    ) ) cnt<-cnt+1
			if( aFStep[4]%in%c(       ) ) cnt<-cnt+1
...
fCutCnt.nextColVal_4 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+1]==c(3,1,1)) ) return(FALSE)	# next rebind of 1,1,2
					if( all(quoSize[1:3+2]==c(1,1,1)) ) return(FALSE)	# next rebind of 1,2,2
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			cnt <- 0
			if( aZoid[1]%in%c( 21,20 ) ) cnt<-cnt+1
			if( aZoid[2]%in%c( 21    ) ) cnt<-cnt+1
			if( aZoid[3]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[4]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[5]%in%c(       ) ) cnt<-cnt+1
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			cnt <- 0
			# < 4>
			if( fCutU.hasPtn(c(  4,NA,NA,NA,32 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(  4,NA,14,31    ),aZoid) ) cnt<-cnt+1
			# <15>
			if( fCutU.hasPtn(c( 15,21          ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c( 15,NA,27,26,39 ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
			# <20>
			if( fCutU.hasPtn(c( 20,25 ),aZoid) ) cnt<-cnt+1
			# <21>
			if( fCutU.hasPtn(c( 15,21          ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(    21,27,26,39 ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
			# <32>
			if( fCutU.hasPtn(c(  4,NA,NA,NA,32 ),aZoid) ) cnt<-cnt+1
			# <38>
			if( fCutU.hasPtn(c(          36,38 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(  2, 4, 9,NA,38 ),aZoid,thld=3,fixIdx=5) ) cnt<-cnt+1
			# <43>
			if( fCutU.hasPtn(c( 20,27,NA,29,NA,43 ),aZoid,thld=3,fixIdx=6) ) cnt<-cnt+1
			cntMtx[idx,"rawFV"] <- cnt
		}
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c(      ),c( 21,20 )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c(      ),c( 21    )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c(      ),c(       )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c(      ),c(       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c(      ),c(       )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c(      ),c( 43,30 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			...
			if( all(aCStep[1:2+1]==c( 4, 6 )) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c( 10, 7 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c(  9, 1 ),aCStep) )	cnt.w1<-cnt.w1+1

			cnt.w2 <- 0
			if( all(aCStep[1:2]== aCStep[5]*c(3,5) ) )	cnt.w2<-cnt.w2+1
			...
			#      4 11 20 23 32 39(1) | 7  9  3  9  7 |  0  -7  -6 -10  -2   1 |1 1 2 2 0 |1 1 2 2
			#      2  4  5 17 27 32(2) | 2  1 12 10  5 | -2  -7 -15  -6  -5  -7 |3 1 1 1 0 |3 1 1 1
			#     12 15 16 20 24 30    | 3  1  4  4  6 | 10  11  11   3  -3  -2 |0 3 2 1 0 |3 2 1
			#     10 15 21 35 38 43(1) | 5  6 14  3  5 | -2   0   5  15  14  13 |0 2 1 2 1 |2 1 2 1
			#     15 21 31 32 41 43(3) | 6 10  1  9  2 |  5   6  10  -3   3   0 |0 1 1 2 2 |1 1 2 2
			# -<standard zoid>---------------------------------------------------------------------
			#      5 11 12 29 33 44    | 6  1 17  4 11 |-10 -10 -19  -3  -8   1 |1 2 1 1 1 |1 2 1 1 1
		}
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c( -2, 7 ) ) cnt<-cnt+1
			if( aFStep[2]%in%c(     ) ) cnt<-cnt+1
			if( aFStep[3]%in%c(     ) ) cnt<-cnt+1
			if( aFStep[4]%in%c(     ) ) cnt<-cnt+1
			if( aFStep[5]%in%c(     ) ) cnt<-cnt+1
...
fCutCnt.nextColVal_5 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(2,1,0)) ) return(FALSE)	# next rebind of 2,1,2
					if( all(quoSize[1:3+2]==c(0,2,2)) ) return(FALSE)	# next rebind of 2,2,0
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			cnt <- 0
			if( aZoid[1]%in%c( 2, 4, 6,19 ) ) cnt<-cnt+1
			if( aZoid[2]%in%c( 7,23       ) ) cnt<-cnt+1
			if( aZoid[3]%in%c(            ) ) cnt<-cnt+1
			if( aZoid[4]%in%c(25,30       ) ) cnt<-cnt+1
			if( aZoid[5]%in%c(            ) ) cnt<-cnt+1
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			# < 5>
			if( fCutU.hasPtn(c(  5,NA, 6,41 ),aZoid) ) cnt<-cnt+1
			# <16>			# <19>
			# <25>
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 2,4,6,9 ),c(  2, 4, 6,19 )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 5,7,3   ),c(  7,23       )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c(         ),c(             )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 5       ),c( 25,30       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 9,1,3,5 ),c(             )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 6,3,5   ),c( 36,43       )) ) cnt<-cnt+1 # 6

			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c( 7, 4 ) ) cnt<-cnt+1
			...
			if( all(aCStep[1:2+0]==c( 11, 5 )) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c(  5,13 ),aCStep) )	cnt.w1<-cnt.w1+1	# -
			# -------------------------------------------------------------------------------------
			#     FV :    1 (3)   3 (6)   4 (4)   5 (4)   6 (2)   8 (4) 
			cnt.w2 <- 0
			if( all(aCStep[1:2]==aCStep[5:4]) )	cnt.w2<-cnt.w2+1

			cntMtx[idx,"cStep.w1"] <- cnt.w1	;cntMtx[idx,"cStep.w2"] <- cnt.w2
			cntMtx[idx,"cStep"] <- cnt + cnt.w1 + cnt.w2

			#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
			#      4  5  8 16 21 29    | 1  3  8  5  8 |                        |3 1 2 0 0 |3 1 2
...
fCutCnt.nextColVal_6 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+2]==c(3,0,2)) ) return(FALSE)	# next rebind of 2,1,1
					if( all(quoSize[1:3+0]==c(2,2,0)) ) return(FALSE)	# next rebind of 0,3,1 reverse
					if( all(quoSize[1:3+2]==c(1,3,0)) ) return(FALSE)	# next rebind of 1,2,0 reverse
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			cnt <- 0
			if( aZoid[1]%in%c( 12, 3, 1,17,42 ) ) cnt<-cnt+1
			if( aZoid[2]%in%c( 18,12,19       ) ) cnt<-cnt+1
			if( aZoid[3]%in%c(                ) ) cnt<-cnt+1
			if( aZoid[4]%in%c( 39             ) ) cnt<-cnt+1
			if( aZoid[5]%in%c( 39,31,33       ) ) cnt<-cnt+1
			if( aZoid[6]%in%c( 41             ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			if( fCutU.hasPtn(c( 12,NA,29,23       ),aZoid) ) cnt<-cnt+1
			# <18>
			if( fCutU.hasPtn(c( 12,18             ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(    18,NA,NA,NA,41 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(    18,29,23       ),aZoid) ) cnt<-cnt+1
			# <31>
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 2,3,7   ),c( 12, 3, 1,17,42 )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 8,2,9,1 ),c( 18,12,19       )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c(         ),c(                )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 9       ),c( 39             )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 0,9,1   ),c( 39,31,33       )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 1,9,3   ),c( 41             )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c( 6, 9, 7 ) ) cnt<-cnt+1
			if( aCStep[2]%in%c( 6, 2, 1 ) ) cnt<-cnt+1
			if( aCStep[3]%in%c( 9, 6    ) ) cnt<-cnt+1
			if( aCStep[4]%in%c( 8, 3    ) ) cnt<-cnt+1
			if( aCStep[5]%in%c( 1, 3    ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]
			if( fCutU.hasPtn(c( 13, 1 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( all(aCStep[1:2+3]==c( 2, 8 )) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c(  5, 6  ),aCStep) )	cnt.w1<-cnt.w1+1
			...
			#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
			#      1  3  8 12 42 43    | 2  5  4 30  1 |                        |3 1 0 0 2 |3 1 2
			...
			# -<standard zoid>---------------------------------------------------------------------
			#      5 11 12 29 33 44(1) | 6  1 17  4 11 | -7  -7 -12   3  -6   4 |1 2 1 1 1 |1 2 1 1 1
		}
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c(        ) ) cnt<-cnt+1
			...
			if( aFStep[4]%in%c(  9,  4 ) ) cnt<-cnt+1
			if( aFStep[5]%in%c(        ) ) cnt<-cnt+1
			if( aFStep[6]%in%c(        ) ) cnt<-cnt+1

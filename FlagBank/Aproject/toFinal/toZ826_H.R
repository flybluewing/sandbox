...
fCutCnt.basic <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[4]%in%c(     ) ) cnt<-cnt+1
			if( aFStep[5]%in%c( 6,2 ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			if( fCutU.hasPtn(c( 8, 6),aFStep) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c(11, 1),aFStep) ) cnt.w1<-cnt.w1+1
			if( all(aFStep[1:2+0]==c( 7, 6)) )	cnt.w1<-cnt.w1+1

			cnt.w2 <- 0
			if( all(aFStep[2:3]== aFStep[4]*c(3,5) ) )		cnt.w2<-cnt.w2+1
			if( sum(aFStep[c(3,4)])==sum(aFStep[c(1,5)]) )	cnt.w2<-cnt.w2+1
			if( aFStep[1]==sum(aFStep[c(4,5)]) )			cnt.w2<-cnt.w2+1

			cntMtx[idx,"fStep.w1"] <- cnt.w1
			cntMtx[idx,"fStep.w2"] <- cnt.w2
			cntMtx[idx,"fStep"] <- cnt + cnt.w1 + cnt.w2
		}

	} # for( idx )	# kIdx<-anaFltCnt(cntMtx[,"raw"],rpt)

	return( list( cccObj=cccObj	,auxCntMtx=auxCntMtx	,cntMtx=cntMtx	) )
		# lastZoid는 cccObj 안에 담겨있음.

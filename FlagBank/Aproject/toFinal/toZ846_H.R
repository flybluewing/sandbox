...
fCutCnt.basic <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( all(quoSize[1:3+1]==c(1,3,1)) ) return(FALSE)	# next rebind of 2,0,1
					if( all(quoSize[1:3+2]==c(1,1,1)) ) return(FALSE)	# next rebind of 0,1,1
					#	unique	동일 패턴 반복이 최근에 일어났다.. 또 일어날 수 있을까?
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			...
			if( aZoid[3]%in%c( 33,42    ) ) cnt<-cnt+1
			if( aZoid[4]%in%c( 36       ) ) cnt<-cnt+1
			if( aZoid[5]%in%c( 12       ) ) cnt<-cnt+1
			if( aZoid[6]%in%c( 45,42,41 ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			...
			if( fCutU.remFilt(aZoid[3],c( 4,3,2,5   ),c( 33,42    )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 4,6,0     ),c( 36       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 3,1,2     ),c( 12       )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 5,2,1,4,6 ),c( 45,42,41 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    1(3)   2(3)   3(5)   4(2)   5(3)   6(2)   7(2)   12(2) 
			cnt.w2 <- 0
			...
			#      2  4 11 28 29 43    | 2  7 17  1 14 |                        |2 1 2 0 1 |2 1 2 1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[6]%in%c(  2, 4  ) ) cnt<-cnt+1
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    -5(3)   0(2)   3(3)   4(2)   6(2)   7(2)   18(2) 
			cnt.w2 <- 0
			if( 1<sum( aFStep[c(1,2)]*c(-3, 2)==aFStep[c(4,3)] ) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextZW <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	auxCntMtx <- matrix( 0 ,nrow=length(allIdxF) ,ncol=2 )	;colnames(auxCntMtx)=c("auxZW","auxQuo")
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					if( (aZoid[6]-aZoid[1]) %in% c( 27, 32 ) ) return( FALSE )
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxZW"] <- !flag
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(0,2,1)) ) return(FALSE)	# next rebind of 0,1,2
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			if( fCutU.hasPtn(c( 37,NA,43 ),aZoid) ) cnt<-cnt+1
			# <44>
			if( fCutU.hasPtn(c( 37,44    ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(          38,NA,44 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c( 22,35,32,NA,NA,44 ),aZoid,thld=3,fixIdx=6) ) cnt<-cnt+1
			# <45>
			if( fCutU.hasPtn(c(  9,NA,NA,NA,45 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(    16,NA,NA,45 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(       31,36,45 ),aZoid) ) cnt<-cnt+1
			cntMtx[idx,"rawFV"] <- cnt
		}
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 4,5        ),c(       )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 6,8        ),c(       )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 5,3,0,6,8  ),c(       )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c(            ),c(       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 7,2        ),c( 42    )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 4,5        ),c( 44,45 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c(  4, 2   ) ) cnt<-cnt+1
			if( aCStep[2]%in%c(  4      ) ) cnt<-cnt+1
			if( aCStep[3]%in%c(         ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(         ) ) cnt<-cnt+1
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    1(2)   2(5)   3(5)   4(3)   5(3)   6(2)   9(3) 
			cnt.w2 <- 0
			if( 1<sum( aCStep[ 2 ]*c(3,2)==aCStep[c(3,4)] ) )	cnt.w2<-cnt.w2+1
			if( aCStep[3]==sum(aCStep[c(1,5)]) )	cnt.w2<-cnt.w2+1
			if( sum(aCStep[c(2,4)])==sum(aCStep[c(1,5)]) )	cnt.w2<-cnt.w2+1	# 9

			cntMtx[idx,"cStep.w1"] <- cnt.w1	;cntMtx[idx,"cStep.w2"] <- cnt.w2
			cntMtx[idx,"cStep"] <- cnt + cnt.w1 + cnt.w2

			#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
			#     29 31 35 38 40 44    | 2  4  3  2  4 |                        |0 0 1 3 2 |1 3 2
			#      5  7 18 37 42 45    | 2 11 19  5  3 |-24 -24 -17  -1   2   1 |2 1 0 1 2 |2 1 1 2
			#      1  8 17 34 39 45(1) | 7  9 17  5  6 | -4   1  -1  -3  -3   0 |2 1 0 2 1 |2 1 2 1
			#     12 15 24 36 41 44    | 3  9 12  5  3 | 11   7   7   2   2  -1 |0 2 1 1 2 |2 1 1 2
			#      3  4  6 10 28 30    | 1  2  4 18  2 | -9 -11 -18 -26 -13 -14 |3 1 1 1 0 |3 1 1 1
			#     17 25 28 37 43 44(1) | 8  3  9  6  1 | 14  21  22  27  15  14 |0 1 2 1 2 |1 2 1 2
		}
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c(        ) ) cnt<-cnt+1
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    -24(2)   -3(2)   -1(3)   1(2)   2(3)   7(2)   14(2) 
			cnt.w2 <- 0
			if( sum(aFStep[c(2,5)])==sum(aFStep[c(1,3)]) )	cnt.w2<-cnt.w2+1	# 36
			if( sum(aFStep[c(2,5)])==sum(aFStep[c(6,3)]) )	cnt.w2<-cnt.w2+1	# 36
...
fCutCnt.nextQuo10 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(0,2,0)) ) return(FALSE)	# next rebind of 1,2,1
					if( all(quoSize[1:3+1]==c(2,1,1)) ) return(FALSE)	# next rebind of 1,0,2
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			...
			if( aZoid[1]%in%c(          ) ) cnt<-cnt+1
			if( aZoid[2]%in%c(          ) ) cnt<-cnt+1
			if( aZoid[3]%in%c( 13,25    ) ) cnt<-cnt+1
			if( aZoid[4]%in%c(          ) ) cnt<-cnt+1
			if( aZoid[5]%in%c( 36       ) ) cnt<-cnt+1
			if( aZoid[6]%in%c( 42,36,37 ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			cnt <- 0
			# <13>
			if( fCutU.hasPtn(c(       13,NA,36    ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(  9, 4,13,45,NA,44 ),aZoid,thld=3,fixIdx=3) ) cnt<-cnt+1
			# <18>
			if( fCutU.hasPtn(c(    18,NA,NA,NA,42 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(  3,18,34,NA,43    ),aZoid,thld=3,fixIdx=2) ) cnt<-cnt+1
			# <30>
			if( fCutU.hasPtn(c(  9,NA,30       ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(       30,NA,40 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(     8,30,31    ),aZoid) ) cnt<-cnt+1
			# <36>
			if( fCutU.hasPtn(c(       13,NA,36    ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(          35,36 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(     3, 5,NA,36 ),aZoid) ) cnt<-cnt+1
			# <37>
			if( fCutU.hasPtn(c( 16,NA,NA,37    ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(          37,42 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(    12,17,37    ),aZoid) ) cnt<-cnt+1
			# <39>
			if( fCutU.hasPtn(c(  3,24,39 ),aZoid) ) cnt<-cnt+1
			# <40>
			if( fCutU.hasPtn(c(       30,NA,40 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(  9,NA,NA,NA,40 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(     8,NA,31,40 ),aZoid) ) cnt<-cnt+1
			# <41>
			if( fCutU.hasPtn(c( 16,NA,NA,NA,41 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(     8, 9,34,41 ),aZoid) ) cnt<-cnt+1
			cntMtx[idx,"rawFV"] <- cnt
		}
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 8,2      ),c(          )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 6,7,9    ),c(          )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 3,5,8    ),c( 13,25    )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 0        ),c(          )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 6,3,5    ),c( 36       )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 2,6,7    ),c( 42,36,37 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			...
			if( aCStep[3]%in%c( 10, 2       ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(  5,11       ) ) cnt<-cnt+1
			if( aCStep[5]%in%c(  3       ) ) cnt<-cnt+1
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    1(3)   2(5)   3(3)   5(5)   7(2)   8(2)   9(2) 
			cnt.w2 <- 0
			if( 1<sum( aCStep[c(2,3)]*c(2,3)==aCStep[c(5,1)] ) )	cnt.w2<-cnt.w2+1
			if( aCStep[1]==sum(aCStep[c(2,4)]) )	cnt.w2<-cnt.w2+1
			if( sum(aCStep[c(1,2)])==sum(aCStep[c(4,5)]) )	cnt.w2<-cnt.w2+1	# 17

			cntMtx[idx,"cStep.w1"] <- cnt.w1	;cntMtx[idx,"cStep.w2"] <- cnt.w2
			cntMtx[idx,"cStep"] <- cnt + cnt.w1 + cnt.w2

			#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
			#     16 23 25 33 36 39    | 7  2  8  3  3 |                        |0 1 2 3 0 |1 2 3
			#     11 18 26 31 37 40    | 7  8  5  6  3 | -5  -5   1  -2   1   1 |0 2 1 2 1 |2 1 2 1
			#      3 12 13 15 34 36    | 9  1  2 19  2 | -8  -6 -13 -16  -3  -4 |1 3 0 2 0 |1 3 2
			#      7 18 30 39 40 41    |11 12  9  1  1 |  4   6  17  24   6   5 |1 1 0 2 2 |1 1 2 2
			#      6  8 13 30 35 40(2) | 2  5 17  5  5 | -1 -10 -17  -9  -5  -1 |2 1 0 2 1 |2 1 2 1
			#      2 17 19 24 37 41    |15  2  5 13  4 | -4   9   6  -6   2   1 |1 2 1 1 1 |1 2 1 1 1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    -6(2)   -5(3)   -4(2)   -1(2)   1(4)   6(3) 
			cnt.w2 <- 0
			if( 1<sum( aFStep[c( 5, 4)]*c(-2,-1)==aFStep[c(1,3)] ) )	cnt.w2<-cnt.w2+1
			...
			if( sum(aFStep[c(2,4)])==sum(aFStep[c(1,3,6)]) )	cnt.w2<-cnt.w2+1	# 3
			if( sum(aFStep[c(2,4)])==sum(aFStep[c(5,6)]) )	cnt.w2<-cnt.w2+1	# 3
...
fCutCnt.nextBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(0,0,0)) ) return(FALSE)	# next rebind of 1,2,0
					if( all(quoSize[1:3+2]==c(0,0,0)) ) return(FALSE)	# next rebind of 0,2,1
					if( all(quoSize[1:3+1]==c(1,0,2)) ) return(FALSE)	# next rebind of 1,0,2
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			...
			if( aZoid[6]%in%c( 45,33      ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			cnt <- 0
			# < 1>
			if( fCutU.hasPtn(c(  1, 4, 8,41 ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
			# < 8>
			# <12>
			...
			# <17>
			# <27>
			if( fCutU.hasPtn(c(    10,NA,27       ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(          27,29    ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(  9,NA, 2,27,NA,39 ),aZoid,thld=3,fixIdx=4) ) cnt<-cnt+1
			# <29>
			if( fCutU.hasPtn(c(          27,29    ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(    10,NA,NA,29    ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(  9,NA, 2,NA,29,39 ),aZoid,thld=3,fixIdx=5) ) cnt<-cnt+1
			# <33>
			# <36>
			# <42>
			if( fCutU.hasPtn(c( 12,NA,NA,42 ),aZoid) ) cnt<-cnt+1
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			...
			if( fCutU.remFilt(aZoid[3],c( 1,0        ),c(       )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 5,1,2,4        ),c( 12, 4 )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 4,2        ),c(       )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 5,3,0     ),c( 45,33 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			...
			if( 1<sum(aCStep[1:2+]==c(  4, 1  )) )	cnt.w1<-cnt.w1+1
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    1 (3)   2 (5)   3 (3)   4 (2)   6 (3)   7 (3)   9 (3) 
			cnt.w2 <- 0
			...
			#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
			#      1 12 26 27 29 33    |11 14  1  2  4 |                        |1 1 3 1 0 |1 1 3 1
			#      5 11 14 27 29 36(2) | 6  3 13  2  7 |  4  -1 -12   0   0   3 |1 2 2 1 0 |1 2 2 1
			#      1  8 17 34 39 45    | 7  9 17  5  6 | -4  -3   3   7  10   9 |2 1 0 2 1 |2 1 2 1
			#      4  8  9 16 17 19(2) | 4  1  7  1  2 |  3   0  -8 -18 -22 -26 |3 3 0 0 0 |3 3
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c( -1, 3  ) ) cnt<-cnt+1
			if( aFStep[2]%in%c(        ) ) cnt<-cnt+1
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    -3 (2)   -1 (3)   0 (3)   3 (3)   4 (2) 
			cnt.w2 <- 0
			if( 1<sum( aFStep[ 6 ]*c(7,3)==aFStep[c(3,5)] ) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextRebNum <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( all(quoSize[1:3+0]==c(0,2,2)) ) return(FALSE)	# next rebind of 1,1,2
					if( all(quoSize[1:3+2]==c(2,1,1)) ) return(FALSE)	# next rebind of 2,2,0
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			...
			if( aZoid[4]%in%c( 24,31 ) ) cnt<-cnt+1
			if( aZoid[5]%in%c(       ) ) cnt<-cnt+1
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			if( fCutU.hasPtn(c(  6,25,NA,NA,35 ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
			# < 8>
			...
			if( fCutU.hasPtn(c( 10,16,NA,43,42 ),aZoid,thld=3,fixIdx=2) ) cnt<-cnt+1
			# <18>
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			...
			if( fCutU.remFilt(aZoid[3],c(         ),c( 19    )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c(         ),c( 24,31 )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c(         ),c(       )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c(         ),c( 37    )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    1 (7)   2(3)   3(2)   6(2)   7(2)   10(2)   12(2)   20(2) 
			cnt.w2 <- 0
			...
			#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
			#      2 16 19 31 34 35    |14  3 12  3  1 |                        |1 2 0 3 0 |1 2 3
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    -10(2)   -4(2)   -3(2)   -2(2)   -1(2)   0(2)   1(2)   2(3)   3(2)   4(2)   5(2)   9(2) 
			cnt.w2 <- 0
			if( 1<sum( aFStep[ 1 ]*c(-2,-3, 3)==aFStep[c(2,4,5)] ) )	cnt.w2<-cnt.w2+1
			if( 1<sum( aFStep[c(1,4)]*c(-2,-1)==aFStep[c(2,5)] ) )	cnt.w2<-cnt.w2+1
			...
			if( sum(aFStep[c(1,2)])==sum(aFStep[c(3,5,6)]) )	cnt.w2<-cnt.w2+1	# -2
			if( sum(aFStep[c(2,6)])==sum(aFStep[c(1,3)]) )	cnt.w2<-cnt.w2+1	# -5
			if( sum(aFStep[c(3,6)])==sum(aFStep[c(1,2,4)]) )	cnt.w2<-cnt.w2+1	# -8
...
fCutCnt.nextCStepBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( all(quoSize[1:3+2]==c(2,0,2)) ) return(FALSE)	# next rebind of 2,0,1
					if( all(quoSize[1:3+0]==c(0,0,0)) ) return(FALSE)	# next rebind of 1,2,0
					if( all(quoSize[1:3+2]==c(0,0,0)) ) return(FALSE)	# next rebind of 0,2,1
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			cnt <- 0
			if( aZoid[1]%in%c( 12,36      ) ) cnt<-cnt+1
			if( aZoid[2]%in%c( 10, 8     ) ) cnt<-cnt+1
			if( aZoid[3]%in%c( 14      ) ) cnt<-cnt+1
			if( aZoid[4]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[5]%in%c( 24      ) ) cnt<-cnt+1
			if( aZoid[6]%in%c( 45,27      ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			if( fCutU.hasPtn(c(  1,16, 9,14,19    ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
			# < 4>
			if( fCutU.hasPtn(c(  4,11,12,20 ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
			# < 8>
			# <12>
			if( fCutU.hasPtn(c( 12,NA,15,16 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(    12,NA,NA,42 ),aZoid) ) cnt<-cnt+1
			# <17>
			# <33>
			# <42>
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 0,2,6        ),c( 12,36 )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 0,8        ),c( 10, 8 )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 4,9,5,2 ),c( 14    )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c(         ),c(       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 4       ),c( 24    )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 5       ),c( 45,27 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			...
			if( aCStep[3]%in%c(         ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(  3, 7      ) ) cnt<-cnt+1
			if( aCStep[5]%in%c( 21      ) ) cnt<-cnt+1
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    1(5)   2(3)   3(2)   5(2)   6(3)   7(2)   9(3)   11(2) 
			cnt.w2 <- 0
			if( 1<sum( aCStep[c(1,4)]*c(1,3)==aCStep[c(3,5)] ) )	cnt.w2<-cnt.w2+1
			if( sum(aCStep[c(1,4)])==sum(aCStep[c(2,5)]) )	cnt.w2<-cnt.w2+1	# 16
			if( sum(aCStep[c(3,4)])==sum(aCStep[c(2,5)]) )	cnt.w2<-cnt.w2+1	# 16
			...
			#      3 12 33 36 42 45    | 9 21  3  6  3 | -1   4  24  20  25  26 |1 1 0 2 2 |1 1 2 2
			#      2 10 12 31 33 42(3) | 8  2 19  2  9 | -1  -2 -21  -5  -9  -3 |1 2 0 2 1 |1 2 2 1
			#      1 12 13 24 29 44(1) |11  1 11  5 15 | -1   2   1  -7  -4   2 |1 2 2 0 1 |1 2 2 1

		}
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c( -1, 0,-2      ) ) cnt<-cnt+1
			if( aFStep[2]%in%c(        ) ) cnt<-cnt+1
			if( aFStep[3]%in%c(        ) ) cnt<-cnt+1
			...
			if( sum(aFStep[c(1,5)])==sum(aFStep[c(2,4)]) )	cnt.w2<-cnt.w2+1	# -5
			if( sum(aFStep[c(1,5)])==sum(aFStep[c(6,4)]) )	cnt.w2<-cnt.w2+1	# -5
...
fCutCnt.nextFStepBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(0,1,2)) ) return(FALSE)	# next rebind of 0,1,2
					if( all(quoSize[1:3+2]==c(2,1,2)) ) return(FALSE)	# next rebind of 2,3,0
					if( all(quoSize[1:3+0]==c(0,1,2)) ) return(FALSE)	# next rebind of 0,1,2
					if( all(quoSize[1:3+1]==c(1,0,2)) ) return(FALSE)	# next rebind of 1,2,1

					if( all(quoSize[1:3+0]==c(1,0,2)) ) return(FALSE)	# next rebind of 0,1,2	# unique
					if( all(quoSize[1:3+2]==c(2,2,1)) ) return(FALSE)	# next rebind of 2,3,0	# unique
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			cnt <- 0
			if( aZoid[1]%in%c(             ) ) cnt<-cnt+1
			if( aZoid[2]%in%c( 22, 9,20    ) ) cnt<-cnt+1
			if( aZoid[3]%in%c(             ) ) cnt<-cnt+1
			if( aZoid[4]%in%c( 31,33,12,24 ) ) cnt<-cnt+1
			if( aZoid[5]%in%c(             ) ) cnt<-cnt+1
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			cnt <- 0
			# <10>
			if( fCutU.hasPtn(c( 10,32,33 ),aZoid) ) cnt<-cnt+1
			# <21>
			if( fCutU.hasPtn(c(  7,NA,NA,21 ),aZoid) ) cnt<-cnt+1
			# <22>
			if( fCutU.hasPtn(c( 13,22,25,29 ),aZoid,thld=3,fixIdx=2) ) cnt<-cnt+1
			# <31>
			if( fCutU.hasPtn(c( 18,18,19,31,32,33 ),aZoid,thld=3,fixIdx=4) ) cnt<-cnt+1
			# <34>
			if( fCutU.hasPtn(c(  6,NA,NA,NA,34 ),aZoid) ) cnt<-cnt+1
			# <38>
			if( fCutU.hasPtn(c(    24,NA,NA,38 ),aZoid) ) cnt<-cnt+1
			cntMtx[idx,"rawFV"] <- cnt
		}
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 6        ),c(             )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 2,9,0        ),c( 22, 9,20    )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 0        ),c(             )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 1,3,2,4        ),c( 31,33,12,24 )) ) cnt<-cnt+1 # 4
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[4]%in%c(          ) ) cnt<-cnt+1
			if( aCStep[5]%in%c(  1, 7, 4 ) ) cnt<-cnt+1
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    1(5)   2(2)   3(2)   4(4)   6(3)   7(4)   8(2) 
			cnt.w2 <- 0
			...
			#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
			#      1  9 10 12 21 40    | 8  1  2  9 19 |                        |2 2 1 0 1 |2 2 1 1
			#      8 15 19 21 34 44(1) | 7  4  2 13 10 |  7   6   9   9  13   4 |1 2 1 1 1 |1 2 1 1 1
			#     18 22 25 31 38 45    | 4  3  6  7  7 | 10   7   6  10   4   1 |0 1 2 2 1 |1 2 2 1
			#      7 22 29 33 34 35(1) |15  7  4  1  1 |-11   0   4   2  -4 -10 |1 0 2 3 0 |1 2 3
			#     10 22 27 31 42 43(1) |12  5  4 11  1 |  3   0  -2  -2   8   8 |0 1 2 1 2 |1 2 1 2
			#     14 20 23 31 37 38(1) | 6  3  8  6  1 |  4  -2  -4   0  -5  -5 |0 1 2 3 0 |1 2 3

		}
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c(  5, -2,  2 ) ) cnt<-cnt+1
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    -5(2)   -4(2)   -2(3)   0(3)   4(4)   6(2)   7(2)   8(2)   9(2)   10(2) 
			cnt.w2 <- 0
...
fCutCnt.nextColVal_1 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+1]==c(0,2,0)) ) return(FALSE)	# next rebind of 1,3,1
					if( all(quoSize[1:3+2]==c(2,0,3)) ) return(FALSE)	# next rebind of 3,1,0
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			if( fCutU.hasPtn(c( 25,28 ),aZoid) ) cnt<-cnt+1
			# <30>
			if( fCutU.hasPtn(c( 30,32 ),aZoid) ) cnt<-cnt+1
			# <31>
			...
			# <45>
			if( fCutU.hasPtn(c( 25,28,27,45 ),aZoid,thld=3,fixIdx=4) ) cnt<-cnt+1
			cntMtx[idx,"rawFV"] <- cnt
		}
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			...
			if( fCutU.remFilt(aZoid[4],c(  8,1       ),c(    )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c(  0,3       ),c(  3 )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c(  4,3,2       ),c( 43 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c(  9,17, 2    ) ) cnt<-cnt+1
			if( aCStep[2]%in%c(  4, 3, 1    ) ) cnt<-cnt+1
			if( aCStep[3]%in%c(  1, 3       ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(  3, 7, 2,19 ) ) cnt<-cnt+1
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    1(5)   2(3)   3(4)   4(2)   6(2)   7(2)   9(5) 
			cnt.w2 <- 0
			if( 1<sum( aCStep[c(,)]*c(,)==aCStep[c( , )] ) )	cnt.w2<-cnt.w2+1
			if( aCStep[ ]==sum(aCStep[c( , )]) )	cnt.w2<-cnt.w2+1
			if( aCStep[ ]==sum(aCStep[c( , )]) )	cnt.w2<-cnt.w2+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c(        ) ) cnt<-cnt+1
			if( aFStep[2]%in%c( -11,-10       ) ) cnt<-cnt+1
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    -15(2)   -14(2)   -12(3)   6(2)   8(2)   12(2) 
			cnt.w2 <- 0
			if( 1<sum( aFStep[c(,)]*c(,)==aFStep[c(,)] ) )	cnt.w2<-cnt.w2+1
			if( aFStep[ ]==sum(aFStep[c( , )]) )	cnt.w2<-cnt.w2+1
			if( aFStep[ ]==sum(aFStep[c( , )]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_2 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	auxCntMtx <- matrix( 0 ,nrow=length(allIdxF) ,ncol=2 )	;colnames(auxCntMtx)=c("auxZW","auxQuo")
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					if( (aZoid[6]-aZoid[1]) %in% c( 39,36 ) ) return( FALSE )
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxZW"] <- !flag
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(1,1,0)) ) return(FALSE)	# next rebind of 1,2,1
					if( all(quoSize[1:3+1]==c(1,0,3)) ) return(FALSE)	# next rebind of 2,1,1
					if( all(quoSize[1:3+2]==c(0,3,1)) ) return(FALSE)	# next rebind of 1,1,1
					#	unique	1 2 1 1 1, 1 1 0 3 1
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			if( fCutU.hasPtn(c(  5,NA,11       ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(  5,18          ),aZoid) ) cnt<-cnt+1
			# <13>
			if( fCutU.hasPtn(c( 13,35,NA,45 ),aZoid) ) cnt<-cnt+1
			# <18>
			if( fCutU.hasPtn(c(  5,18          ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(    18,40,44,45 ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
			# <19>
			...
			# <41>
			if( fCutU.hasPtn(c(          37,41 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c( 24,25,NA,NA,41 ),aZoid) ) cnt<-cnt+1
			cntMtx[idx,"rawFV"] <- cnt
		}
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 4,7,1       ),c(  4, 7 )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 6       ),c( 16    )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 3,0        ),c(       )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 4,9        ),c( 24    )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 9        ),c( 39    )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 3,2        ),c( 43,32 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			...
			if( aCStep[3]%in%c(         ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(  4,13   ) ) cnt<-cnt+1
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    1(2)   2(2)   3(3)   4(5)   5(3)   6(2)   7(2)   9(2)   11(2)   13(2) 
			cnt.w2 <- 0
			if( 1<sum( aCStep[c(,)]*c(,)==aCStep[c( , )] ) )	cnt.w2<-cnt.w2+1
			if( aCStep[ ]==sum(aCStep[c( , )]) )	cnt.w2<-cnt.w2+1
			if( aCStep[ ]==sum(aCStep[c( , )]) )	cnt.w2<-cnt.w2+1
			...
			#     13 19 28 37 38 43    | 6  9  9  1  5 |  6   1   6  13   7   9 |0 2 1 2 1 |2 1 2 1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    -5(2)   -3(2)   -1(3)   0(2)   1(3)   4(2)   6(3)   7(2) 
			cnt.w2 <- 0
			if( 1<sum( aFStep[c(,)]*c(,)==aFStep[c(,)] ) )	cnt.w2<-cnt.w2+1
			if( aFStep[ ]==sum(aFStep[c( , )]) )	cnt.w2<-cnt.w2+1
			if( aFStep[ ]==sum(aFStep[c( , )]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_3 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( all(quoSize[1:3+1]==c(0,1,1)) ) return(FALSE)	# next rebind of 1,2,0
					if( all(quoSize[1:3+2]==c(1,1,3)) ) return(FALSE)	# next rebind of 2,0,1
					#	unique	2 1 2 0 1-->1 0 1 1 3
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			# <11>
			# <16>
			if( fCutU.hasPtn(c(  6,NA,13,16 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(     6,13,16 ),aZoid) ) cnt<-cnt+1
			# <28>
			# <41>
			# <44>
			cntMtx[idx,"rawFV"] <- cnt
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			...
			if( fCutU.remFilt(aZoid[4],c( 1,2,9    ),c( 21 )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 1,8      ),c(    )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 6,5      ),c( 45 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    1(4)   2(3)   3(4)   4(3)   5(2)   6(2)   7(5)   12(2) 
			cnt.w2 <- 0
			if( 1<sum( aCStep[c(,)]*c(,)==aCStep[c( , )] ) )	cnt.w2<-cnt.w2+1
			if( aCStep[ ]==sum(aCStep[c( , )]) )	cnt.w2<-cnt.w2+1
			if( aCStep[ ]==sum(aCStep[c( , )]) )	cnt.w2<-cnt.w2+1
			...
			#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
			#     11 23 26 29 39 44    |12  3  3 10  5 |                        |0 1 3 1 1 |1 3 1 1
			#      2  8 15 22 25 41    | 6  7  7  3 16 | -9 -15 -11  -7 -14  -3 |2 1 2 0 1 |2 1 2 1
			#      1 28 35 41 43 44(1) |27  7  6  2  1 | -1  20  20  19  18   3 |1 0 1 1 3 |1 1 1 3
			#      4  8  9 16 17 19    | 4  1  7  1  2 |  3 -20 -26 -25 -26 -25 |3 3 0 0 0 |3 3
			#      5  7 11 16 41 45(1) | 2  4  5 25  4 |  1  -1   2   0  24  26 |2 2 0 0 2 |2 2 2
			#      8  9 18 21 28 40    | 1  9  3  7 12 |  3   2   7   5 -13  -5 |2 1 2 0 1 |2 1 2 1
		}
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c(  -1,  2,-25      ) ) cnt<-cnt+1
			if( aFStep[2]%in%c(        ) ) cnt<-cnt+1
			if( aFStep[3]%in%c(   3     ) ) cnt<-cnt+1
			if( aFStep[4]%in%c(        ) ) cnt<-cnt+1
			if( aFStep[5]%in%c(        ) ) cnt<-cnt+1
...
fCutCnt.nextColVal_4 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(0,0,0)) ) return(FALSE)	# next rebind of 2,2,0
					if( all(quoSize[1:3+1]==c(0,0,0)) ) return(FALSE)	# next rebind of 2,0,1
					if( all(quoSize[1:3+2]==c(0,3,3)) ) return(FALSE)	# next rebind of 0,1,1
					if( all(quoSize[1:3+2]==c(2,0,1)) ) return(FALSE)	# next rebind of 0,0,0
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			...
			if( aZoid[2]%in%c(  7, 8,14  ) ) cnt<-cnt+1
			if( aZoid[3]%in%c(           ) ) cnt<-cnt+1
			if( aZoid[4]%in%c(           ) ) cnt<-cnt+1
			if( aZoid[5]%in%c(           ) ) cnt<-cnt+1
			if( aZoid[6]%in%c(           ) ) cnt<-cnt+1
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			cnt <- 0
			# < 8>
			if( fCutU.hasPtn(c(  8,17,12,45 ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
			# < 9>
			# <13>
			if( fCutU.hasPtn(c(  3, 4,13 ),aZoid) ) cnt<-cnt+1
			# <16>
			# <17>
			...
			# <45>
			if( fCutU.hasPtn(c(  8,NA,NA,NA,NA,45 ),aZoid) ) cnt<-cnt+1
			cntMtx[idx,"rawFV"] <- cnt
		}
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 4,8     ),c(  4        )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 7,8,4,3 ),c(  7, 8,14  )) ) cnt<-cnt+1 # 2
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[2]%in%c(  2,21, 3      ) ) cnt<-cnt+1
			if( aCStep[3]%in%c(  3, 4      ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(  2, 1      ) ) cnt<-cnt+1
			if( aCStep[5]%in%c(  6, 4      ) ) cnt<-cnt+1
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    1(7)   2(4)   3(2)   4(3)   5(2)   6(2)   7(2)   12(2) 
			cnt.w2 <- 0
			if( 1<sum( aCStep[c(,)]*c(,)==aCStep[c( , )] ) )	cnt.w2<-cnt.w2+1
			if( aCStep[ ]==sum(aCStep[c( , )]) )	cnt.w2<-cnt.w2+1
			if( aCStep[ ]==sum(aCStep[c( , )]) )	cnt.w2<-cnt.w2+1
			...
			#      7  8 13 15 33 45(1) | 1  5  2 18 12 |  4  -1   2   3  20  26 |2 2 0 1 1 |2 2 1 1

		}
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c(        ) ) cnt<-cnt+1
			if( aFStep[2]%in%c( -2, 2  ) ) cnt<-cnt+1
			if( aFStep[3]%in%c(        ) ) cnt<-cnt+1
			if( aFStep[4]%in%c(        ) ) cnt<-cnt+1
			if( aFStep[5]%in%c(        ) ) cnt<-cnt+1
			if( aFStep[6]%in%c(        ) ) cnt<-cnt+1
...
fCutCnt.nextColVal_5 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(1,1,1)) ) return(FALSE)	# next rebind of 0,2,0
					if( all(quoSize[1:3+1]==c(2,1,1)) ) return(FALSE)	# next rebind of 2,0,2
					if( all(quoSize[1:3+2]==c(1,2,1)) ) return(FALSE)	# next rebind of 0,2,2
					if( all(quoSize[1:3+1]==c(2,1,1)) ) return(FALSE)	# next rebind of 1,2,2
					if( all(quoSize[1:3+2]==c(1,1,2)) ) return(FALSE)	# next rebind of 2,2,1
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			cnt <- 0
			if( aZoid[1]%in%c(          ) ) cnt<-cnt+1
			if( aZoid[2]%in%c( 30,39    ) ) cnt<-cnt+1
			if( aZoid[3]%in%c(          ) ) cnt<-cnt+1
			if( aZoid[4]%in%c( 40       ) ) cnt<-cnt+1
			if( aZoid[5]%in%c( 39       ) ) cnt<-cnt+1
			if( aZoid[6]%in%c( 42,45,43 ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			# <30>
			if( fCutU.hasPtn(c(       30,NA,40 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(  3,14,30,43    ),aZoid,thld=3,fixIdx=3) ) cnt<-cnt+1
			# <31>
			# <32>
			...
			# <39>
			if( fCutU.hasPtn(c(  2, 5,28,39 ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
			# <41>
			if( fCutU.hasPtn(c(    17,NA,NA,41 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(  7,NA,36,NA,41 ),aZoid) ) cnt<-cnt+1
			# <42>
			if( fCutU.hasPtn(c( 14,15,38,NA,NA,42 ),aZoid,thld=3,fixIdx=6) ) cnt<-cnt+1
			cntMtx[idx,"rawFV"] <- cnt
		}
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 8        ),c(          )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 0,9      ),c( 30,39    )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 9        ),c(          )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 0        ),c( 40       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 2,9      ),c( 39       )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 2,5,3,4  ),c( 42,45,43 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c(         ) ) cnt<-cnt+1
			if( aCStep[2]%in%c(         ) ) cnt<-cnt+1
			if( aCStep[3]%in%c( 10,13   ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(         ) ) cnt<-cnt+1
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    1(4)   2(3)   3(2)   4(4)   5(2)   6(3)   7(2)   9(2)   11(2) 
			cnt.w2 <- 0
			if( 1<sum( aCStep[c(,)]*c(,)==aCStep[c( , )] ) )	cnt.w2<-cnt.w2+1
			if( aCStep[ ]==sum(aCStep[c( , )]) )	cnt.w2<-cnt.w2+1
			if( aCStep[ ]==sum(aCStep[c( , )]) )	cnt.w2<-cnt.w2+1
			...
			#     12 18 30 39 41 42(2) | 6 12  9  2  1 |  2  -3   8   9   6   0 |0 2 0 2 2 |2 2 2
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    -15(2)   -3(2)   -1(4)   2(2)   5(2)   6(3)   8(3)   9(2) 
			cnt.w2 <- 0
			if( 1<sum( aFStep[c(,)]*c(,)==aFStep[c(,)] ) )	cnt.w2<-cnt.w2+1
			if( aFStep[ ]==sum(aFStep[c( , )]) )	cnt.w2<-cnt.w2+1
			if( aFStep[ ]==sum(aFStep[c( , )]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_6 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			...
			if( aZoid[4]%in%c( 33,14      ) ) cnt<-cnt+1
			if( aZoid[5]%in%c( 35,25      ) ) cnt<-cnt+1
			if( aZoid[6]%in%c( 38,36      ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			# <16>
			if( fCutU.hasPtn(c( 16,40,29,42 ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
			# <17>
			# <21>
			if( fCutU.hasPtn(c(  3,21,NA,34,43 ),aZoid,thld=3,fixIdx=2) ) cnt<-cnt+1
			# <26>
			if( fCutU.hasPtn(c(          26,29 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(  1, 1, 3,26    ),aZoid,thld=3,fixIdx=4) ) cnt<-cnt+1
			# <33>
			...
			if( fCutU.hasPtn(c( 13,22,NA,NA,NA,38 ),aZoid) ) cnt<-cnt+1
			cntMtx[idx,"rawFV"] <- cnt
		}
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 5,7     ),c(  5,15 )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c(         ),c(       )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 8       ),c( 18    )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 3,4     ),c( 33,14 )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 5,0,6   ),c( 35,25 )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 8,6,5   ),c( 38,36 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    1(5)   2(4)   3(3)   4(3)   5(3)   7(2)   8(2) 
			cnt.w2 <- 0
			if( 1<sum( aCStep[c(,)]*c(,)==aCStep[c( , )] ) )	cnt.w2<-cnt.w2+1
			if( aCStep[ ]==sum(aCStep[c( , )]) )	cnt.w2<-cnt.w2+1
			if( aCStep[ ]==sum(aCStep[c( , )]) )	cnt.w2<-cnt.w2+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    -5(2)   -4(3)   0(2)   2(2)   4(2)   6(2)   8(2)   10(2) 
			cnt.w2 <- 0
			if( 1<sum( aFStep[c(,)]*c(,)==aFStep[c(,)] ) )	cnt.w2<-cnt.w2+1
			if( aFStep[ ]==sum(aFStep[c( , )]) )	cnt.w2<-cnt.w2+1
			if( aFStep[ ]==sum(aFStep[c( , )]) )	cnt.w2<-cnt.w2+1

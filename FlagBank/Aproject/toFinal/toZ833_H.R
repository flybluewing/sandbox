...
fCutCnt.nextRebNum <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[5]%in%c( 15,16    ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]	# fv in cStep
			if( fCutU.hasPtn(c( 4,3 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( 1<sum(aCStep[1:2+3]==c(  1,21 )) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c( 1, 1 ),aCStep) )	cnt.w1<-cnt.w1+1	# -
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w2 <- 0
			if( 1<sum( aFStep[2]*c(1,-3)==aFStep[c(3,4)] ) )	cnt.w2<-cnt.w2+1
			...
			if( aFStep[6]==sum(aFStep[c( 1,4,2 )]) )	cnt.w2<-cnt.w2+1
			if( aFStep[6]==sum(aFStep[c( 1,4,3 )]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextCStepBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]	# fv in cStep
			if( fCutU.hasPtn(c( 2,1 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( 1<sum(aCStep[1:2+3]==c(  4,18 )) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c( 1,1 ),aCStep) )	cnt.w1<-cnt.w1+1	# -
			...
			if( aCStep[4]==sum(aCStep[c( 1,3,5 )]) )	cnt.w2<-cnt.w2+1
			if( aCStep[5]==sum(aCStep[c( 1,2 )]) )	cnt.w2<-cnt.w2+1
			if( aCStep[5]==sum(aCStep[c( 3,2 )]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextFStepBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c(  0       ) ) cnt<-cnt+1
			if( aCStep[2]%in%c(  4, 3, 1 ) ) cnt<-cnt+1
			if( aCStep[3]%in%c(          ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(  3, 5, 1 ) ) cnt<-cnt+1
			...
			cnt.w2 <- 0
			if( 1<sum( aCStep[c(1,3)]*c(2,2 )==aCStep[c(5,4)] ) )	cnt.w2<-cnt.w2+1
			if( aCStep[2]==sum(aCStep[c(1,4)]) )	cnt.w2<-cnt.w2+1
			if( sum(aCStep[c(1,2)])==sum(aCStep[c(4,5)]) )	cnt.w2<-cnt.w2+1	# 20
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[2]==sum(aFStep[c( 5,4 )]) )	cnt.w2<-cnt.w2+1
			if( aFStep[4]==sum(aFStep[c( 2,6,3 )]) )	cnt.w2<-cnt.w2+1
			if( aFStep[4]==sum(aFStep[c( 2,6,5 )]) )	cnt.w2<-cnt.w2+1
			if( aFStep[3]==sum(aFStep[c( 4,5,6 )]) )	cnt.w2<-cnt.w2+1
			if( aFStep[5]==sum(aFStep[c( 4,3,6 )]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_1 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[3]%in%c(         ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(  0, 8   ) ) cnt<-cnt+1
			if( aCStep[5]%in%c(  1      ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]	# fv in cStep
			if( fCutU.hasPtn(c( 6,1 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( 1<sum(aCStep[1:2+0]==c( 13, 3 )) )	cnt.w1<-cnt.w1+1
			if( 1<sum(aCStep[1:2+3]==c( 10, 1 )) )	cnt.w1<-cnt.w1+1	# -
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w2 <- 0
			if( 1<sum( aFStep[c(2,3)]*c(-2,-4)==aFStep[c(5,6)] ) )	cnt.w2<-cnt.w2+1
			if( aFStep[4]==sum(aFStep[c( 6,1 )]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_2 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[3]%in%c(  2, 3, 5, 4 ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(  6          ) ) cnt<-cnt+1
			if( aCStep[5]%in%c(             ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]	# fv in cStep
			if( fCutU.hasPtn(c( 4,5 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( 1<sum(aCStep[1:2+2]==c( 9,1 )) )	cnt.w1<-cnt.w1+1
			...
			cnt.w2 <- 0
			if( aCStep[4]==sum(aCStep[c( 5,1,2 )]) )	cnt.w2<-cnt.w2+1
			if( aCStep[5]==sum(aCStep[c( 1,2,3 )]) )	cnt.w2<-cnt.w2+1
			if( aCStep[ ]==sum(aCStep[c( , )]) )	cnt.w2<-cnt.w2+1
			if( sum(aCStep[c(1,4)])==sum(aCStep[c(3,5)]) )	cnt.w2<-cnt.w2+1	# 17
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[2]%in%c(         ) ) cnt<-cnt+1
			if( aFStep[3]%in%c(         ) ) cnt<-cnt+1
			if( aFStep[4]%in%c(  -1,-16 ) ) cnt<-cnt+1
			if( aFStep[5]%in%c(   3     ) ) cnt<-cnt+1
			if( aFStep[6]%in%c(  16     ) ) cnt<-cnt+1

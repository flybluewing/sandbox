...
fCutCnt.basic <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(1,2,1)) ) return(FALSE)	# next rebind of 1,1,2
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
    ...
    cntMtx[,"rawFV"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[4,],aZoid,posDiff=0 )	# <10>
					if( fCutU.hasPtn(c(10,NA,24      ),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(10,NA,NA,NA,34),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(10,23,NA,36   ),aZoid) ) cnt<-cnt+1
					...
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[4,],aZoid,posDiff=-2 )	# <14>
					if( fCutU.hasPtn(c(14,NA,16,19),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[5,],aZoid,posDiff=-1 )	# <18>
					if( fCutU.hasPtn(c(  7,18,19,36),aZoid,thld=3,fixIdx=2) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=0 )	# <26>
					if( fCutU.hasPtn(c(11,NA,26      ),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(   12,26      ),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(      26,34,29),aZoid) ) cnt<-cnt+1
					...
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[4,],aZoid,posDiff=-2 )	# <32>
					if( fCutU.hasPtn(c(17,24,NA,32),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(      31,32),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[2,],aZoid,posDiff=0 )	# <43>
					if( fCutU.hasPtn(c(25,33,NA,38,NA,43),aZoid,thld=3,fixIdx=6) ) cnt<-cnt+1
    ...
    cntMtx[,"rem"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( fCutU.remFilt(aZoid[2],c(2,1  ),c(     )) ) cnt<-cnt+1 # 2
    ...
    cntMtx[,"cStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( aCStep[5]%in%c( 1, 2) ) cnt<-cnt+1

					if( 1<sum(aCStep[1:3+0]==c( 4, 8, 7)) ) cnt<-cnt+1	# 6
		...
		#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
		#      5  9 14 26 30 43    | 4  5 12  4 13 |                        |2 1 1 1 1 |2 1 1 1 1
		#      1 10 13 26 32 36(1) | 9  3 13  6  4 | -4   1  -1   0   2  -7 |1 2 1 2 0 |1 2 1 2
		#      3 12 13 18 31 32(2) | 9  1  5 13  1 |  2   2   0  -8  -1  -4 |1 3 0 2 0 |1 3 2
		#     14 20 23 31 37 38(1) | 6  3  8  6  1 | 11   8  10  13   6   6 |0 1 2 3 0 |1 2 3
		#      6 10 18 25 34 35    | 4  8  7  9  1 | -8 -10  -5  -6  -3  -3 |1 2 1 2 0 |1 2 1 2
		#     15 21 31 32 41 43    | 6 10  1  9  2 |  9  11  13   7   7   8 |0 1 1 2 2 |1 1 2 2
		# -<standard zoid>---------------------------------------------------------------------
    ...
    cntMtx[,"fStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( 1<sum(aFStep[1:3+0]==c( , , )) ) cnt<-cnt+1 # 9
					if( 1<sum(aFStep[1:3+1]==c( -8,-10, -5)) ) cnt<-cnt+1 #11
					if( 1<sum(aFStep[1:3+1]==c( -5, -6, -3)) ) cnt<-cnt+1 #13
					if( 1<sum(aFStep[1:3+1]==c(  8,  1,  4)) ) cnt<-cnt+1 # 7
					if( 1<sum(aFStep[1:3+2]==c(  8,  1,  4)) ) cnt<-cnt+1 # 7
					if( 1<sum(aFStep[1:2+4]==c( -8,-10    )) ) cnt<-cnt+1 # 8
...
fCutCnt.nextZW <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+2]==c(3,0,0)) ) return(FALSE)	# next rebind of 0,3,1
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
    flgCnt[!flag] <- flgCnt[!flag] + 1
    ...
    cntMtx[,"rawFV"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[2,],aZoid,posDiff=0 )	# < 2>
					if( fCutU.hasPtn(c( 2,17,23,32,37,41),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[3,],aZoid,posDiff=1 )	# < 8>
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[3,],aZoid,posDiff=-2 )	# <18>
					if( fCutU.hasPtn(c(18,43,45),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[5,],aZoid,posDiff=3 )	# <19>
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[5,],aZoid,posDiff=-5 )	# <23>
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=-2 )	# <28>
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[2,],aZoid,posDiff=-1 )	# <31>
					if( fCutU.hasPtn(c( 8,31,32),aZoid) ) cnt<-cnt+1
					...
					if( fCutU.hasPtn(c( 4,15,34,37,40,45),aZoid,thld=3,fixIdx=6) ) cnt<-cnt+1
    ...
    cntMtx[,"rem"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					# u0.zoidMtx_ana( stdMI$rawTail%%10 )	;u0.zoidMtx_ana( stdMI$rawTail )
					cnt <- 0
					if( fCutU.remFilt(aZoid[1],c(1,0,8),c(     )) ) cnt<-cnt+1 # 1
					if( fCutU.remFilt(aZoid[2],c(8,7  ),c(     )) ) cnt<-cnt+1 # 2
					if( fCutU.remFilt(aZoid[3],c(9,8  ),c(28   )) ) cnt<-cnt+1 # 3
    ...
    cntMtx[,"cStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( aCStep[2]%in%c(13      , 3 ) ) cnt<-cnt+1
					if( aCStep[3]%in%c( 8          ) ) cnt<-cnt+1
					if( aCStep[4]%in%c( 3, 3, 5    ) ) cnt<-cnt+1
					if( aCStep[5]%in%c(            ) ) cnt<-cnt+1

					if( 1<sum(aCStep[1:3+1]==c( 3,10, 3)) ) cnt<-cnt+1	#13
					if( 1<sum(aCStep[1:3+1]==c(10, 4, 1)) ) cnt<-cnt+1	# 3
					if( 1<sum(aCStep[1:3+0]==c( 1,10, 4)) ) cnt<-cnt+1	# 3
					if( 1<sum(aCStep[1:2+3]==c(12,13   )) ) cnt<-cnt+1	# 4
					if( 1<sum(aCStep[1:3+2]==c(12,13, 3)) ) cnt<-cnt+1	# 7
					return( cnt )
				})	;kIdx<-anaFltCnt(cntMtx[,"cStep"],rpt)
			#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
			#      8 21 28 31 36 45    |13  7  3  5  9 |                        |1 0 2 2 1 |1 2 2 1
			#      2  5 15 18 19 23    | 3 10  3  1  4 | -6 -16 -13 -13 -17 -22 |2 3 1 0 0 |2 3 1
    ...
    cntMtx[,"fStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( 1<sum(aFStep[1:3+1]==c(-22,-18,-21)) ) cnt<-cnt+1 #22
					if( 1<sum(aFStep[1:2+4]==c(-19,-19    )) ) cnt<-cnt+1 #21
					return( cnt )
				})	;kIdx<-anaFltCnt(cntMtx[,"fStep"],rpt)
	score <- sapply( 1:length(flgCnt) ,function( idx ){
					if( any(cntMtx[idx,] >cntThld ) )	return( 2 )
...
fCutCnt.nextQuo10 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( all(quoSize[1:3+0]==c(2,1,1)) ) return(FALSE)	# next rebind of 2,1,0
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
    flgCnt[!flag] <- flgCnt[!flag] + 1
    ...
    cntMtx[,"raw"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					cnt <- 0
					if( aZoid[1]%in%c(        ) ) cnt<-cnt+1
					if( aZoid[2]%in%c(10, 9,16) ) cnt<-cnt+1
					if( aZoid[3]%in%c(11,12, 5) ) cnt<-cnt+1
					if( aZoid[4]%in%c(        ) ) cnt<-cnt+1
					if( aZoid[5]%in%c(        ) ) cnt<-cnt+1
					if( aZoid[6]%in%c(        ) ) cnt<-cnt+1
					return( cnt )
    ...
    cntMtx[,"rawFV"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[2,],aZoid,posDiff=-1 )	# <15>
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[4,],aZoid,posDiff=1 )	# <23>
					if( fCutU.hasPtn(c(17,20,20,23),aZoid,thld=3,fixIdx=4) ) cnt<-cnt+1
    ...
    cntMtx[,"rem"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( fCutU.remFilt(aZoid[2],c(0  ,6,5),c(10, 9,16)) ) cnt<-cnt+1 # 2
					if( fCutU.remFilt(aZoid[3],c(1,2,7,5),c(11,12, 5)) ) cnt<-cnt+1 # 3
    ...
    cntMtx[,"cStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( aCStep[2]%in%c( 9, 3) ) cnt<-cnt+1
					if( aCStep[3]%in%c(     ) ) cnt<-cnt+1
			...
			#  1  4  8 23 33 42    | 3  4 15 10  9 | -2  -7  -6   8   1   6 |3 0 1 1 1 |3 1 1 1
    ...
    cntMtx[,"fStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( 1<sum(aFStep[1:3+0]==c( -1, -2,  0)) ) cnt<-cnt+1 # -3
...
fCutCnt.nextBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    cntMtx[,"rawFV"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( fCutU.hasPtn(c( 1, 3, 5, 6,36   ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
					...
					if( fCutU.hasPtn(c( 6, 7      ),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(    7,10,11),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[4,],aZoid,posDiff=2 )	# <12>
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[3,],aZoid,posDiff=1 )	# <15>
					if( fCutU.hasPtn(c(13,15,NA,NA,20),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=1 )	# <20>
					...
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[5,],aZoid,posDiff=-1 )	# <39>
					if( fCutU.hasPtn(c(11, 4,25,39),aZoid,thld=3,fixIdx=4) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[5,],aZoid,posDiff=0 )	# <42>
					if( fCutU.hasPtn(c(23,35,39,30,42),aZoid,thld=3,fixIdx=5) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[4,],aZoid,posDiff=0 )	# <43>
					if( fCutU.hasPtn(c( 1, 9,12,23,39,43),aZoid,thld=3,fixIdx=6) ) cnt<-cnt+1
    ...
    cntMtx[,"rem"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( fCutU.remFilt(aZoid[4],c(0,7  ),c(     )) ) cnt<-cnt+1 # 4
					if( fCutU.remFilt(aZoid[5],c(0    ),c(20   )) ) cnt<-cnt+1 # 5
    ...
    cntMtx[,"cStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( 1<sum(aCStep[1:3+0]==c( 3,11,16)) ) cnt<-cnt+1	# 8
				...
				#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
				#      1 15 19 40 42 44    |14  4 21  2  2 |                        |1 2 0 0 3 |1 2 3
				#      6 12 18 31 38 43    | 6  6 13  7  5 |  5  -3  -1  -9  -4  -1 |1 2 0 2 1 |1 2 2 1
...
fCutCnt.nextRebNum <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	auxCntMtx <- matrix( 0 ,nrow=length(allIdxF) ,ncol=2 )	;colnames(auxCntMtx)=c("auxZW","auxQuo")
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					if( (aZoid[6]-aZoid[1]) %in% c(35,40,34) ) return( FALSE )
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxZW"] <- !flag
    flgCnt[!flag] <- flgCnt[!flag] + 1	
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+1]==c(1,1,0)) ) return(FALSE)	# next rebind of 2,1,2
					if( all(quoSize[1:3+1]==c(1,1,2)) ) return(FALSE)	# next rebind of 1,2,0
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
    flgCnt[!flag] <- flgCnt[!flag] + 1
    ...
    cntMtx[,"raw"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( aZoid[2]%in%c(        ) ) cnt<-cnt+1
					if( aZoid[3]%in%c(15,20   ) ) cnt<-cnt+1
					if( aZoid[4]%in%c(19      ) ) cnt<-cnt+1
					if( aZoid[5]%in%c(23      ) ) cnt<-cnt+1
					if( aZoid[6]%in%c(36,36,45) ) cnt<-cnt+1
					return( cnt )
				})	;kIdx<-anaFltCnt(cntMtx[,"raw"],rpt)
	neighborObj <- fCutU.neighborObj( stdMI$rawTail )
    cntMtx[,"rawFV"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[ ,],aZoid,posDiff=0 )	# < 1>
					if( fCutU.hasPtn(c( 1,NA,NA,NA,NA,36),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c( 1,18,11,33,40   ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[ ,],aZoid,posDiff=1 )	# <13>
					if( fCutU.hasPtn(c(13,32,40,39),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[ ,],aZoid,posDiff=0 )	# <15>
					if( fCutU.hasPtn(c(15,NA,23),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[ ,],aZoid,posDiff=-1 )	# <20>
					if( fCutU.hasPtn(c(20,24),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[ ,],aZoid,posDiff=1 )	# <24>
					if( fCutU.hasPtn(c(20,24),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(18,23,24),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[ ,],aZoid,posDiff=-1 )	# <27>
					if( fCutU.hasPtn(c(18,27,37),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[ ,],aZoid,posDiff=0 )	# <36>
					if( fCutU.hasPtn(c( 1,NA,NA,NA,NA,36),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(18,11,33,NA,36),aZoid,thld=3,fixIdx=5) ) cnt<-cnt+1
    ...
    cntMtx[,"rem"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( fCutU.remFilt(aZoid[2],c(2,3     ),c(     )) ) cnt<-cnt+1 # 2
					if( fCutU.remFilt(aZoid[3],c(5,0,1   ),c(15,20)) ) cnt<-cnt+1 # 3
					if( fCutU.remFilt(aZoid[4],c(4,9     ),c(19   )) ) cnt<-cnt+1 # 4
					if( fCutU.remFilt(aZoid[5],c(3,5,2,7 ),c(23   )) ) cnt<-cnt+1 # 5
					if( fCutU.remFilt(aZoid[6],c(6,5     ),c(36,45)) ) cnt<-cnt+1 # 6
					return( cnt )
				})	;kIdx<-anaFltCnt(cntMtx[,"rem"],rpt)
    cntMtx[,"cStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					cnt <- 0
				...
				#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
				#      3 13 20 24 33 37    |10  7  4  9  4 |                        |1 1 2 2 0 |1 1 2 2
				#     18 20 24 27 31 42(2) | 2  4  3  4 11 | 15   7   4   3  -2   5 |0 1 3 1 1 |1 3 1 1
				#      7 12 15 24 25 43(1) | 5  3  9  1 18 |-11  -8  -9  -3  -6   1 |1 2 2 0 1 |1 2 2 1
				#      5 21 27 34 44 45    |16  6  7 10  1 | -2   9  12  10  19   2 |1 0 2 1 2 |1 2 1 2
				#      1  2 15 19 24 36    | 1 13  4  5 12 | -4 -19 -12 -15 -20  -9 |2 2 1 1 0 |2 2 1 1
				#      1 10 13 26 32 36(2) | 9  3 13  6  4 |  0   8  -2   7   8   0 |1 2 1 2 0 |1 2 1 2
				# -<standard zoid>---------------------------------------------------------------------
    ...
    cntMtx[,"fStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( 1<sum(aFStep[1:3+0]==c(  2, -9,-12)) ) cnt<-cnt+1 # 8
					if( 1<sum(aFStep[1:3+2]==c( -4,-19,-12)) ) cnt<-cnt+1 #-2
					if( 1<sum(aFStep[1:3+2]==c(-11, -8, -9)) ) cnt<-cnt+1 # 7
...
fCutCnt.nextCStepBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    cntMtx[,"raw"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( aZoid[2]%in%c( 7,22, 9) ) cnt<-cnt+1
    ...
    cntMtx[,"rawFV"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=-1 )	# <16>
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=1 )	# <20>
					if( fCutU.hasPtn(c(11,NA,NA,20,41),aZoid) ) cnt<-cnt+1
    ...
    cntMtx[,"rem"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( fCutU.remFilt(aZoid[2],c(7,2,9 ),c( 7,22, 9 )) ) cnt<-cnt+1 # 2
					if( fCutU.remFilt(aZoid[3],c(1,5,4 ),c(         )) ) cnt<-cnt+1 # 3
					if( fCutU.remFilt(aZoid[4],c(4     ),c(11       )) ) cnt<-cnt+1 # 4
    ...
    cntMtx[,"cStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( 1<sum(aCStep[1:3+0]==c( 9, 2,16)) ) cnt<-cnt+1	# 8
					if( 1<sum(aCStep[1:2+0]==c(16, 4  )) ) cnt<-cnt+1	# 3
					if( 1<sum(aCStep[1:3+0]==c( 2,16, 4)) ) cnt<-cnt+1	#11
					if( 1<sum(aCStep[1:3+2]==c(12,17, 3)) ) cnt<-cnt+1	#16
					if( 1<sum(aCStep[1:3+2]==c(12,17, 3)) ) cnt<-cnt+1	# 4
					return( cnt )
				})	;kIdx<-anaFltCnt(cntMtx[,"cStep"],rpt)
				#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
				#      2 22 27 33 36 37    |20  5  6  3  1 |                        |1 0 2 3 0 |1 2 3
				#      3  6  7 20 21 39    | 3  1 13  1 18 |  1 -16 -20 -13 -15   2 |3 0 2 1 0 |3 2 1
				#      6  7 15 16 20 31(3) | 1  8  1  4 11 |  3   1   8  -4  -1  -8 |2 2 1 1 0 |2 2 1 1
    ...
    cntMtx[,"fStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( 1<sum(aFStep[1:3+1]==c(  2, 14,  7)) ) cnt<-cnt+1 # -1
...
fCutCnt.nextFStepBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(1,2,1)) ) return(FALSE)	# next rebind of  2,1,2 
					if( all(quoSize[1:3+1]==c(2,2,0)) ) return(FALSE)	# next rebind of  1,2,1 
					if( all(quoSize[1:3+2]==c(1,1,1)) ) return(FALSE)	# next rebind of  2,1,0 
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
    flgCnt[!flag] <- flgCnt[!flag] + 1
    ...
    cntMtx[,"raw"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( aZoid[2]%in%c(  6,17 ) ) cnt<-cnt+1
    ...
    cntMtx[,"rawFV"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( fCutU.hasPtn(c( 5,NA, 7,15),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=0 )	# < 6>
					if( fCutU.hasPtn(c( 3, 6),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(    6,19,33,14,22),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=-1 )	# < 8>
					if( fCutU.hasPtn(c( 8,21,11,20,41),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[3,],aZoid,posDiff=1 )	# <11>
					if( fCutU.hasPtn(c(11,NA,38),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[3,],aZoid,posDiff=0 )	# <15>
					if( fCutU.hasPtn(c( 3,NA,15         ),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(    3,15,31,30,30),aZoid,thld=3,fixIdx=2) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=2 )	# <17>
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[4,],aZoid,posDiff=1 )	# <24>
					if( fCutU.hasPtn(c(24,40),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[4,],aZoid,posDiff=0 )	# <25>
					if( fCutU.hasPtn(c( 6, 4,17,25,NA,29),aZoid,thld=3,fixIdx=4) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[3,],aZoid,posDiff=1 )	# <38>
					if( fCutU.hasPtn(c(11,NA,38),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[4,],aZoid,posDiff=0 )	# <39>
					if( fCutU.hasPtn(c(13,23,NA,NA,39),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[3,],aZoid,posDiff=0 )	# <44>
					if( fCutU.hasPtn(c(37,44),aZoid) ) cnt<-cnt+1
    ...
    cntMtx[,"rem"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( fCutU.remFilt(aZoid[2],c( 6,7,3 ),c(  6,17 )) ) cnt<-cnt+1 # 2
					if( fCutU.remFilt(aZoid[3],c( 7     ),c(  7    )) ) cnt<-cnt+1 # 3
					if( fCutU.remFilt(aZoid[4],c( 4     ),c(       )) ) cnt<-cnt+1 # 4
					if( fCutU.remFilt(aZoid[5],c( 8,9,0 ),c( 39    )) ) cnt<-cnt+1 # 5
					if( fCutU.remFilt(aZoid[6],c( 2,6,5 ),c(       )) ) cnt<-cnt+1 # 6
					return( cnt )
				})	;kIdx<-anaFltCnt(cntMtx[,"rem"],rpt)
    cntMtx[,"cStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					cnt <- 0
					...
					if( 1<sum(aCStep[1:3+0]==c( 1, 5, 6)) ) cnt<-cnt+1	# 9
		...
		#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
		#      3 11 24 38 39 44    | 8 13 14  1  5 |                        |1 1 1 2 1 |1 1 1 2 1
		#      5  9 15 19 22 36    | 4  6  4  3 14 |  2  -2  -9 -19 -17  -8 |2 2 1 1 0 |2 2 1 1
		#      2  8 13 25 28 37    | 6  5 12  3  9 | -3  -1  -2   6   6   1 |2 1 2 1 0 |2 1 2 1
		#      8 17 18 24 39 45(1) | 9  1  6 15  6 |  6   9   5  -1  11   8 |1 2 1 1 1 |1 2 1 1 1
		#      5  6 11 17 38 44(1) | 1  5  6 21  6 | -3 -11  -7  -7  -1  -1 |2 2 0 1 1 |2 2 1 1
		#      4  6 15 25 26 33(1) | 2  9 10  1  7 | -1   0   4   8 -12 -11 |2 1 2 1 0 |2 1 2 1
		# -<standard zoid>---------------------------------------------------------------------
...
fCutCnt.nextColVal_1 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+2]==c(2,1,0)) ) return(FALSE)	# next rebind of  1,1,1 
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
    flgCnt[!flag] <- flgCnt[!flag] + 1
    ...
    cntMtx[,"raw"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( aZoid[2]%in%c( 17,16,10 ) ) cnt<-cnt+1
    ...
    cntMtx[,"rawFV"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( fCutU.hasPtn(c(   14,NA,40,45),aZoid) ) cnt<-cnt+1
					...
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[2,],aZoid,posDiff=1 )	# <19>
					if( fCutU.hasPtn(c(19,NA,39),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[4,],aZoid,posDiff=1 )	# <24>
					if( fCutU.hasPtn(c(20,22,23,24),aZoid,thld=3,fixIdx=4) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=-1 )	# <32>
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[5,],aZoid,posDiff=0 )	# <36>
					if( fCutU.hasPtn(c( 1, 9,11, 7,NA,36),aZoid,thld=3,fixIdx=6) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[2,],aZoid,posDiff=1 )	# <39>
					if( fCutU.hasPtn(c(19,NA,39),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(23,24,NA,28,39),aZoid,thld=3,fixIdx=5) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[3,],aZoid,posDiff=0 )	# <41>
					if( fCutU.hasPtn(c(11,NA,NA,NA,NA,41),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(      17,NA,NA,41),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(         22,NA,41),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(   18,NA,NA,29,41),aZoid) ) cnt<-cnt+1
    ...
    cntMtx[,"rem"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					# u0.zoidMtx_ana( stdMI$rawTail%%10 )
					cnt <- 0
					if( fCutU.remFilt(aZoid[1],c( 2    ,9 ),c( 12       )) ) cnt<-cnt+1 # 1
					if( fCutU.remFilt(aZoid[2],c( 7,6,0   ),c( 17,16,10 )) ) cnt<-cnt+1 # 2
					if( fCutU.remFilt(aZoid[3],c( 8,4     ),c( 18,14    )) ) cnt<-cnt+1 # 3
					if( fCutU.remFilt(aZoid[4],c(         ),c(          )) ) cnt<-cnt+1 # 4
					if( fCutU.remFilt(aZoid[5],c( 4,2  ,0 ),c( 20       )) ) cnt<-cnt+1 # 5
    ...
    cntMtx[,"cStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					cnt <- 0
					aCStep <- aZoid[2:6]-aZoid[1:5]
					if( aCStep[1]%in%c( 5, 8, 1 ) ) cnt<-cnt+1
					...
					if( 1<sum(aCStep[1:3+1]==c( 8, 3, 1)) ) cnt<-cnt+1	# 2
		...
		#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
		#      9 12 19 20 39 41    | 3  7  1 19  2 |                        |1 2 1 1 1 |1 2 1 1 1
		#      5 13 17 23 28 36    | 8  4  6  5  8 | -4   1  -2   3 -11  -5 |1 2 2 1 0 |1 2 2 1
		#     12 14 15 24 27 32    | 2  1  9  3  5 |  7   1  -2   1  -1  -4 |0 3 2 1 0 |3 2 1
		#      3 11 14 15 32 36(3) | 8  3  1 17  4 | -9  -3  -1  -9   5   4 |1 3 0 2 0 |1 3 2
		#     10 15 18 21 34 41(1) | 5  3  3 13  7 |  7   4   4   6   2   5 |0 3 1 1 1 |3 1 1 1
		#     11 16 18 19 24 39(1) | 5  2  1  5 15 |  1   1   0  -2 -10  -2 |0 4 1 1 0 |4 1 1
		# -<standard zoid>---------------------------------------------------------------------
		#      6 11 15 17 23 40(1) | 5  4  2  6 17 | -5  -5  -3  -2  -1   1 |1 3 1 0 1 |1 3 1 1
    cntMtx[,"fStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					cnt <- 0
					aFStep <- aZoid - stdMI$lastZoid
					...
					if( 1<sum(aFStep[1:3+0]==c( -3, -1, -9)) ) cnt<-cnt+1 #  1
					if( 1<sum(aFStep[1:3+0]==c( -9,  5,  4)) ) cnt<-cnt+1 #  1
					if( 1<sum(aFStep[1:3+1]==c( -9, -3, -1)) ) cnt<-cnt+1 #  1
					if( 1<sum(aFStep[1:3+1]==c( -1, -9,  5)) ) cnt<-cnt+1 #  1
					if( 1<sum(aFStep[1:3+2]==c( -3, -1, -9)) ) cnt<-cnt+1 # -2
...
fCutCnt.nextColVal_2 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(0,0,3)) ) return(FALSE)	# next rebind of 1,0,1 
					if( all(quoSize[1:3+1]==c(0,0,1)) ) return(FALSE)	# next rebind of 0,1,3 reverse
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
    flgCnt[!flag] <- flgCnt[!flag] + 1
    ...
    cntMtx[,"rawFV"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=0 )	# < 7>
					if( fCutU.hasPtn(c( 7,NA,NA,19,26   ),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c( 7,NA,NA,NA,NA,44),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=0 )	# <10>
					if( fCutU.hasPtn(c( 9,10            ),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(   10,NA,43,41,36),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[2,],aZoid,posDiff=-2 )	# <31>
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[5,],aZoid,posDiff=3 )	# <37>
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[3,],aZoid,posDiff=1 )	# <39>
					if( fCutU.hasPtn(c(25,26,39),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[5,],aZoid,posDiff=1 )	# <40>
					if( fCutU.hasPtn(c(13,NA,34,35,40),aZoid,thld=3,fixIdx=5) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[3,],aZoid,posDiff=0 )	# <41>
					if( fCutU.hasPtn(c( 9,20,31,37,37,41),aZoid,thld=3,fixIdx=6) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=0 )	# <44>
					if( fCutU.hasPtn(c( 7,NA,NA,NA,NA,44),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(19,26,44),aZoid) ) cnt<-cnt+1
    ...
    cntMtx[,"rem"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					# u0.zoidMtx_ana( stdMI$rawTail%%10 )
					cnt <- 0
					if( fCutU.remFilt(aZoid[1],c( 4,5  ),c( 4, 5 )) ) cnt<-cnt+1 # 1
					if( fCutU.remFilt(aZoid[2],c( 2,0,4),c(      )) ) cnt<-cnt+1 # 2
    ...
    cntMtx[,"cStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( 1<sum(aCStep[1:3+1]==c( 3, 7,12)) ) cnt<-cnt+1	# 1
					if( 1<sum(aCStep[1:3+2]==c(17, 9, 1)) ) cnt<-cnt+1	# 7
					if( 1<sum(aCStep[1:3+1]==c( 1, 7, 6)) ) cnt<-cnt+1	# 7
				...
				#      7 37 38 39 40 44(1) |30  1  1  1  4 |  4  33  31  28   9   3 |1 0 0 3 2 |1 3 2
				#      7 10 17 29 33 44(2) | 3  7 12  4 11 |  0 -27 -21 -10  -7   0 |1 2 1 1 1 |1 2 1 1 1
...
fCutCnt.nextColVal_3 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(1,3,1)) ) return(FALSE)	# next rebind of  1,2,1
					if( all(quoSize[1:3+1]==c(3,1,0)) ) return(FALSE)	# next rebind of  2,1,1
					if( all(quoSize[1:3+2]==c(2,1,2)) ) return(FALSE)	# next rebind of  0,1,1
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
    flgCnt[!flag] <- flgCnt[!flag] + 1
    ...
    cntMtx[,"rawFV"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[5,],aZoid,posDiff= 2)	# < 8>
					if( fCutU.hasPtn(c( 8,29,45),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=2 )	# <14>
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[5,],aZoid,posDiff=1 )	# <17>
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=0 )	# <23>
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[4,],aZoid,posDiff=0 )	# <36>
					...
					if( fCutU.hasPtn(c(17,31,42),aZoid) ) cnt<-cnt+1
    ...
    cntMtx[,"rem"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					# u0.zoidMtx_ana( stdMI$rawTail%%10 )
					cnt <- 0
					if( fCutU.remFilt(aZoid[1],c(3,0     ),c(    )) ) cnt<-cnt+1 # 1
					if( fCutU.remFilt(aZoid[2],c(3    ,8 ),c(    )) ) cnt<-cnt+1 # 2
					if( fCutU.remFilt(aZoid[3],c(5,7,2   ),c( 15 )) ) cnt<-cnt+1 # 3
					if( fCutU.remFilt(aZoid[4],c(      3 ),c( 13 )) ) cnt<-cnt+1 # 4
					if( fCutU.remFilt(aZoid[5],c(8,2,4,7 ),c(    )) ) cnt<-cnt+1 # 5
					if( fCutU.remFilt(aZoid[6],c(7,4,1   ),c( 41 )) ) cnt<-cnt+1 # 6
					return( cnt )
				})	;kIdx<-anaFltCnt(cntMtx[,"rem"],rpt)
    cntMtx[,"cStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					cnt <- 0
					...
					if( 1<sum(aCStep[1:3+0]==c( 3, 4,15)) ) cnt<-cnt+1	# 8
					if( 1<sum(aCStep[1:3+ 0==c( 1, 1, 1)) ) cnt<-cnt+1	# 4
					if( 1<sum(aCStep[1:3+2]==c( 3, 4,15)) ) cnt<-cnt+1	# 8
					if( 1<sum(aCStep[1:3+2]==c( 1,21, 7)) ) cnt<-cnt+1	#10
					if( 1<sum(aCStep[1:2+3]==c( 1, 1   )) ) cnt<-cnt+1	# 4
			...
			#     Raw value(reb)       |cStep          |fStep                   |QuoSize   |QuoTbl 
			#     12 25 29 35 42 43    |13  4  6  7  1 |                        |0 1 2 1 2 |1 2 1 2
			#      8 17 21 24 27 31    | 9  4  3  3  4 | -4  -8  -8 -11 -15 -12 |1 1 3 1 0 |1 1 3 1
			#      5 13 17 23 28 36(1) | 8  4  6  5  8 | -3  -4  -4  -1   1   5 |1 2 2 1 0 |1 2 2 1
    ...
    cntMtx[,"fStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( 1<sum(aFStep[1:3+0]==c( -5   6   9)) ) cnt<-cnt+1 # -5
...
fCutCnt.nextColVal_4 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(1,2,2)) ) return(FALSE)	# next rebind of 1,2,1 
					if( all(quoSize[1:3+2]==c(0,0,2)) ) return(FALSE)	# next rebind of 1,2,0 REVERSE
					if( all(quoSize[1:3+1]==c(1,2,1)) ) return(FALSE)	# next rebind of 0,3,2 reverse
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
    flgCnt[!flag] <- flgCnt[!flag] + 1
    ...
    cntMtx[,"raw"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( aZoid[5]%in%c(          ) ) cnt<-cnt+1
					if( aZoid[6]%in%c( 36,41,33 ) ) cnt<-cnt+1
					return( cnt )
				})	;kIdx<-anaFltCnt(cntMtx[,"raw"],rpt)
	neighborObj <- fCutU.neighborObj( stdMI$rawTail )
    cntMtx[,"rawFV"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[3,],aZoid,posDiff=-1 )	# < 2>
					if( fCutU.hasPtn(c( 2,14,12,25,22),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[4,],aZoid,posDiff=0 )	# < 5>
					if( fCutU.hasPtn(c( 5,NA,NA,24      ),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c( 5,19,14,NA,19,30),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[3,],aZoid,posDiff=-1 )	# < 7>
					if( fCutU.hasPtn(c( 7,24,26,25,26),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=-2 )	# <19>
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[5,],aZoid,posDiff=1 )	# <20>
					if( fCutU.hasPtn(c(19,NA,20,NA,45),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=2 )	# <22>
					if( fCutU.hasPtn(c(21,22,40,41),aZoid,thld=3,fixIdx=2) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=0 )	# <36>
    ...
    cntMtx[,"rem"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( fCutU.remFilt(aZoid[2],c(2,7  ,1     ),c(          )) ) cnt<-cnt+1 # 2
					if( fCutU.remFilt(aZoid[3],c(4,7  ,0     ),c(          )) ) cnt<-cnt+1 # 3
					if( fCutU.remFilt(aZoid[4],c(3,2,4       ),c( 24       )) ) cnt<-cnt+1 # 4
					if( fCutU.remFilt(aZoid[5],c(9           ),c(          )) ) cnt<-cnt+1 # 5
					if( fCutU.remFilt(aZoid[6],c(6,1  ,3,0,7 ),c( 36,41,33 )) ) cnt<-cnt+1 # 6
					return( cnt )
				})	;kIdx<-anaFltCnt(cntMtx[,"rem"],rpt)
    cntMtx[,"cStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					cnt <- 0
					...
					if( aCStep[4]%in%c(      ) ) cnt<-cnt+1
					if( aCStep[5]%in%c( 2, 5 ) ) cnt<-cnt+1

					if( 1<sum(aCStep[1:3+0]==c(15, 2, 7)) ) cnt<-cnt+1	# 8
					if( 1<sum(aCStep[1:3+0]==c(15, 2, 7)) ) cnt<-cnt+1	# 4
					if( 1<sum(aCStep[1:3+2]==c(15, 2, 7)) ) cnt<-cnt+1	# 8
					if( 1<sum(aCStep[1:3+0]==c( 7, 3, 2)) ) cnt<-cnt+1	# 8
					if( 1<sum(aCStep[1:3+2]==c( 7, 1,19)) ) cnt<-cnt+1	#10
					if( 1<sum(aCStep[1:2+3]==c(15, 2   )) ) cnt<-cnt+1	# 4
			...
			#      1  2  6 16 19 42(1) | 1  4 10  3 23 | -4  -5 -14  -6 -18   0 |3 2 0 0 1 |3 2 1
    ...
    cntMtx[,"fStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( 1<sum(aFStep[1:3+0]==c( 10, 13,  4)) ) cnt<-cnt+1 # -5
					if( 1<sum(aFStep[1:3+1]==c( -4,  1, -2)) ) cnt<-cnt+1 #-10
...
fCutCnt.nextColVal_5 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(3,1,1)) ) return(FALSE)	# next rebind of  2,1,1 
					if( all(quoSize[1:3+1]==c(1,1,1)) ) return(FALSE)	# next rebind of  1,1,1
    ...
    cntMtx[,"raw"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( aZoid[5]%in%c( 34,33 ) ) cnt<-cnt+1
					if( aZoid[6]%in%c( 40,43 ) ) cnt<-cnt+1
					return( cnt )
				})	;kIdx<-anaFltCnt(cntMtx[,"raw"],rpt)
	neighborObj <- fCutU.neighborObj( stdMI$rawTail )
    cntMtx[,"rawFV"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[5,],aZoid,posDiff=0 )	# <13>
					...
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[4,],aZoid,posDiff=0 )	# <23>
					if( fCutU.hasPtn(c( 1,NA,NA,23      ),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(         23,NA,42),aZoid) ) cnt<-cnt+1
					if( fCutU.hasPtn(c(    7, 3,23,44   ),aZoid,thld=3,fixIdx=3) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[5,],aZoid,posDiff=0 )	# <34>
					...
					if( fCutU.hasPtn(c(15,20,23,15,31,41),aZoid,thld=3,fixIdx6) ) cnt<-cnt+1
    ...
    cntMtx[,"rem"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					# u0.zoidMtx_ana( stdMI$rawTail%%10 )
					cnt <- 0
					if( fCutU.remFilt(aZoid[1],c(2,0   ),c( 10    )) ) cnt<-cnt+1 # 1
					if( fCutU.remFilt(aZoid[2],c(    3 ),c(       )) ) cnt<-cnt+1 # 2
					if( fCutU.remFilt(aZoid[3],c(    2 ),c(  8    )) ) cnt<-cnt+1 # 3
					if( fCutU.remFilt(aZoid[4],c(1  ,2 ),c( 21    )) ) cnt<-cnt+1 # 4
					if( fCutU.remFilt(aZoid[5],c(4,3   ),c( 34,33 )) ) cnt<-cnt+1 # 5
					if( fCutU.remFilt(aZoid[6],c(0,3,1 ),c( 40,43 )) ) cnt<-cnt+1 # 6
					return( cnt )
				})	;kIdx<-anaFltCnt(cntMtx[,"rem"],rpt)
    cntMtx[,"cStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					cnt <- 0
					aCStep <- aZoid[2:6]-aZoid[1:5]
					if( aCStep[1]%in%c( 8, 3,10    ) ) cnt<-cnt+1
					if( aCStep[2]%in%c( 3,10, 2,11 ) ) cnt<-cnt+1
					if( aCStep[3]%in%c(13, 4       ) ) cnt<-cnt+1
					if( aCStep[4]%in%c(10          ) ) cnt<-cnt+1
					if( aCStep[5]%in%c(10,16       ) ) cnt<-cnt+1

					if( 1<sum(aCStep[1:3+0]==c( 8  3 11)) ) cnt<-cnt+1	# 3
					if( 1<sum(aCStep[1:3+0]==c( 3 11 16)) ) cnt<-cnt+1	# 3
					if( 1<sum(aCStep[1:3+1]==c(14 10  4)) ) cnt<-cnt+1	#11
					if( 1<sum(aCStep[1:3+2]==c( 3 13  7)) ) cnt<-cnt+1	# 4
			...
			#      2  6 13 16 29 30    | 4  7  3 13  1 |  1   2   5  -7  -4 -12 |2 2 1 1 0 |2 2 1 1
    ...
    cntMtx[,"fStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( aFStep[2]%in%c(  5, -6  ) ) cnt<-cnt+1
					if( aFStep[3]%in%c(         ) ) cnt<-cnt+1
					if( aFStep[4]%in%c(         ) ) cnt<-cnt+1
					if( aFStep[5]%in%c(         ) ) cnt<-cnt+1
					if( aFStep[6]%in%c(         ) ) cnt<-cnt+1

					if( 1<sum(aFStep[1:3+0]==c( -6,  2,  5)) ) cnt<-cnt+1 # -6
					if( 1<sum(aFStep[1:3+1]==c( -6,  2,  5)) ) cnt<-cnt+1 # -6
					if( 1<sum(aFStep[1:3+2]==c( -1,  5,  8)) ) cnt<-cnt+1 #  2
...
fCutCnt.nextColVal_6 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    cntMtx[,"rawFV"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( fCutU.hasPtn(c( 6,NA,NA,20   ),aZoid) ) cnt<-cnt+1
					...
					if( fCutU.hasPtn(c( 2,NA,18,NA,31),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=-1 )	# <24>
					if( fCutU.hasPtn(c( 7,NA,24),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[5,],aZoid,posDiff=3 )	# <30>
					if( fCutU.hasPtn(c(29,30,42),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[6,],aZoid,posDiff=-2 )	# <34>
					if( fCutU.hasPtn(c(29,34),aZoid) ) cnt<-cnt+1
					cnt <- cnt + fCutU.spanMatch(stdMI$rawTail[3,],aZoid,posDiff=0 )	# <36>
					if( fCutU.hasPtn(c( 2,21,30,36),aZoid) ) cnt<-cnt+1
    ...
    cntMtx[,"rem"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					# u0.zoidMtx_ana( stdMI$rawTail%%10 )
					cnt <- 0
					if( fCutU.remFilt(aZoid[1],c( 1,7,0 ),c(  1,10 )) ) cnt<-cnt+1 # 1
					if( fCutU.remFilt(aZoid[2],c( 0     ),c( 30    )) ) cnt<-cnt+1 # 2
					if( fCutU.remFilt(aZoid[3],c(       ),c(       )) ) cnt<-cnt+1 # 3
					if( fCutU.remFilt(aZoid[4],c(       ),c(       )) ) cnt<-cnt+1 # 4
					if( fCutU.remFilt(aZoid[5],c( 0,7   ),c( 34    )) ) cnt<-cnt+1 # 5
					if( fCutU.remFilt(aZoid[6],c(       ),c(       )) ) cnt<-cnt+1 # 6
					return( cnt )
				})	;kIdx<-anaFltCnt(cntMtx[,"rem"],rpt)
    cntMtx[,"cStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					cnt <- 0
					aCStep <- aZoid[2:6]-aZoid[1:5]
					if( aCStep[1]%in%c(  5   , 2 ) ) cnt<-cnt+1
					if( aCStep[2]%in%c( 11       ) ) cnt<-cnt+1
					if( aCStep[3]%in%c(  1,13    ) ) cnt<-cnt+1
					if( aCStep[4]%in%c(          ) ) cnt<-cnt+1
					if( aCStep[5]%in%c(  4       ) ) cnt<-cnt+1

					if( 1<sum(aCStep[1:3+0]==c( 1,11, 8)) ) cnt<-cnt+1	# 3
					if( 1<sum(aCStep[1:3+1]==c( 2, 7, 3)) ) cnt<-cnt+1	#13
					if( 1<sum(aCStep[1:3+2]==c( 7, 1, 3)) ) cnt<-cnt+1	# 4
				...
				#      7 22 24 31 34 36(2) |15  2  7  3  2 |  3  15  13   7  -8  -9 |1 0 2 3 0 |1 2 3
				#      9 30 34 35 39 41(1) |21  4  1  4  2 |  2   8  10   4   5   5 |1 0 0 4 1 |1 4 1
				#      6 10 17 18 21 29    | 4  7  1  3  8 | -3 -20 -17 -17 -18 -12 |1 3 2 0 0 |1 3 2
				#      6  7 18 19 30 38(2) | 1 11  1 11  8 |  0  -3   1   1   9   9 |2 2 0 2 0 |2 2 2
				#      1 10 13 26 32 36    | 9  3 13  6  4 | -5   3  -5   7   2  -2 |1 2 1 2 0 |1 2 1 2
				# -<standard zoid>---------------------------------------------------------------------
				#      6 11 15 17 23 40    | 5  4  2  6 17 |  5   1   2  -9  -9   4 |1 3 1 0 1 |1 3 1 1
    cntMtx[,"fStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					cnt <- 0
					...
					if( 1<sum(aFStep[1:3+0]==c(  5, -3,  5)) ) cnt<-cnt+1 #  3
					if( 1<sum(aFStep[1:3+1]==c( 17, 18, 12)) ) cnt<-cnt+1 # -5
					if( 1<sum(aFStep[1:3+2]==c( 10,  4,  5)) ) cnt<-cnt+1 #  7
					if( 1<sum(aFStep[1:2+4]==c( -3,-20    )) ) cnt<-cnt+1 #  2

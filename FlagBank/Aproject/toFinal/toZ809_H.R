...
fCutCnt.basic <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[5]%in%c( 1 ,13, 3, 2 ) ) cnt<-cnt+1
...
fCutCnt.nextQuo10 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[2]%in%c( 3, 9,11 ) ) cnt<-cnt+1
			if( aCStep[3]%in%c(         ) ) cnt<-cnt+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			cnt.w2 <- 0
			if( (aFStep[1]==aFStep[2])&&(aFStep[3]==aFStep[5]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c(         ) ) cnt<-cnt+1
			if( aFStep[2]%in%c(  3, -2  ) ) cnt<-cnt+1
			if( aFStep[3]%in%c(  1      ) ) cnt<-cnt+1
			if( aFStep[4]%in%c( 18      ) ) cnt<-cnt+1
			if( aFStep[5]%in%c( 39,  6  ) ) cnt<-cnt+1
			if( aFStep[6]%in%c(         ) ) cnt<-cnt+1
...
fCutCnt.nextRebNum <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			...
			if( fCutU.remFilt(aZoid[6],c( 6,5     ),c( 36,36,45 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			...
			if( aCStep[3]%in%c(            ) ) cnt<-cnt+1
			if( aCStep[4]%in%c( 7,11,13,16 ) ) cnt<-cnt+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			if( all(aFStep[c(1,6)]==c(0,0)) )	cnt.w1<-cnt.w1+1

			cnt.w2 <- 0
			if( (aFStep[1]==aFStep[6]) && (aFStep[2]==aFStep[5]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextCStepBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			...
			if( fCutU.remFilt(aZoid[3],c( 1,5,4 ),c( 11       )) ) cnt<-cnt+1 # 3
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[4]%in%c( 15, 4,11 ) ) cnt<-cnt+1
			if( aCStep[5]%in%c(  5,11    ) ) cnt<-cnt+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c( -1    ) ) cnt<-cnt+1
			if( aFStep[2]%in%c( -1,  2    ) ) cnt<-cnt+1
			...
			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			cnt.w2 <- 0
			if( (aFStep[1]==aFStep[3]) && (aFStep[4]==aFStep[6]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextFStepBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[2]%in%c( 5,13    ) ) cnt<-cnt+1
			if( aCStep[3]%in%c(10       ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(         ) ) cnt<-cnt+1
			if( aCStep[5]%in%c( 8, 7, 6 ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]
			if( all(aCStep[1:2+0]==c(3,9)) )	cnt.w1<-cnt.w1+1
...
fCutCnt.nextColVal_1 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[3]%in%c( 1, 9     ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(          ) ) cnt<-cnt+1
			...
			cnt.w2 <- 0
			if( (aFStep[1]==aFStep[2])&&(aFStep[4]==aFStep[6]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_2 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c( 3,13 ) ) cnt<-cnt+1
...
fCutCnt.nextColVal_3 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w2 <- 0
			if( (aCStep[1]==aCStep[3]) && (aCStep[2]==aCStep[5]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_4 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[4]%in%c(         ) ) cnt<-cnt+1
			if( aCStep[5]%in%c( 2, 5,13 ) ) cnt<-cnt+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[6]%in%c(  0, -5,  1, 10 ) ) cnt<-cnt+1
...
fCutCnt.nextColVal_5 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[3]%in%c( 4,13     ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(10,19     ) ) cnt<-cnt+1
			...
			if( all(aCStep[1:2+2]==c(14,10)) )	cnt.w1<-cnt.w1+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[2]%in%c(  5, -6,  3 ) ) cnt<-cnt+1
			...
			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			cnt.w2 <- 0
			if( (aFStep[2]==aFStep[3]) && (aFStep[4]==aFStep[6]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_6 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[2]==aCStep[4] )				cnt.w1<-cnt.w1+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			if( aFStep[1]==aFStep[2] )	cnt.w1<-cnt.w1+1
			if( aFStep[5]==aFStep[6] )	cnt.w1<-cnt.w1+1

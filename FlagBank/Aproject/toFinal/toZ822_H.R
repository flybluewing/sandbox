...
fCutCnt.basic <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( all(aCStep[1:2+2]==c( 1,11 )) )	cnt.w1<-cnt.w1+1

			cnt.w2 <- 0
			if( all(aCStep[c(3,5)]== (aCStep[c(2,4)]+10) ) )	cnt.w2<-cnt.w2+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w2 <- 0
			if( (aFStep[1]==aFStep[2]) && (aFStep[1]==aFStep[3]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.colValSeqNext <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					cnt <- 0
					if( 1<sum(aZoid[c(1,2,3  ,5,6)]==c( 3,17,26,   41,45)) ) cnt<-cnt+1
					if( 1<sum(aZoid[c(1,2,3  ,5,6)]==c( 1, 8,24,   36,43)) ) cnt<-cnt+1
					if( 1<sum(aZoid[c(1,2,3  ,5,6)]==c(25, 9,18,   20,44)) ) cnt<-cnt+1
					if( 1<sum(aZoid[c(1,2    ,5,6)]==c( 9,20,      35,38)) ) cnt<-cnt+1
					if( 1<sum(aZoid[c(1,      5,6)]==c( 9,         15,31)) ) cnt<-cnt+1
					if( 1<sum(aZoid[c(1,        6)]==c( 9,            38)) ) cnt<-cnt+1
    ...
    fltCnt <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( all(aZoid[1:2+1]==c(25,27)) ) cnt<-cnt+1
					if( all(aZoid[1:2+1]==c(40,44)) ) cnt<-cnt+1

					remCnt <- 0
						if( fCutU.remFilt(aZoid[1],c( 1, 9),c( 1 )) )	remCnt <- remCnt+1
						if( fCutU.remFilt(aZoid[2],c( 8, 1),c(   )) )	remCnt <- remCnt+1
						if( fCutU.remFilt(aZoid[3],c(     ),c(   )) )	remCnt <- remCnt+1
						if( fCutU.remFilt(aZoid[4],c( 9   ),c(   )) )	remCnt <- remCnt+1
						if( fCutU.remFilt(aZoid[5],c( 1, 5),c(   )) )	remCnt <- remCnt+1
    ...
    fltCnt <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					#	[1] 27   7   2  12  11 
					#	[2]  3   3   4  13  22   4   5   3  26   1   5   8 
					#	[3] 12   8 
					#	[4]  2  14   5  11 
					#	[5]  3   2   2   2   2   4 
					aCStep <- aZoid[2:6]-aZoid[1:5]

					score <- sum(aCStep==c( 27, 3,12, 2, 3),na.rm=T)

					matCnt <- sum(aCStep==c( 7, 3, 8,14, 2),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					matCnt <- sum(aCStep==c( 2, 4,NA, 5, 2),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					matCnt <- sum(aCStep==c(12,13,NA,11, 2),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					matCnt <- sum(aCStep==c(11,22,NA,NA, 2),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )

					cnt <- 0
						if( aCStep[1]%in%c(         ) ) cnt<-cnt+1
						if( aCStep[2]%in%c( 3, 4,26 ) ) cnt<-cnt+1
...
fCutCnt.colValSeqNext.cStep <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					tCnt <- 0
						if( aCStep[1]%in%c( 5,15,12 ) ) tCnt<-tCnt+1	# 마지막 값 연속도 포함.
						if( aCStep[2]%in%c( 3       ) ) tCnt<-tCnt+1
						if( aCStep[3]%in%c( 1, 3    ) ) tCnt<-tCnt+1
						if( aCStep[4]%in%c( 2, 3    ) ) tCnt<-tCnt+1
						if( aCStep[5]%in%c(         ) ) tCnt<-tCnt+1
					cnt <- cnt + ifelse( tCnt>1 ,tCnt-1 ,0 )

					# [1]*   5  5 15  5  5 12
					# [2]*   2  1 11 10  6  3 12 10  2 14  4  2 10  8 15  1  1  7  3  7  9 11  9 16 15  2 10  2  6  1  2  2  3  3 15  4 27  6  7  3 11 20  2  2  1  7  6  9  2  4  2 10  3  6  9  1 15
					# [3]    2  3 14  5  6
					# [4]*   3  1  4  8 13  1  3  2  6 11  1  8  6  7 11  3  6  1  4  1  3  4  4  2  1  6  4
					# [5]    9  3  4
					matCnt<-sum(aCStep==c( 5, 2, 2, 3, 9),na.rm=T)	;cnt<-cnt+ifelse(matCnt>2,1,0)
					matCnt<-sum(aCStep==c( 5, 1, 3, 1, 3),na.rm=T)	;cnt<-cnt+ifelse(matCnt>2,1,0)
					matCnt<-sum(aCStep==c(15,11,14, 4, 4),na.rm=T)	;cnt<-cnt+ifelse(matCnt>2,1,0)
					matCnt<-sum(aCStep==c( 5,10, 5, 8,NA),na.rm=T)	;cnt<-cnt+ifelse(matCnt>2,1,0)
					matCnt<-sum(aCStep==c( 5, 6, 6,13,NA),na.rm=T)	;cnt<-cnt+ifelse(matCnt>2,1,0)
					matCnt<-sum(aCStep==c(12, 3,NA, 1,NA),na.rm=T)	;cnt<-cnt+ifelse(matCnt>2,1,0)
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					# [  3] 16  4     5  5     2  2     9 11
					# [  4] 18  3     3  4     1  2     3 15
					# [  5]  6  2     9  5    11 19    16 14
					# [  6]  5 14     2  4    16  1    14  2
					if( aCStep[1]%in%c(    ) ) cnt<-cnt+1
					...
					if( all(aCStep[1:2+2]==c( 3, 2)) ) cnt<-cnt+1
...
fCutCnt.nextZW <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[5]%in%c(  5,12, 1 ) ) cnt<-cnt+1
    ...
    cntMtx[,"fStep"] <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( aFStep[2]==sum(aFStep[c(1,4)]) ) cnt<-cnt+1
					if( aFStep[3]==sum(aFStep[c(1,5,6)]) ) cnt<-cnt+1
...
fCutCnt.nextBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w2 <- 0
			if( all(aCStep[c(1,5)]== aCStep[2]*c(2,2)) )	cnt.w2<-cnt.w2+1
			if( sum(aCStep[c(3,5)])==sum(aCStep[c(1,2)]) )	cnt.w2<-cnt.w2+1
			if( sum(aCStep[c(3,5)])==sum(aCStep[c(4,2)]) )	cnt.w2<-cnt.w2+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( (aFStep[1]==aFStep[3]) && (aFStep[1]==aFStep[4]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextRebNum <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[6]%in%c(  -8, 17 ) ) cnt<-cnt+1
...
fCutCnt.nextCStepBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w2 <- 0
			if( all(aCStep[c(1,4)]== aCStep[2]*c(2,2) ) )	cnt.w2<-cnt.w2+1
			if( sum(aCStep[c(3,5)])==sum(aCStep[c(1,2)]) )	cnt.w2<-cnt.w2+1
			if( sum(aCStep[c(3,5)])==sum(aCStep[c(4,2)]) )	cnt.w2<-cnt.w2+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[3]==sum(aFStep[c(2,5)]) )	cnt.w2<-cnt.w2+1
			if( aFStep[4]==sum(aFStep[c(2,5)]) )	cnt.w2<-cnt.w2+1
			if( sum(aFStep[c(1,2,4)])==sum(aFStep[c(3,4)]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextFStepBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( all(aFStep[3:4]== aFStep[1]*c(1,3) ) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_1 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[5]%in%c( 3, 2, 9 ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]
			if( fCutU.hasPtn(c( 4, 1 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( all(aCStep[1:2+1]==c( 7, 1)) )	cnt.w1<-cnt.w1+1
			...
			if( all(aCStep[1:2+2]==c( 9, 6)) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c(  1, 3 ),aCStep) )	cnt.w1<-cnt.w1+1 # -
			if( fCutU.hasPtn(c(  3,10 ),aCStep) )	cnt.w1<-cnt.w1+1
			# -------------------------------------------------------------------------------------
			#     FV :    1 (7)   2 (2)   3 (2)   4 (4)   6 (2)   7 (3)   8 (2)   9 (3) 
			cnt.w2 <- 0
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			if( fCutU.hasPtn(c( -20,-12 ),aFStep) )	cnt.w1<-cnt.w1+1	# -
			# -------------------------------------------------------------------------------------
			#     FV :    -17 (2)   -12 (3)   -3 (2)   -1 (2)   12 (3)   14 (2)   18 (2) 

			cnt.w2 <- 0
			if( sum(aFStep[c(1,3)])==sum(aFStep[c(4,6)]) )	cnt.w2<-cnt.w2+1	# 29( 9)
			if( sum(aFStep[c(1,3)])==sum(aFStep[c(2,5)]) )	cnt.w2<-cnt.w2+1	# 29(10)
			if( sum(aFStep[c(4,6)])==sum(aFStep[c(2,5)]) )	cnt.w2<-cnt.w2+1	# 29
...
fCutCnt.nextColVal_2 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[4]%in%c( 6, 1, 5 ) ) cnt<-cnt+1
			...
			# -<standard zoid>---------------------------------------------------------------------
			#      9 18 20 24 27 36(1) | 9  2  4  3  9 | -5  -2  -3  -7 -10  -2 |1 1 3 1 0 |1 1 3 1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			# -------------------------------------------------------------------------------------
			#     FV :    -7 (2)   -6 (2)   -4 (2)   -2 (2)   4 (2)   6 (2)   9 (2)   10 (2)   14 (2) 
			cnt.w2 <- 0
			...
			if( sum(aFStep[c(1,3)])==sum(aFStep[c(4,6)]) )	cnt.w2<-cnt.w2+1	# 3
			if( sum(aFStep[c(1,3)])==sum(aFStep[c(5,6)]) )	cnt.w2<-cnt.w2+1	# 3
...
fCutCnt.nextColVal_3 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[3]%in%c(            ) ) cnt<-cnt+1
			if( aCStep[4]%in%c( 1          ) ) cnt<-cnt+1
			if( aCStep[5]%in%c( 1,11, 9,22 ) ) cnt<-cnt+1
...
fCutCnt.nextColVal_4 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[3]%in%c( 2,10, 8 ) ) cnt<-cnt+1
			if( aCStep[4]%in%c( 6,10    ) ) cnt<-cnt+1
			if( aCStep[5]%in%c( 3       ) ) cnt<-cnt+1
...
fCutCnt.nextColVal_5 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c( 13, 1    ) ) cnt<-cnt+1
			if( aCStep[2]%in%c(          ) ) cnt<-cnt+1
			if( aCStep[3]%in%c(          ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(  3, 1, 4 ) ) cnt<-cnt+1
			...
			# -------------------------------------------------------------------------------------
			#     FV :    1 (2)   2 (4)   3 (3)   5 (6)   6 (4)   8 (2)   9 (2)   10 (2)   14(2) 
			cnt.w2 <- 0
			if( 1<sum( aCStep[4:5]*c(4,1)==aCStep[2:3] ) )	cnt.w2<-cnt.w2+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[3]==(aFStep[4]*sum(aFStep[5:6]) ) )	cnt.w2<-cnt.w2+1

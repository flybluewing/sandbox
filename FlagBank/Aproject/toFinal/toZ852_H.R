...
fCutCnt.basic <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( all(quoSize[1:3+0]==c(2,1,1)) ) return(FALSE)	# next rebind of 0,2,2
					if( all(quoSize[1:3+1]==c(1,1,2)) ) return(FALSE)	# next rebind of 2,2,1
					if( all(quoSize[1:3+2]==c(1,2,1)) ) return(FALSE)	# next rebind of 2,1,1
					if( all(quoSize[1:3+0]==c(1,2,0)) ) return(FALSE)	# next rebind of 0,1,3
					# unique (0 2 2 1 1) 중복 (2 1 1 2 0)재발 가능?
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			...
			if( aZoid[6]%in%c( 39,45,44  ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			...
			if( fCutU.remFilt(aZoid[2],c(  7       ),c( 16       )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c(  6,9     ),c(          )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c(          ),c(          )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c(  5       ),c(          )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c(  9,5,4   ),c( 39,45,44 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			#	unique	( 9, 2, 2 ) ( 4, 6,18) ( 4,NA, 3, 5) (14, 6,12)
			# -------------------------------------------------------------------------------------
			...
			#      5 18 30 41 43 45    |13 12 11  2  2 |                        |1 1 0 1 3 |1 1 1 3
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c(   6,-2 ) ) cnt<-cnt+1
...
fCutCnt.colValSeqNext <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					cnt <- 0
					# [1]    7  8 12
					# [2]*  24 14 19 11 17 18 18
					# [3]*  25  6 13 17 27 15 15
					# [4]   32 29
					# [5]   34 18 38 12 34 43 45 19
					# [6]*  39 28 44 26 45 30 43 43 44 44

					if( 1<sum(aZoid==c(  7,24,25,32,34,39 ) ,na.rm=T) ) cnt<-cnt+1
					if( 1<sum(aZoid==c(  8,14, 6,29,18,28 ) ,na.rm=T) ) cnt<-cnt+1
					if( 1<sum(aZoid==c( 12,19,13,NA,38,44 ) ,na.rm=T) ) cnt<-cnt+1
					if( 1<sum(aZoid==c( 11,17,NA,NA,12,26 ) ,na.rm=T) ) cnt<-cnt+1
					if( 1<sum(aZoid==c( 17,27,NA,NA,34,45 ) ,na.rm=T) ) cnt<-cnt+1
					if( 1<sum(aZoid==c( 18,15,NA,NA,43,30 ) ,na.rm=T) ) cnt<-cnt+1
					if( 1<sum(aZoid==c( 18,15,NA,NA,45,43 ) ,na.rm=T) ) cnt<-cnt+1
					if( 1<sum(aZoid==c( NA,NA,NA,NA,19,43 ) ,na.rm=T) ) cnt<-cnt+1
					# if( 1<sum(aZoid==c( NA,NA,NA,NA,NA,NA ) ,na.rm=T) ) cnt<-cnt+1

					# if( fCutU.hasPtn(c(  ,  ),aZoid) ) cnt<-cnt+1

					score <- 0
					if( aZoid[1]%in%c(  6      ) ) score<-score+1
					if( aZoid[2]%in%c( 20      ) ) score<-score+1
    ...
    fltCnt <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( aZoid[3]%in%c( 25,27,31,33 ) ) cnt<-cnt+1
					if( aZoid[4]%in%c(             ) ) cnt<-cnt+1	# 
					if( aZoid[5]%in%c( 36          ) ) cnt<-cnt+1
					if( aZoid[6]%in%c(             ) ) cnt<-cnt+1

					if( all(aZoid[c(3,5)]==c(33,36)) ) cnt<-cnt+1	# unique

					# [  1] 13 16    24 26     4 12    40 41    33 34
					# [  2] 17 19    16 27    22 37    36 37    24 25
					# [  3] 20 33    20 32    24 33    32 34    35 37
					# [  4] 37 38    23 27    37 43    17 23    38 40
					# [  5] 13 29    17 31                      32 44
					# [  6] 27 42    29 31                      34 45
					# [  7]  1  8    18 30                      23 38
					# [  8]  8 19    26 30                      11 23
					...
					remCnt <- 0
						if( fCutU.remFilt(aZoid[1],c( 7,8     ),c(             )) )	remCnt <- remCnt+1
						if( fCutU.remFilt(aZoid[2],c( 0,4     ),c(             )) )	remCnt <- remCnt+1
						if( fCutU.remFilt(aZoid[3],c( 5,7,1,3 ),c( 25,27,31,33 )) )	remCnt <- remCnt+1
						if( fCutU.remFilt(aZoid[4],c(         ),c(             )) )	remCnt <- remCnt+1
						if( fCutU.remFilt(aZoid[5],c( 2,1,6   ),c( 36          )) )	remCnt <- remCnt+1
						if( fCutU.remFilt(aZoid[6],c( 3       ),c(             )) )	remCnt <- remCnt+1
						...
						# grp (1:2+0)
							...
							if( aZoid[2]==19   && fCutU.remFilt(aZoid[1],c(6),c( )) ) remCnt <- remCnt+1
						# grp (1:2+1)
						# grp (1:2+2)
						# grp (1:2+3)
						# grp (1:2+4)
    ...
    fltCnt <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					matCnt <- sum(aCStep==c(  2,11,15, 1, 1 ),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					matCnt <- sum(aCStep==c( 13,12, 9, 2, 2 ),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					matCnt <- sum(aCStep==c(  1, 4, 6, 6, 2 ),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					matCnt <- sum(aCStep==c( 16,14,NA,NA,12 ),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					matCnt <- sum(aCStep==c( 15, 2,NA,NA,11 ),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					matCnt <- sum(aCStep==c(  7,12,NA,NA,15 ),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					matCnt <- sum(aCStep==c( 11, 4,NA,NA,12 ),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					matCnt <- sum(aCStep==c(  1,10,NA,NA, 5 ),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					matCnt <- sum(aCStep==c( NA, 5,NA,NA, 1 ),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					matCnt <- sum(aCStep==c( NA, 4,NA,NA, 6 ),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					matCnt <- sum(aCStep==c( NA, 3,NA,NA, 2 ),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					matCnt <- sum(aCStep==c( NA, 9,NA,NA, 8 ),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					matCnt <- sum(aCStep==c( NA, 2,NA,NA,11 ),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					matCnt <- sum(aCStep==c( NA, 1,NA,NA, 4 ),na.rm=T)
					score <- score + ifelse( matCnt>1 ,matCnt-1 ,0 )
					#	matCnt <- sum(aCStep==c( NA,NA,NA,NA,NA ),na.rm=T)

					#	[1]  3   2  13   1  16  15   7  11   1 
					#	[2]  2  11  12   4  14   2  12   4  10   5   4   3   9   2   1 
					#	[3]  8  15   9   6 
					#	[4]  1   1   2   6 
					#	[5]  1   1   2   2  12  11  15  12   5   1   6   2   8  11   4   3 

					cnt <- 0
						# if( fCutU.hasPtn(c(   ,   ),aCStep) ) cnt<-cnt+1
						# if( 1<sum(aCStep[1:2+ ]==c(   ,   )) )	cnt<-cnt+1
						if( 1<sum(aCStep[1:2+3]==c(  1, 1 )) )	cnt<-cnt+1	# unique
						if( 1<sum(aCStep[1:2+3]==c(  2, 2 )) )	cnt<-cnt+1	# unique

						if( aCStep[1]%in%c(  3, 4, 2 ) ) cnt<-cnt+1
						if( aCStep[2]%in%c(          ) ) cnt<-cnt+1
						if( aCStep[3]%in%c(  2, 1    ) ) cnt<-cnt+1
						if( aCStep[4]%in%c(  1, 2    ) ) cnt<-cnt+1
						if( aCStep[5]%in%c(  1, 2    ) ) cnt<-cnt+1

						if( 1<sum( aCStep[c(2,4)]*c(4,1)==aCStep[c(3,5)] ) )	cnt<-cnt+1	# unique
...
fCutCnt.colValSeqNext.cStep <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					# [1]*   2  7  1  5 12  1  2  1  4  4 14  8  2  7  7 14  8  3  4  3  3  5 14
					# [2]*   8 13 13  6  6  4  6 11  5 20  4  6  4  6  5  3  1 11  5  2  4 15  8  5  9  7...
					# [3]    2  3  5  1  9 15  2  4
					# [4]*  11  7  4  5 19  7  8  9 14  7  8  2  1  9  2  2 10  5  1 16  3  3  7  3  3
					# [5]*   8 11 13  2  1  8  6  2  4  8  4  1  6  3  1  8  2 14  1  8 25  6  1  7 26  5...

					tCnt <- 0
						if( aCStep[1]%in%c(  1, 4       ) ) tCnt<-tCnt+1
						if( aCStep[2]%in%c(  8          ) ) tCnt<-tCnt+1
						if( aCStep[3]%in%c(  1, 9,11    ) ) tCnt<-tCnt+1
						if( aCStep[4]%in%c(  9          ) ) tCnt<-tCnt+1
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					cnt <- 0
					# [  1]  6  1     6  1     2 19     4 11
					# [  2]  3 11     3 11     6 10     1  1
					# [  3]  2 13     2 13     6 11    16 17
					# [  4] 17  5    17  5    13  7    19  2
					# [  5]  1  2     1  2     7  7     4  7
					# [  6]  1  6     1  6     1  8    13  1
					# [  7] 13  2    13  2    16  3    10  9
					# [  8]  4  7     4  7     6  8     5  5
					# [  9] 17  2    17  2     8  4     7  1
					# [ 10] 11 15    11 15     2  2     3  9
					# [ 11]  3 15     3 15     2 11    16  6
					...
					if( all(aCStep[1:2+2]==c(  5, 3 )) ) cnt<-cnt+1
					if( all(aCStep[1:2+2]==c(  6, 9 )) ) cnt<-cnt+1
					if( all(aCStep[1:2+2]==c( 13, 7 )) ) cnt<-cnt+1
					if( all(aCStep[1:2+2]==c( 13, 5 )) ) cnt<-cnt+1
					if( all(aCStep[1:2+2]==c( 14, 3 )) ) cnt<-cnt+1
					if( all(aCStep[1:2+3]==c(  1, 1 )) ) cnt<-cnt+1
					if( all(aCStep[1:2+3]==c(  8, 7 )) ) cnt<-cnt+1
					if( all(aCStep[1:2+3]==c( 14, 1 )) ) cnt<-cnt+1
...
fCutCnt.nextZW <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	auxCntMtx <- matrix( 0 ,nrow=length(allIdxF) ,ncol=2 )	;colnames(auxCntMtx)=c("auxZW","auxQuo")
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					if( (aZoid[6]-aZoid[1]) %in% c( 39,29 ) ) return( FALSE )
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxZW"] <- !flag
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+2]==c(1,4,0)) ) return(FALSE)	# next rebind of 1,2,0
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			...
			if( aZoid[2]%in%c( 34    ) ) cnt<-cnt+1
			if( aZoid[3]%in%c( 31,22 ) ) cnt<-cnt+1
			if( aZoid[4]%in%c(       ) ) cnt<-cnt+1
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			# <16>
			if( fCutU.hasPtn(c( 16,20,NA,45 ),aZoid) ) cnt<-cnt+1
			# <18>
			...
			# <38>
			if( fCutU.hasPtn(c(  3,11,38 ),aZoid) ) cnt<-cnt+1
			# <39>
			if( fCutU.hasPtn(c( 20,26,NA,39 ),aZoid) ) cnt<-cnt+1
			# <42>
			cntMtx[idx,"rawFV"] <- cnt
		}
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 3        ),c( 13    )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 2,7,4    ),c( 34    )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 2        ),c( 31,22 )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 5,3      ),c(       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 1,2      ),c( 22    )) ) cnt<-cnt+1 # 5
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[5]%in%c(  1, 3,15 ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]	# fv in cStep
			if( fCutU.hasPtn(c(  2, 1 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( 1<sum(aCStep[1:2+3]==c( 10,17 )) )	cnt.w1<-cnt.w1+1
			#	unique	( 13, 2, 1 )
			# -------------------------------------------------------------------------------------
			#     FV :    1 (7)   2 (3)   3 (2)   4 (2)   5 (3)   6 (2)   8 (2)   17 (2) 
			cnt.w2 <- 0
			...
			#      1  2 16 22 38 39    | 1 14  6 16  1 | -5  -6  -2 -13  -4  -4 |2 1 1 2 0 |2 1 1 2
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[6]%in%c(  -9, -4,  6 ) ) cnt<-cnt+1
			...
			if( fCutU.hasPtn(c( , ),aFStep) )	cnt.w1<-cnt.w1+1
			#	unique	( -4&-6:-6, 6,-14, -4) ( 6: 4, 6,NA, 7)
			# -------------------------------------------------------------------------------------
			#     FV :    -14 (2)   -6 (2)   -4 (3)   -3 (2)   0 (2)   1 (2)   2 (2)   6 (4) 
			cnt.w2 <- 0
			if( 1<sum( aFStep[ 3 ]*c(3,2,2)==aFStep[c(2,5,6)] ) )	cnt.w2<-cnt.w2+1
			if( 1<sum( aFStep[c(3,6)]*c(3,1)==aFStep[c(2,5)] ) )	cnt.w2<-cnt.w2+1
			if( aFStep[4]==sum(aFStep[c(1,5,6)]) )	cnt.w2<-cnt.w2+1
			if( aFStep[4]==sum(aFStep[c(1,2,3)]) )	cnt.w2<-cnt.w2+1
			if( sum(aFStep[c(2,3)])==sum(aFStep[c(5,6)]) )	cnt.w2<-cnt.w2+1	# -8
...
fCutCnt.nextQuo10 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			cnt <- 0
			if( aZoid[1]%in%c(  1, 7, 6,29, 2  ) ) cnt<-cnt+1
			if( aZoid[2]%in%c( 17             ) ) cnt<-cnt+1
			if( aZoid[3]%in%c(  3              ) ) cnt<-cnt+1
			if( aZoid[4]%in%c( 16, 7           ) ) cnt<-cnt+1
			if( aZoid[5]%in%c(                 ) ) cnt<-cnt+1
			if( aZoid[6]%in%c(                 ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			if( fCutU.hasPtn(c(    16,NA,NA,38 ),aZoid) ) cnt<-cnt+1
			# <25>
			...
			# <38>
			if( fCutU.hasPtn(c(    16,NA,NA,38 ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(  3,10,38 ),aZoid) ) cnt<-cnt+1
			cntMtx[idx,"rawFV"] <- cnt
		}
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 1,7,6,9,2  ),c(  1, 7, 6,29, 2 )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 7          ),c( 17             )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 3          ),c(  3             )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 1,6,7      ),c( 16, 7          )) ) cnt<-cnt+1 # 4
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c(          ) ) cnt<-cnt+1
			if( aCStep[2]%in%c( 15,16, 5 ) ) cnt<-cnt+1
			if( aCStep[3]%in%c(  5,13    ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(          ) ) cnt<-cnt+1
			if( aCStep[5]%in%c(  4       ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]	# fv in cStep
			if( fCutU.hasPtn(c(  4, 7 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( 1<sum(aCStep[1:2+2]==c(  1,10 )) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c( 10, 2 ),aCStep) )	cnt.w1<-cnt.w1+1	# -
			#	unique	( 2,20, 5) ( 4, 7, 8) (15,NA, 3,10)
			# -------------------------------------------------------------------------------------
			#     FV :    1 (6)   2 (2)   3 (2)   4 (4)   5 (3)   6 (2)   7 (2)   10 (2)   15 (2) 
			cnt.w2 <- 0
			if( aCStep[4]==sum(aCStep[c(1,5,2)]) )	cnt.w2<-cnt.w2+1
			...
			#      8 12 19 21 31 35    | 4  7  2 10  4 |                        |1 2 1 2 0 |1 2 1 2
			...
			#      1  2 16 22 38 39(2) | 1 14  6 16  1 |  0 -14 -13 -11  -2  -6 |2 1 1 2 0 |2 1 1 2
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]	# fv in fStep
			if( fCutU.hasPtn(c( -14,-12 ),aFStep) )	cnt.w1<-cnt.w1+1	# -
			# -------------------------------------------------------------------------------------
			#     FV :    -14 (3)   -6 (3)   -3 (2)   5 (2)   7 (2)   9 (3) 
			cnt.w2 <- 0
			if( 1<sum( aFStep[ 5 ]*c(7,3)==aFStep[c(2,6)] ) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(2,2,0)) ) return(FALSE)	# next rebind of 0,1,1
					if( all(quoSize[1:3+1]==c(1,2,0)) ) return(FALSE)	# next rebind of 1,1,3
					if( all(quoSize[1:3+1]==c(1,1,0)) ) return(FALSE)	# next rebind of 0,2,2
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			# <19>
			if( fCutU.hasPtn(c( 19,NA,20,23 ),aZoid) ) cnt<-cnt+1
			# <24>
			if( fCutU.hasPtn(c(  9,24       ),aZoid) ) cnt<-cnt+1
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 3        ),c( 23    )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c(          ),c( 31    )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 4,2,3        ),c( 32    )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 8        ),c(       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 4,3,2,5  ),c( 34,33 )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 5,8      ),c( 45    )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c(  2, 1, 3 ) ) cnt<-cnt+1
			...
			if( 1<sum(aCStep[1:2+3]==c( 16,11 )) )	cnt.w1<-cnt.w1+1
			#	unique	( 1, 4, 2 ) (  1, 4,NA,22 ) ( 1, 4,NA,NA,16) ( 3,NA,NA, 1, 8) ( 13,NA, 1, 8)
			# -------------------------------------------------------------------------------------
			#     FV :    1 (5)   2 (2)   3 (3)   4 (4)   5 (3)   6 (2)   8 (2)   10 (2)   15 (2) 
			...
			#     12 15 19 26 40 43    | 3  4  7 14  3 |                        |0 3 1 0 2 |3 1 2
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( fCutU.hasPtn(c( , ),aFStep) )	cnt.w1<-cnt.w1+1
			#	unique	( -1:-7,-1,-16) (-1:-16,-1,-7) (11,12,11) (11,12,NA,14)
			# -------------------------------------------------------------------------------------
			#     FV :    -7 (3)   -6 (2)   -4 (2)   -2 (2)   -1 (2)   1 (2)   2 (2)   3 (2)   4 (2)   12 (3) 
			cnt.w2 <- 0
			if( 1<sum( aFStep[c(1,4)]*c(1,1)==aFStep[c(2,6)] ) )	cnt.w2<-cnt.w2+1
			if( 1<sum( aFStep[ 4 ]*c(3,3)==aFStep[c(1,2)] ) )	cnt.w2<-cnt.w2+1
			if( 1<sum( aFStep[ 6 ]*c(3,3)==aFStep[c(1,2)] ) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextRebNum <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			...
			if( fCutU.remFilt(aZoid[2],c( 0,2      ),c( 10,12 )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 2        ),c( 12    )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 6,3,8    ),c(       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 0,3,8,2  ),c( 33    )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 2,4      ),c( 42    )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c(  9, 4   ) ) cnt<-cnt+1
			if( aCStep[2]%in%c(         ) ) cnt<-cnt+1
			if( aCStep[3]%in%c(  9,11   ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(         ) ) cnt<-cnt+1
			if( aCStep[5]%in%c(         ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]	# fv in cStep
			if( fCutU.hasPtn(c(  7, 1 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( 1<sum(aCStep[1:2+3]==c(  1, 3 )) )	cnt.w1<-cnt.w1+1
			#	unique	( 6,NA, 7, 1) (26, 7, 1)
			# -------------------------------------------------------------------------------------
			#     FV :    1 (6)   4 (5)   5 (2)   6 (2)   7 (2)   8 (3)   10 (2)   11 (2)   14 (2) 
			...
			#      5 10 16 17 31 32    | 5  6  1 14  1 |                        |1 3 0 2 0 |1 3 2
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    -16 (2)   -1 (4)   0 (3)   2 (2)   5 (2)   8 (2)   14 (2) 
			cnt.w2 <- 0
			if( 1<sum( aFStep[c(,)]*c(,)==aFStep[c(,)] ) )	cnt.w2<-cnt.w2+1
			if( sum(aFStep[c(1,3)])==sum(aFStep[c(4,6)]) )	cnt.w2<-cnt.w2+1	# 10
			if( sum(aFStep[c(5,6)])==sum(aFStep[c(2,3,4)]) )	cnt.w2<-cnt.w2+1	# 12
			if( sum(aFStep[c(5,6)])==sum(aFStep[c(2,1,4)]) )	cnt.w2<-cnt.w2+1	# 12
...
fCutCnt.nextCStepBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	auxCntMtx <- matrix( 0 ,nrow=length(allIdxF) ,ncol=2 )	;colnames(auxCntMtx)=c("auxZW","auxQuo")
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					if( (aZoid[6]-aZoid[1]) %in% c( 23,31 ) ) return( FALSE )
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxZW"] <- !flag
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(1,3,1)) ) return(FALSE)	# next rebind of 0,1,1
					if( all(quoSize[1:3+1]==c(2,2,0)) ) return(FALSE)	# next rebind of 1,1,3
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			cnt <- 0
			if( aZoid[1]%in%c(  8,19     ) ) cnt<-cnt+1
			if( aZoid[2]%in%c( 19      ) ) cnt<-cnt+1
			if( aZoid[3]%in%c( 11,21      ) ) cnt<-cnt+1
			if( aZoid[4]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[5]%in%c( 35,36      ) ) cnt<-cnt+1
			if( aZoid[6]%in%c(       ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			if( fCutU.hasPtn(c(  9,NA,NA,35 ),aZoid) ) cnt<-cnt+1
			# <20>
			if( fCutU.hasPtn(c(    20,NA,28    ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c( 19,20,21,NA,32 ),aZoid,thld=3,fixIdx=2) ) cnt<-cnt+1
			# <24>
			if( fCutU.hasPtn(c(  9,24       ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c( 24,27       ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c( 24,NA,NA,40 ),aZoid) ) cnt<-cnt+1
			# <28>
			if( fCutU.hasPtn(c(    20,NA,28    ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c( 19,NA,21,28,32 ),aZoid,thld=3,fixIdx=4) ) cnt<-cnt+1
			# <33>
			# <34>
			cntMtx[idx,"rawFV"] <- cnt
		}
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 8,9,0    ),c(  8,19 )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 5,2,4    ),c( 19    )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 1        ),c( 11,21 )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 3        ),c(       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 5,6      ),c( 35,36 )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c(          ),c(       )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c(  3, 4, 5 ) ) cnt<-cnt+1
			if( aCStep[2]%in%c(          ) ) cnt<-cnt+1
			if( aCStep[3]%in%c(  5, 8    ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(          ) ) cnt<-cnt+1
			if( aCStep[5]%in%c(          ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]	# fv in cStep
			if( fCutU.hasPtn(c(  1, 8 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( 1<sum(aCStep[1:2+3]==c(  5, 4 )) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c(  7, 3 ),aCStep) )	cnt.w1<-cnt.w1+1	# -
			if( fCutU.hasPtn(c(  3, 4 ),aCStep) )	cnt.w1<-cnt.w1+1
			#	unique	( 3,NA,NA, 1, 8) (13,NA, 1, 8) ( 6, 7, 3) ( 3, 3, 4 )
			#	unique  ( a, a, 4 ) 5,5,4  4,4,4
			# -------------------------------------------------------------------------------------
			#     FV :    1 (5)   2 (2)   3 (2)   4 (5)   5 (3)   7 (3)   8 (3)   15 (2) 
			cnt.w2 <- 0
			if( 1<sum( aCStep[ 1 ]*c(1,1,2)==aCStep[c(2,3,4)] ) )	cnt.w2<-cnt.w2+1
			if( 1<sum( aCStep[c(2,1)]*c(1,2)==aCStep[c(3,4)] ) )	cnt.w2<-cnt.w2+1
			...
			#      7  9 24 29 34 38(1) | 2 15  5  5  4 |  1   2  13  12   1  -6 |2 0 2 2 0 |2 2 2
			#     19 21 30 33 34 42(1) | 2  9  3  1  8 | 12  12   6   4   0   4 |0 1 1 3 1 |1 1 3 1
			#     16 20 24 28 36 39    | 4  4  4  8  3 | -3  -1  -6  -5   2  -3 |0 1 3 2 0 |1 3 2
		}
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c(        ) ) cnt<-cnt+1
			...
			#	unique	(11,12,11)
			# -------------------------------------------------------------------------------------
			#     FV :    -7 (2)   -6 (2)   -3 (5)   1 (2)   2 (3)   3 (2)   4 (2)   12 (3) 
			cnt.w2 <- 0
			if( 1<sum( aFStep[ 1 ]*c(2,1)==aFStep[c(3,6)] ) )	cnt.w2<-cnt.w2+1
			if( 1<sum( aFStep[c(1,5)]*c( 1,-3)==aFStep[c(6,3)] ) )	cnt.w2<-cnt.w2+1
			...
			if( aFStep[2]==sum(aFStep[c(6,5)]) )	cnt.w2<-cnt.w2+1
			if( aFStep[3]==sum(aFStep[c(1,6)]) )	cnt.w2<-cnt.w2+1
			if( aFStep[3]==sum(aFStep[c(2,4)]) )	cnt.w2<-cnt.w2+1
			if( aFStep[4]==sum(aFStep[c(1,2,5,6)]) )	cnt.w2<-cnt.w2+1
			if( aFStep[4]==sum(aFStep[c(2,3,5)]) )	cnt.w2<-cnt.w2+1
			if( sum(aFStep[c(1,3)])==sum(aFStep[c(2,4,6)]) )	cnt.w2<-cnt.w2+1	# -9
...
fCutCnt.nextFStepBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( all(quoSize[1:3+2]==c(0,2,0)) ) return(FALSE)	# next rebind of 1,1,1
					#	unique (2 1 1 1)존재. (2 2 0 2) 반복가능? quoSize[2:5]
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			cnt <- 0
			if( aZoid[1]%in%c(  8, 5   ) ) cnt<-cnt+1
			if( aZoid[2]%in%c( 21,13   ) ) cnt<-cnt+1
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			# < 9>
			if( fCutU.hasPtn(c(  9,NA,NA,17,33 ),aZoid) ) cnt<-cnt+1
			...
			if( fCutU.hasPtn(c(  9,NA,13       ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(    10,13       ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c(       13,25,41 ),aZoid) ) cnt<-cnt+1
			# <19>
			...
			# <38>
			if( fCutU.hasPtn(c( 11, 2, 7,26,38 ),aZoid,thld=3,fixIdx=5) ) cnt<-cnt+1
			# <43>
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 5        ),c(  8, 5 )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 1,3      ),c( 21,13 )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 2,0,4    ),c( 10    )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 0        ),c(       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 3        ),c( 29    )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 1,9      ),c( 41    )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			...
			if( aCStep[4]%in%c( 15      ) ) cnt<-cnt+1
			if( aCStep[5]%in%c(  7, 8   ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]	# fv in cStep
			if( fCutU.hasPtn(c(  1, 3 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( 1<sum(aCStep[1:2+0]==c( 12, 2 )) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c( 11, 1 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( 1<sum(aCStep[1:2+0]==c(  3,15 )) )	cnt.w1<-cnt.w1+1
			#	unique	( 1, 3,12) ( 1, 3,NA,16) (11, 1, 5) (11, 1,NA, 2)
			# -------------------------------------------------------------------------------------
			#     FV :    1 (6)   2 (3)   3 (3)   4 (2)   7 (2)   8 (3)   11 (4) 
			cnt.w2 <- 0
			...
			#      9 10 13 28 38 45(1) | 1  3 15 10  7 | -1 -11  -9  -2   3   3 |1 2 1 1 1 |1 2 1 1 1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			#	unique	()
			# -------------------------------------------------------------------------------------
			#     FV :    -9 (2)   -5 (2)   -2 (4)   -1 (2)   2 (2)   3 (2)   4 (3)   5 (2)   8 (2) 
			cnt.w2 <- 0
			if( aFStep[2]==sum(aFStep[c(2,3)]) )	cnt.w2<-cnt.w2+1
			if( aFStep[3]==sum(aFStep[c(1,2,5)]) )	cnt.w2<-cnt.w2+1
			if( aFStep[3]==sum(aFStep[c(1,2,6)]) )	cnt.w2<-cnt.w2+1
			if( sum(aFStep[c(1,4)])==sum(aFStep[c(3,5,6)]) )	cnt.w2<-cnt.w2+1	# -3
			if( sum(aFStep[c(2,5)])==sum(aFStep[c(3,4,6)]) )	cnt.w2<-cnt.w2+1	# -8
...
fCutCnt.nextColVal_1 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	auxCntMtx <- matrix( 0 ,nrow=length(allIdxF) ,ncol=2 )	;colnames(auxCntMtx)=c("auxZW","auxQuo")
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					if( (aZoid[6]-aZoid[1]) %in% c( 23 ) ) return( FALSE )
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxZW"] <- !flag
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					...
					if( all(quoSize[1:3+1]==c(0,1,1)) ) return(FALSE)	# next rebind of 1,1,2
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			cnt <- 0
			if( aZoid[1]%in%c(          ) ) cnt<-cnt+1
			if( aZoid[2]%in%c( 37       ) ) cnt<-cnt+1
			if( aZoid[3]%in%c(          ) ) cnt<-cnt+1
			if( aZoid[4]%in%c(          ) ) cnt<-cnt+1
			if( aZoid[5]%in%c( 31,33,10 ) ) cnt<-cnt+1
			if( aZoid[6]%in%c(          ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			if( fCutU.hasPtn(c(    21,24,29,25 ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
			# <25>
			if( fCutU.hasPtn(c(    25,32    ),aZoid) ) cnt<-cnt+1
			if( fCutU.hasPtn(c( 14,25,NA,41 ),aZoid) ) cnt<-cnt+1
			# <33>
			if( fCutU.hasPtn(c( 26,NA,33,NA,44 ),aZoid) ) cnt<-cnt+1
			# <34>
			# <36>
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			...
			if( fCutU.remFilt(aZoid[2],c( 7        ),c( 37       )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 3,7,9    ),c(          )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 3        ),c(          )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 1,3,0    ),c( 31,33,10 )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 2,4      ),c(          )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c(  8       ) ) cnt<-cnt+1
			if( aCStep[2]%in%c( 10, 9, 4 ) ) cnt<-cnt+1
			if( aCStep[3]%in%c(  8, 1, 9 ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(          ) ) cnt<-cnt+1
			if( aCStep[5]%in%c(  1       ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]	# fv in cStep
			if( fCutU.hasPtn(c(  3, 1 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( 1<sum(aCStep[1:2+2]==c(  1, 6 )) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c(  7,10 ),aCStep) )	cnt.w1<-cnt.w1+1	# -
			#	unique	( 12, 3, 1) (15, 7,10)
			#	unique	3>sum(aCStep[1:3+1]==c( 7,13,1))  <-- 5/3/( 7,13, 1)
			#			3>sum(aCStep[1:2+0]==c( 1, 8))  <-- 9/2/( 1, 8)
			# -------------------------------------------------------------------------------------
			#     FV :    1 (5)   2 (2)   3 (3)   5 (3)   6 (2)   7 (4)   8 (4)   9 (3) 
			cnt.w2 <- 0
			if( 1<sum( aCStep[c(1,3)]*c(4,3)==aCStep[c(5,2)] ) )	cnt.w2<-cnt.w2+1
			if( aCStep[3]==sum(aCStep[c(1,4)]) )	cnt.w2<-cnt.w2+1
			if( aCStep[2]==sum(aCStep[c(4,5)]) )	cnt.w2<-cnt.w2+1
			...
			#     19 21 30 33 34 42(1) | 2  9  3  1  8 |  3  -4  -3  -5  -6  -3 |0 1 1 3 1 |1 1 3 1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( 1<sum(aFStep[1:2+0]==c( 6,10 )) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c( -6,-2 ),aFStep) )	cnt.w1<-cnt.w1+1	# -
			#	unique	( 0,NA,NA,-6,-2) (-8,NA,-6,-4) ( 2,-6,-2)
			# -------------------------------------------------------------------------------------
			#     FV :    -8 (2)   -6 (3)   -5 (2)   -4 (4)   -3 (2)   1 (2)   3 (4)   6 (2)   10 (2)   15 (2) 
			cnt.w2 <- 0
			if( 1<sum( aFStep[ 1 ]*c(-1,-2,-1)==aFStep[c(3,5,6)] ) )	cnt.w2<-cnt.w2+1
			if( 1<sum( aFStep[c(1,3)]*c(-2, 1)==aFStep[c(5,6)] ) )	cnt.w2<-cnt.w2+1
			if( sum(aFStep[c(2,4)])==sum(aFStep[c(3,5)]) )	cnt.w2<-cnt.w2+1	# 9
			if( sum(aFStep[c(2,4)])==sum(aFStep[c(6,5)]) )	cnt.w2<-cnt.w2+1	# 9
...
fCutCnt.nextColVal_2 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(1,0,1)) ) return(FALSE)	# next rebind of 0,2,2
					if( all(quoSize[1:3+1]==c(0,2,2)) ) return(FALSE)	# next rebind of 2,2,1
					if( all(quoSize[1:3+2]==c(2,2,0)) ) return(FALSE)	# next rebind of 2,1,1
					...
					#	unique (0 2 2 1 1) rebind. (2 0 2 2 0) again?
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			# <12>
			if( fCutU.hasPtn(c( 12,14,28,30,21,44 ),aZoid,thld=3,fixIdx=1) ) cnt<-cnt+1
			# <16>
			if( fCutU.hasPtn(c(  9,16,35,18 ),aZoid,thld=3,fixIdx=2) ) cnt<-cnt+1
			# <18>
			# <24>
			if( fCutU.hasPtn(c(  2,NA,24,32,29,36 ),aZoid,thld=3,fixIdx=3) ) cnt<-cnt+1
			# <26>
			if( fCutU.hasPtn(c(  6, 8,26 ),aZoid) ) cnt<-cnt+1
			# <38>
			if( fCutU.hasPtn(c(  4,33,NA,NA,38 ),aZoid) ) cnt<-cnt+1
			# <42>
			if( fCutU.hasPtn(c( 26,NA,38,NA,42 ),aZoid) ) cnt<-cnt+1
			# <43>
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			...
			if( fCutU.remFilt(aZoid[2],c( 8,5,7    ),c(    )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 6        ),c(    )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 4        ),c( 44 )) ) cnt<-cnt+1 # 4
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[2]%in%c( 10,15   ) ) cnt<-cnt+1
			...
			#	unique	( 6, 1, 25 ) (14,NA, 2,11)
			# -------------------------------------------------------------------------------------
			#     FV :    1 (6)   2 (5)   4 (2)   5 (2)   6 (3)   7 (2)   10 (2)   12 (2) 
			cnt.w2 <- 0
			...
			#     14 15 16 17 38 45    | 1  1  1 21  7 |                        |0 4 0 1 1 |4 1 1
			...
			#     12 16 26 28 30 42(1) | 4 10  2  2 12 |  6   8   8  -7 -12  -1 |0 2 2 1 1 |2 2 1 1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			#	unique	( 6, 8,11)
			# -------------------------------------------------------------------------------------
			#     FV :    -6 (2)   -5 (2)   -4 (2)   -2 (2)   -1 (3)   6 (2)   8 (3)   9 (2) 
			cnt.w2 <- 0
			if( 1<sum( aFStep[c(1,2)]*c(-2,1)==aFStep[c(5,3)] ) )	cnt.w2<-cnt.w2+1
			if( aFStep[6]==sum(aFStep[c(1,4)]) )	cnt.w2<-cnt.w2+1
			if( sum(aFStep[c(2,6)])==sum(aFStep[c(1,3,4)]) )	cnt.w2<-cnt.w2+1	# 7
...
fCutCnt.nextColVal_3 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+2]==c(1,2,2)) ) return(FALSE)	# next rebind of 2,0,1
					if( all(quoSize[1:3+1]==c(1,1,2)) ) return(FALSE)	# next rebind of 0,1,1
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			cnt <- 0
			if( aZoid[1]%in%c(  1, 8 ) ) cnt<-cnt+1
			if( aZoid[2]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[3]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[4]%in%c( 13    ) ) cnt<-cnt+1
			if( aZoid[5]%in%c(       ) ) cnt<-cnt+1
			if( aZoid[6]%in%c( 44,38 ) ) cnt<-cnt+1
			cntMtx[idx,"raw"] <- cnt
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			# <44>
			if( fCutU.hasPtn(c( 15,17,44 ),aZoid) ) cnt<-cnt+1
			cntMtx[idx,"rawFV"] <- cnt
		}
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 0,1,8     ),c(  1, 8 )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 2,3,6     ),c(       )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 7,2       ),c(       )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 5,3       ),c( 13    )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 1,5,7,4   ),c(       )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 4,8,2     ),c( 44,38 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c(  11, 1  ) ) cnt<-cnt+1
			if( aCStep[2]%in%c(         ) ) cnt<-cnt+1
			...
			if( fCutU.hasPtn(c( 11, 4 ),aCStep) )	cnt.w1<-cnt.w1+1	# -
			if( fCutU.hasPtn(c(  6,20 ),aCStep) )	cnt.w1<-cnt.w1+1
			#	unique	( 1, 3, 5) ( 1, 3,NA,NA,11) ( 3, 4,NA,11) (12,NA,11, 4) (11, 6,28) ( 6,20, 2)
			#	unique ( 1,11) ( 8, 3)
			# -------------------------------------------------------------------------------------
			#     FV :    1 (4)   2 (3)   3 (4)   5 (3)   6 (2)   7 (2)   9 (2)   10 (2)   11 (3)   20 (2) 
			cnt.w2 <- 0
			...
			#      1 12 13 24 29 44(1) |11  1 11  5 15 | -1   5 -14  -9 -12   0 |1 2 2 0 1 |1 2 2 1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[3]%in%c(        ) ) cnt<-cnt+1
			if( aFStep[4]%in%c(        ) ) cnt<-cnt+1
			if( aFStep[5]%in%c(  5,-12 ) ) cnt<-cnt+1
			if( aFStep[6]%in%c(  6     ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]	# fv in fStep
			if( fCutU.hasPtn(c(  5, 6 ),aFStep) )	cnt.w1<-cnt.w1+1
			if( 1<sum(aFStep[1:2+4]==c(  5, 6 )) )	cnt.w1<-cnt.w1+1
			#	unique	( 1,NA,NA,NA, 5, 6) (-9,NA,NA, 5, 6)
			# -------------------------------------------------------------------------------------
			#     FV :    -11 (3)   -10 (2)   0 (2)   3 (2)   5 (4)   6 (2)   8 (2) 
			cnt.w2 <- 0
...
fCutCnt.nextColVal_4 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(1,2,0)) ) return(FALSE)	# next rebind of 0,2,0
					if( all(quoSize[1:3+1]==c(0,2,0)) ) return(FALSE)	# next rebind of 2,0,2
					if( all(quoSize[1:3+2]==c(2,0,2)) ) return(FALSE)	# next rebind of 0,2,2
					#	unique (2 0 2 2)... quoSize[2:5] (0 2 0 2) 가능?
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){	# raw
			...
			if( aZoid[2]%in%c( 12,31  ) ) cnt<-cnt+1
			if( aZoid[3]%in%c(        ) ) cnt<-cnt+1
		...
		if( TRUE ){ # rawFV		# anaMtx.freqVal( stdMI$rawTail )
			...
			if( fCutU.hasPtn(c(  3,10,24,NA,41 ),aZoid,thld=3,fixIdx=5) ) cnt<-cnt+1
			cntMtx[idx,"rawFV"] <- cnt
		}
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 3        ),c(  3    )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 7,2        ),c( 12,31 )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 5,9        ),c(       )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 9,8,0        ),c( 39    )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 2,1,5        ),c(       )) ) cnt<-cnt+1 # 5
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			#	unique	(27,NA, 4, 4)
			# -------------------------------------------------------------------------------------
			#     FV :    1 (5)   2 (3)   4 (3)   5 (4)   6 (3)   9 (3)   10 (2)   13 (2) 
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[3]%in%c(  7, 0  ) ) cnt<-cnt+1
			if( aFStep[4]%in%c(        ) ) cnt<-cnt+1
			if( aFStep[5]%in%c(        ) ) cnt<-cnt+1
			if( aFStep[6]%in%c(        ) ) cnt<-cnt+1
			...
			if( fCutU.hasPtn(c(  6, 9 ),aFStep) )	cnt.w1<-cnt.w1+1	# -
			#	unique	( 9, 6,14 ) ( 9, 6,NA, 3) ( 9, 6,NA,NA, 8)
			#			( 1,NA, 6, 9) (12, 6, 9) ( 6, 9,14) ( 6, 9,NA, 1)
			# -------------------------------------------------------------------------------------
			#     FV :    -8 (2)   -5 (2)   -1 (2)   0 (4)   2 (2)   5 (2)   6 (3)   7 (2)   9 (2) 
			cnt.w2 <- 0
			if( aFStep[2]==sum(aFStep[c(1,6)]) )	cnt.w2<-cnt.w2+1
			if( aFStep[4]==sum(aFStep[c(3,6)]) )	cnt.w2<-cnt.w2+1
			if( sum(aFStep[c(1,3)])==sum(aFStep[c(5,6)]) )	cnt.w2<-cnt.w2+1	# 11
			if( sum(aFStep[c(1,2)])==sum(aFStep[c(4,6)]) )	cnt.w2<-cnt.w2+1	# 14
			if( sum(aFStep[c(2,6)])==sum(aFStep[c(3,5)]) )	cnt.w2<-cnt.w2+1	# 13
			if( sum(aFStep[c(1,4)])==sum(aFStep[c(2,3)]) )	cnt.w2<-cnt.w2+1	# 15
			if( sum(aFStep[c(2,5)])==sum(aFStep[c(1,4)]) )	cnt.w2<-cnt.w2+1	# 16
...
fCutCnt.nextColVal_5 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+0]==c(2,1,2)) ) return(FALSE)	# next rebind of 0,3,1
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			cnt <- 0
			if( fCutU.remFilt(aZoid[1],c( 4,2 ),c(       )) ) cnt<-cnt+1 # 1
			if( fCutU.remFilt(aZoid[2],c( 3,9 ),c(       )) ) cnt<-cnt+1 # 2
			if( fCutU.remFilt(aZoid[3],c( 6,4 ),c( 26    )) ) cnt<-cnt+1 # 3
			if( fCutU.remFilt(aZoid[4],c( 7   ),c(       )) ) cnt<-cnt+1 # 4
			if( fCutU.remFilt(aZoid[5],c( 6   ),c(       )) ) cnt<-cnt+1 # 5
			if( fCutU.remFilt(aZoid[6],c( 4,8 ),c( 44,38 )) ) cnt<-cnt+1 # 6
			cntMtx[idx,"rem"] <- cnt
		}
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c(  1, 6     ) ) cnt<-cnt+1
			if( aCStep[2]%in%c(  3,26, 6  ) ) cnt<-cnt+1
			...
			if( 1<sum(aCStep[1:2+0]==c(  1, 5 )) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c(  5, 8 ),aCStep) )	cnt.w1<-cnt.w1+1	# -
			#	unique	( 6, 3,NA,18) 
			# -------------------------------------------------------------------------------------
			#     FV :    1 (5)   2 (2)   3 (5)   6 (4)   7 (2)   8 (3)   10 (2) 
			...
			#     12 15 18 28 34 42    | 3  3 10  6  8 |                        |0 3 1 1 1 |3 1 1 1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( 1<sum(aFStep[1:2+3]==c( 14, 8 )) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c( -3,-1 ),aFStep) )	cnt.w1<-cnt.w1+1	# -
			#	unique	( -9,-3,-1) (-16,-9,-3) (-16,NA,-9,-1)
			# -------------------------------------------------------------------------------------
			#     FV :    -12 (2)   -11 (3)   -9 (3)   -8 (2)   -3 (3)   5 (2)   9 (2) 

			cnt.w2 <- 0
			if( 1<sum( aFStep[ 1 ]*c( 1,-1)==aFStep[c(2,3)] ) )	cnt.w2<-cnt.w2+1
			if( 1<sum( aFStep[ 5 ]*c(4,3)==aFStep[c(3,4)] ) )	cnt.w2<-cnt.w2+1
			if( 1<sum( aFStep[c(1,6)]*c(1,6)==aFStep[c(2,3)] ) )	cnt.w2<-cnt.w2+1
			if( aFStep[3]==sum(aFStep[c(3,4)]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_6 <- function( gEnv ,allIdxF ,rpt=FALSE ){
    ...
    flag <- apply( gEnv$allZoidMtx[allIdxF,,drop=F] ,1 ,function( aZoid ){
					quoSize <- fCutU.getQuoObj( aZoid )$size
					if( all(quoSize[1:3+2]==c(1,3,1)) ) return(FALSE)	# next rebind of 1,2,0
					return( TRUE )
				})	;kIdx<-anaFlagFnd(!flag,rpt)
	auxCntMtx[,"auxQuo"] <- !flag
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # rem		# u0.zoidMtx_ana( stdMI$rawTail%%10 )
			...
			if( fCutU.remFilt(aZoid[2],c( 6,2     ),c(    )) ) cnt<-cnt+1 # 2
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[2]%in%c(  3,16    ) ) cnt<-cnt+1
			if( aCStep[3]%in%c(          ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(  1       ) ) cnt<-cnt+1
			if( aCStep[5]%in%c(  7, 2     ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]	# fv in cStep
			if( fCutU.hasPtn(c(  4, 3 ),aCStep) )	cnt.w1<-cnt.w1+1	# unique 
			if( 1<sum(aCStep[1:2+3]==c( 19, 7 )) )	cnt.w1<-cnt.w1+1
			if( 1<sum(aCStep[1:2+2]==c(  2, 8 )) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c(  2, 4 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( 1<sum(aCStep[1:2+1]==c( 19, 7 )) )	cnt.w1<-cnt.w1+1
			#	unique	( 2, 4, 1) (11,NA, 4, 3)
			#	unique	aCStep[1:2+3] c(20, 8)   4/2/1/(19, 7) --> 4/3/2/(20, 8)
			# -------------------------------------------------------------------------------------
			#     FV :    1 (2)   2 (3)   3 (4)   4 (6)   6 (3)   9 (3)   18 (2) 
			cnt.w2 <- 0
			if( 1<sum( aCStep[c(1,5)]*c(2,2)==aCStep[c(2,3)] ) )	cnt.w2<-cnt.w2+1
			if( 1<sum( aCStep[ 5 ]*c(3,8,4)==aCStep[c(2,3,5)] ) )	cnt.w2<-cnt.w2+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( 1<sum(aFStep[1:2+3]==c( -8,10 )) )	cnt.w1<-cnt.w1+1
			#	unique	( 5, 4, 5)
			# -------------------------------------------------------------------------------------
			#     FV :    -14 (2)   -8 (2)   -3 (2)   2 (3)   3 (2)   4 (2)   5 (2) 
			cnt.w2 <- 0
			if( aFStep[3]==sum(aFStep[c(2,5)]) )	cnt.w2<-cnt.w2+1
			if( sum(aFStep[c(1,6)])==sum(aFStep[c(3,4)]) )	cnt.w2<-cnt.w2+1	# -2
			if( sum(aFStep[c(2,6)])==sum(aFStep[c(1,3,5)]) )	cnt.w2<-cnt.w2+1	# -8
			if( sum(aFStep[c(3,5)])==sum(aFStep[c(2,4,6)]) )	cnt.w2<-cnt.w2+1	# -3
			if( sum(aFStep[c(1,4)])==sum(aFStep[c(3,5,6)]) )	cnt.w2<-cnt.w2+1	#  0

...
fCutCnt.basic <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( all(aCStep[1:2+0]==c( 9, 2)) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c( 3, 1 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( all(aCStep[1:2+2]==c( 2, 4)) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c( 8, 5 ),aCStep) )	cnt.w1<-cnt.w1+1
			if( all(aCStep[1:2+2]==c( 2, 4)) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c( 2, 3 ),aCStep) )	cnt.w1<-cnt.w1+1	# -
			# -------------------------------------------------------------------------------------
			#     FV :    1 (6)   2 (2)   3 (3)   5 (4)   8 (2)   9 (3)   11 (3) 

			cnt.w2 <- 0
			if( 1<sum( aCStep[c(2,4)]*c(2,3)==aCStep[c(3,5)] ) )	cnt.w2<-cnt.w2+1
			if( 1<sum( aCStep[c(2,4)]*c(2,3)==aCStep[c(3,1)] ) )	cnt.w2<-cnt.w2+1
			if( 1<sum( aCStep[5]*c(3,3)==aCStep[c( 1,5 )] ) )	cnt.w2<-cnt.w2+1
			if( aCStep[1]==sum(aCStep[2:4]) )	cnt.w2<-cnt.w2+1
			if( aCStep[5]==sum(aCStep[2:4]) )	cnt.w2<-cnt.w2+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			# -------------------------------------------------------------------------------------
			#     FV :    -9 (3)   -8 (2)   -6 (3)   2 (2)   6 (2)   8 (2)   10 (2)   11 (2)   15 (2) 
...
fCutCnt.nextZW <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[4]%in%c( 12, 1, 3 ) ) cnt<-cnt+1
			...
			cnt.w2 <- 0
			if( all(aCStep[4:5] == (aCStep[1]*2:3) ) )	cnt.w2<-cnt.w2+1
			if( aCStep[5]==sum(aCStep[c(1,5)]) )	cnt.w2<-cnt.w2+1
			if( aCStep[5]==(aCStep[1]*aCStep[2]) )	cnt.w2<-cnt.w2+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[5]%in%c( -4, -7 ) ) cnt<-cnt+1
			if( aFStep[6]%in%c(  0,  2 ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			cnt.w2 <- 0
			if( aFStep[5]==aFStep[6] )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextQuo10 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]
			cnt.w2 <- 0
			if( (aCStep[1]==aCStep[3]) )	cnt.w2<-cnt.w2+1
			if( all(aCStep[1:2]==aCStep[c(3,5)]) )	cnt.w2<-cnt.w2+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w2 <- 0
			if( aFStep[5]==aFStep[6] )	cnt.w2<-cnt.w2+1
			if( sum(aFStep[c(2,5)])==sum(aFStep[c(3,6)]) )	cnt.w2<-cnt.w2+1
			if( aFStep[1]==sum(aFStep[c(3,4,5,6 )]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			# -------------------------------------------------------------------------------------
			#     FV :    1(5)   2(2)   3(2)   5(3)   6(3)   7(3)   8(2)   9(2)   10(4)   12(2) 
			cnt.w2 <- 0
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c( -3,  7 ) ) cnt<-cnt+1
			if( aFStep[2]%in%c(        ) ) cnt<-cnt+1
			...
			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			cnt.w2 <- 0
			if( sum(aFStep[c(3,4)])==sum(aFStep[c(2,6)]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextRebNum <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			cnt.w2 <- 0
			if( all(aFStep[c(1,3)]== aFStep[5]*c(-1,-1) ) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextCStepBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[3]%in%c(  3       ) ) cnt<-cnt+1
			if( aCStep[4]%in%c(          ) ) cnt<-cnt+1
			if( aCStep[5]%in%c(  4, 3, 2 ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]
			if( fCutU.hasPtn(c( 1,11 ),aCStep) )	cnt.w1<-cnt.w1+1
			cnt.w2 <- 0
			if( (aCStep[1]==aCStep[3]) && (aCStep[3]==aCStep[4]) )	cnt.w2<-cnt.w2+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			cnt.w2 <- 0
			if( (aFStep[1]==aFStep[4]) && (aFStep[4]==-aFStep[6]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextFStepBin <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			cnt.w2 <- 0
			if( all(aFStep[2:4]== aFStep[6]*c(-6,-6,-3)) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_1 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c( 8,12, 6    ) ) cnt<-cnt+1
			if( aCStep[2]%in%c( 5, 3,11,27 ) ) cnt<-cnt+1
			...
			if( sum(aCStep[c(1,4)])==sum(aCStep[c(2,5)]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_2 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w2 <- 0
			if( all( aCStep[4:5]==(aCStep[1]*2:3)) )	cnt.w2<-cnt.w2+1
			if( all(aCStep[c(1,4)]== aCStep[2]*c(2,4) ) )	cnt.w2<-cnt.w2+1
			if( aCStep[5]==sum(aCStep[c(1,5)]) )	cnt.w2<-cnt.w2+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( sum(aFStep[c(2,5)])==sum(aFStep[c(1,3,4)]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_3 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( aCStep[1]== 2*sum(aCStep[c(2,4)]) )	cnt.w2<-cnt.w2+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[3]%in%c(        ) ) cnt<-cnt+1
			if( aFStep[4]%in%c(  7, 11 ) ) cnt<-cnt+1
			...
			if( all(aFStep[2:3]== aFStep[1]*c(2,2) ) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_4 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aCStep[1]%in%c(  2, 6, 9 ) ) cnt<-cnt+1
			...
			if( 1<sum(aCStep[1:2+2]==c( 1,7 )) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c(  2, 9 ),aCStep) )	cnt.w1<-cnt.w1+1
			# if( 1<sum(aCStep[1:2+0]==c( 2,9 )) )	cnt.w1<-cnt.w1+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			cnt <- 0
			if( aFStep[1]%in%c(-10, -4     ) ) cnt<-cnt+1
			if( aFStep[2]%in%c(-11,  6,  3 ) ) cnt<-cnt+1
			if( aFStep[3]%in%c( -1,  4     ) ) cnt<-cnt+1
			if( aFStep[4]%in%c(            ) ) cnt<-cnt+1
			if( aFStep[5]%in%c( -4         ) ) cnt<-cnt+1
			...
			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			if( fCutU.hasPtn(c( -10,-11 ),aFStep) )	cnt.w1<-cnt.w1+1	# -
			if( fCutU.hasPtn(c(  -4, -3 ),aFStep) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c(   6, -1 ),aFStep) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c(  -2,  1 ),aFStep) )	cnt.w1<-cnt.w1+1
			if( fCutU.hasPtn(c(   1,  2 ),aFStep) )	cnt.w1<-cnt.w1+1
			# -------------------------------------------------------------------------------------
			#     FV :    -12 (2)   -11 (3)   -4 (2)   -1 (2)   1 (3)   4 (3)   5 (4)   14 (2) 
			cnt.w2 <- 0
			if( 1<sum( aFStep[c(1,4)]*c( 1,-1)==aFStep[c(3,5)] ) )	cnt.w2<-cnt.w2+1
			if( aFStep[2]==sum(aFStep[c(1,3,4,6)]) )	cnt.w2<-cnt.w2+1
			if( sum(aFStep[c(1,4)])==sum(aFStep[c(5,6)]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_5 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			cnt.w1 <- cccObj$scoreMtx[idx,"w1CStep.cnt"]
			cnt.w2 <- 0
			if( all(aCStep[c(1,5)]==(aCStep[4]*3:4)) )	cnt.w2<-cnt.w2+1
			if( aCStep[3]==sum(aCStep[c(4,5)]) )	cnt.w2<-cnt.w2+1
		...
		if( TRUE ){ # fStep		#	u0.zoidFMtx_ana.rpt( stdMI$rawTail )
			...
			if( aFStep[4]%in%c(            ) ) cnt<-cnt+1
			if( aFStep[5]%in%c(            ) ) cnt<-cnt+1
			if( aFStep[6]%in%c(  0, -5, -1 ) ) cnt<-cnt+1

			cnt.w1 <- cccObj$scoreMtx[idx,"w1FStep.cnt"]
			cnt.w2 <- 0
			if( (aFStep[1]==aFStep[4]) && (aFStep[2]==aFStep[3]) )	cnt.w2<-cnt.w2+1
...
fCutCnt.nextColVal_6 <- function( gEnv ,allIdxF ,rpt=FALSE ){
	...
	for( idx in seq_len(length(allIdxF)) ){
		...
		if( TRUE ){ # cStep		#	u0.zoidCMtx_ana.rpt( stdMI$rawTail )
			...
			if( all(aCStep[1:2+0]==c( 3, 5)) )	cnt.w1<-cnt.w1+1
			if( all(aCStep[1:2+0]==c( 6, 1)) )	cnt.w1<-cnt.w1+1
